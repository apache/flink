...
class ScalaCollectionSerializerTest {
  ...
  @Test
  def testBitSet(): Unit = {
    val testData = Array(BitSet(1,2,3), BitSet(2,3,2))
    runTests(testData)
  }
...
//
  ...
  private final def runTests[T : TypeInformation](instances: Array[T]) {
    try {
      ...
      val test = new ScalaCollectionSerializerTestInstance[T](serializer, typeClass, -1, instances)
      test.testAll()
    } catch {
      case e: Exception => {
