...
class ScalaCsvReaderWithPOJOITCase(mode: TestExecutionMode) extends MultipleProgramsTestBase(mode) {
  ...
  def testPOJOType(): Unit = {
    ...
    val data = env.readCsvFile[POJOItem](dataPath, pojoFieldsOrder = Array("f1", "f2", "f3"))
  ...
  def testPOJOTypeWithFieldsOrder(): Unit = {
    ...
    val env = ExecutionEnvironment.getExecutionEnvironment
    val data = env.readCsvFile[POJOItem](dataPath, pojoFieldsOrder = Array("f2", "f1", "f3"))
