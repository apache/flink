...
object BroadcastStateJobMigrationITCase {
...
class BroadcastStateJobMigrationITCase(
  ...
  def testCreateSavepointWithBroadcastState(): Unit = {
    ...
    BroadcastStateJobMigrationITCase.GENERATE_SAVEPOINT_BACKEND_TYPE match {
    ...
    executeAndSavepoint(
      ...
      s"src/test/resources/stateful-scala-with-broadcast" +
        s"${BroadcastStateJobMigrationITCase.SCALA_VERSION}" +
        s"-udf-migration-itcase-flink" +
        s"${BroadcastStateJobMigrationITCase.GENERATE_SAVEPOINT_VER}" +
        s"-${BroadcastStateJobMigrationITCase.GENERATE_SAVEPOINT_BACKEND_TYPE}-savepoint",
      new Tuple2(
        AccumulatorCountingSink.NUM_ELEMENTS_ACCUMULATOR,
        BroadcastStateJobMigrationITCase.NUM_ELEMENTS
      )
    )
  }
  ...
  def testRestoreSavepointWithBroadcast(): Unit = {
    ...
    restoreAndExecute(
      ...
      SavepointMigrationTestBase.getResourceFilename(
        s"stateful-scala-with-broadcast${BroadcastStateJobMigrationITCase.SCALA_VERSION}" +
...
class TestBroadcastProcessFunction
  ...
  override def processElement(
      ...
      ctx: KeyedBroadcastProcessFunction
        [Long, (Long, Long), (Long, Long), (Long, Long)]#KeyedReadOnlyContext,
  ...
  override def processBroadcastElement(
      value: (Long, Long),
      ctx: KeyedBroadcastProcessFunction
        [Long, (Long, Long), (Long, Long), (Long, Long)]#KeyedContext,
      out: Collector[(Long, Long)]): Unit = {
