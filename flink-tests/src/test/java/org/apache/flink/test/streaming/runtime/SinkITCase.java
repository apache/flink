...
import org.apache.flink.streaming.runtime.operators.sink.TestSink.ExplosiveGlobalCommitter;
...
public class SinkITCase extends AbstractTestBase {
    ...
    // The global committer throws exception on 3rd checkpoint (commitNumberToFailOn == 2).
    // The local mini cluster is executing the recovery, we should expect no data loss,
    // however global committer is missing records, where in the same time committers are not.
    @Test
    public void testGlobalCommitterMissingRecordsDuringRecovery() throws Exception {
        ...
        assertAll(
                () -> {
                    // we observe no data loss on committer side.
                    assertThat(committerData.size(), equalTo(expectedNumberOfRecords));

                    // this will fail, global committer is missing data.
                    assertThat(globalCommittedData.size(), equalTo(expectedNumberOfRecords));
                });
    }
