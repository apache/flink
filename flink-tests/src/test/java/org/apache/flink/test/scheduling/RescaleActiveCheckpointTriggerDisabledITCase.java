...
/**
 * Integration test for the Adaptive Scheduler when the active checkpoint trigger for rescaling is
 * disabled via {@link JobManagerOptions#SCHEDULER_RESCALE_TRIGGER_ACTIVE_CHECKPOINT_ENABLED}.
 */
@ExtendWith(TestLoggerExtension.class)
class RescaleActiveCheckpointTriggerDisabledITCase {

    private static final Logger LOG =
            LoggerFactory.getLogger(RescaleActiveCheckpointTriggerDisabledITCase.class);
    ...
    private static Configuration createConfiguration() {
        ...
        // the feature under test: disable the active checkpoint trigger on the cluster. With it
        // disabled, a resource requirement change must wait for a checkpoint to complete on its own
        // (periodic or manual) rather than the scheduler proactively triggering one.
        configuration.set(
    ...
    @Test
    void testNoActiveTriggerButManualCheckpointStillRescales(
        ...
        try {
            ...
            LOG.info(
                    "Verifying that job {} has NOT rescaled while the active checkpoint trigger is disabled.",
                    jobId);
            waitForRunningTasks(restClusterClient, jobId, BEFORE_RESCALE_PARALLELISM);

            LOG.info("Manually triggering a checkpoint for job {} to drive the rescale.", jobId);
