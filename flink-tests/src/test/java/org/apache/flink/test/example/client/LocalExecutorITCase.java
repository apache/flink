...
public class LocalExecutorITCase extends TestLogger {
    ...
    public void testLocalExecutorWithWordCount() throws InterruptedException {
        try {
            ...
            StreamGraph wcStreamGraph = getWordCountPlan(inFile, outFile, parallelism);
            JobClient jobClient =
    ...
    private StreamGraph getWordCountPlan(File inFile, File outFile, int parallelism) {
        ...
        env.createInput(new TextInputFormat(new Path(inFile.getAbsolutePath())))
                ...
                .addSink(
                        new OutputFormatSinkFunction<>(
                                new CsvOutputFormat<>(new Path(outFile.getAbsolutePath()))));
