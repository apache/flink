...
class AdaptiveBatchSchedulerITCase {
    ...
    void testAdaptiveOptimizeStreamGraph() throws Exception {
        ...
        // Because the parallelism of map2 is automatically determined to be 2, the result will have
        // three times the produced numbers. One part comes from source2, while the other two parts
        // come from the broadcast results of source1.
        Map<Long, Long> expectedResult =
                LongStream.range(0, NUMBERS_TO_PRODUCE)
                        .boxed()
                        .collect(Collectors.toMap(Function.identity(), i -> 3L));
