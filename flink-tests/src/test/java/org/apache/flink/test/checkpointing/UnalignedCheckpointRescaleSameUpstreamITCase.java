...
class UnalignedCheckpointRescaleSameUpstreamITCase {
    ...
    @Parameter private SameUpstreamDag dag;

    @Parameters(name = "dag={0}")
    private static List<SameUpstreamDag> parameters() {
        return Arrays.asList(SameUpstreamDag.values());
    }

    @TestTemplate
    void testRescaleFromUnalignedCheckpointWithSameUpstream(
            @InjectMiniCluster MiniCluster miniCluster) throws Exception {
        final JobGraph initialJobGraph = createJobGraph(null, INITIAL_PARALLELISM);
    ...
    private JobGraph createJobGraph(@Nullable String recoveryPath, int parallelism)
            throws Exception {
        final StreamExecutionEnvironment env =
                StreamExecutionEnvironment.getExecutionEnvironment(
                        createConfiguration(recoveryPath));
        env.disableOperatorChaining();
        dag.build(env, parallelism);
        return env.getStreamGraph().getJobGraph();
    }

    private Configuration createConfiguration(@Nullable String recoveryPath) {
