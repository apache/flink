 ...
 *
 * <p>The topology currently tests the proper behaviour of the {@link StreamGroupedReduce} and the
 * {@link StreamGroupedFold} operators.
 */
...
public class UdfStreamOperatorCheckpointingITCase extends StreamFaultToleranceTestBase {
    ...
    public void testProgram(StreamExecutionEnvironment env) {
        ...
        stream
                ...
                .fold(
                        Tuple2.of(0, 0L),
                        new FoldFunction<Tuple2<Integer, Long>, Tuple2<Integer, Long>>() {
                            @Override
                            public Tuple2<Integer, Long> fold(
                                    Tuple2<Integer, Long> accumulator, Tuple2<Integer, Long> value)
                                    throws Exception {
    ...
    public void postSubmit() {
        ...
        for (int i = 0; i < PARALLELISM; i++) {
            ...
            while (!FoldEvictingQueueSink.queues[i].isEmpty()) {
                ...
                Assert.assertTrue(
                        "Unexpected fold value " + value + " instead of " + sum + ".",
                        value == sum);
            }
        }
    }
