...
public class TaskManagerFailureRecoveryITCase extends TestLogger {
	...
	public void testRestartWithFailingTaskManager() {
		...
		try {
			...
			config.setString(AkkaOptions.AKKA_WATCH_HEARTBEAT_INTERVAL, "500 ms");
			config.setString(AkkaOptions.AKKA_WATCH_HEARTBEAT_PAUSE, "20 s");
			config.setInteger(AkkaOptions.AKKA_WATCH_THRESHOLD, 20);
