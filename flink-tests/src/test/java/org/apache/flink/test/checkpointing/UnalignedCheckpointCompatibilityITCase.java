...
public class UnalignedCheckpointCompatibilityITCase extends TestLogger {
    ...
    private Tuple2<String, Map<String, Object>> runAndTakeSavepoint() throws Exception {
        ...
        CompletableFuture<JobResult> resultFuture =
                miniCluster.getMiniCluster().requestJobResult(jobID);
        Future<String> savepointFuture =
                miniCluster
                        .getMiniCluster()
                        .stopWithSavepoint(jobID, tempFolder().toURI().toString(), false);
        return new Tuple2<>(
                savepointFuture.get(),
                resultFuture
                        .get()
    ...
    private static JobID submitJobInitially(StreamExecutionEnvironment env) throws Exception {
        return miniCluster
                .getClusterClient()
                .submitJob(
                        dag(FIRST_RUN_EL_COUNT, true, FIRST_RUN_BACKPRESSURE_MS, env).getJobGraph())
                .get();
    }
