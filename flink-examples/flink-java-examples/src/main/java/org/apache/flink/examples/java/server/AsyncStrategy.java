...
/**
 * Shows how to use the async update strategy for Parameter Server
 * 
 */
	...
	public static void main(String[] args) throws Exception {
		...
		env.generateSequence(1,4).setParallelism(4).map(new RichMapFunction<Long, Long>() {
			public void open(Configuration parameters) throws Exception {
				getRuntimeContext().registerAsync("KEY", new TestParameter(0L));
			...
			public Long map(Long value) throws Exception {
				...
				// wait for everyone to finish updates
				// eventual consistency
				Thread.sleep(5000);
				if (((TestParameter) getRuntimeContext().fetchParameter("KEY")).value != 50) {
