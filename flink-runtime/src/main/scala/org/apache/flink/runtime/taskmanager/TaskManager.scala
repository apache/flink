...
object TaskManager {
  ...
  def selectNetworkInterfaceAndRunTaskManager(
    ...
    val (taskManagerHostname, actorSystemPortRange) =
      selectNetworkInterfaceAndPortRange(configuration)
  ...
  def runTaskManager(
    ...
    val result = AkkaUtils.retryOnBindException({
      ...
      runTaskManager(taskManagerHostname, resourceID, port, configuration, taskManagerClass)
