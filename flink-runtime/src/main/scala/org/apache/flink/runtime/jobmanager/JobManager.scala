...
import org.apache.flink.runtime.jobmanager.SubmittedJobGraphs.SubmittedJobGraphListener
...
class JobManager(
    ...
    protected val submittedJobGraphs : SubmittedJobGraphs,
      ...
      currentJobs.get(jobID) match {
        case Some((executionGraph, jobInfo)) => executionGraph.getJobName
          ...
          if (newJobStatus.isTerminalState()) {
            ...
            future {
              // TODO If removing the JobGraph from the SubmittedJobGraphs fails, the job will
   ...
   *
   * This should be called asynchronously. Removing the job from the [[SubmittedJobGraphs]] might
   * block. Therefore be careful not to block the actor thread.
   *
...
object JobManager {
      ...
      RecoveryMode.fromConfig(configuration) match {
        case RecoveryMode.STANDALONE =>
          (new StandaloneLeaderElectionService(),
            new StandaloneSubmittedJobGraphs(),
  ...
  def startJobManagerActors(
    ...
    val (executionContext,
      ...
      leaderElectionService,
      submittedJobGraphs,
      checkpointRecoveryFactory) = createJobManagerComponents(configuration)
