...
object AkkaUtils {
  ...
  private def getBasicAkkaConfig(configuration: Configuration): Config = {
    val akkaThroughput = configuration.getInteger(AkkaOptions.AKKA_DISPATCHER_THROUGHPUT)
    val lifecycleEvents = configuration.getBoolean(AkkaOptions.AKKA_LOG_LIFECYCLE_EVENTS)

    val jvmExitOnFatalError = if (
      configuration.getBoolean(AkkaOptions.AKKA_JVM_EXIT_ON_FATAL_ERROR)){
      "on"
    } else {
      "off"
    }
  ...
  private def getRemoteAkkaConfig(configuration: Configuration,
    ...
    val akkaAskTimeout = Duration(configuration.getString(AkkaOptions.AKKA_ASK_TIMEOUT))
    ...
    val akkaEnableSSLConfig = configuration.getBoolean(AkkaOptions.AKKA_SSL_ENABLED) &&
  ...
  def getTimeout(config: Configuration): FiniteDuration = {
    val duration = Duration(config.getString(AkkaOptions.AKKA_ASK_TIMEOUT))
  ...
  def getLookupTimeout(config: Configuration): FiniteDuration = {
    val duration = Duration(config.getString(AkkaOptions.AKKA_LOOKUP_TIMEOUT))
  ...
  def getClientTimeout(config: Configuration): FiniteDuration = {
    val duration = Duration(config.getString(AkkaOptions.AKKA_CLIENT_TIMEOUT))
