...
public class FileSerializableStateHandleV0<T extends Serializable> extends AbstractFileStateHandleV0 implements StateHandleV0<T> {
	 ...
	 */
	public FileSerializableStateHandleV0(Path filePath) {
		super(filePath);
	}
	...
	public T getState(ClassLoader userCodeClassLoader) throws Exception {
		try (FSDataInputStream inStream = getFileSystem().open(getFilePath())) {
			ObjectInputStream ois = new MigrationInstantiationUtil.ClassLoaderObjectInputStream(
				SavepointV0.VERSION, inStream, userCodeClassLoader);
