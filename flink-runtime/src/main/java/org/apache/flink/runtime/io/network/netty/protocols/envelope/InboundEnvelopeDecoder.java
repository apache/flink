...
class InboundEnvelopeDecoder extends ChannelInboundHandlerAdapter implements EventListener<BufferFuture> {
	...
	private BufferFuture currentBufferFuture;
	...
	private boolean decodeBuffer(ByteBuf in, ChannelHandlerContext ctx) throws IOException {
		...
		while ((decoderState = decodeEnvelope(in)) != DecoderState.PENDING) {
			if (decoderState == DecoderState.COMPLETE) {
				...
				Buffer buffer = currentBufferFuture.getBuffer();
				if (buffer == null) {
					...
					currentBufferFuture.addListener(this);
					return false;
				}
				else {
					this.currentEnvelope.setBuffer(buffer);
					this.currentDataBuffer = buffer.getNioBuffer();
					this.currentBufferRequestSize = 0;
				}
			}
			else if (decoderState == DecoderState.RECEIVER_DESTROYED) {
	...
	public void onEvent(BufferFuture bufferFuture) {
		if (bufferFuture.isSuccess()) {
			this.bufferBroker.offer(bufferFuture.getBuffer());
			this.channelHandlerContext.channel().eventLoop().execute(this.bufferAvailabilityChangedTask);
		}
		else {
			throw new RuntimeException("Received buffer future callback without a buffer.");
		}
	}
	...
	private class BufferAvailabilityChangedTask implements Runnable {
		...
		public void run() {
			...
			if (availableBuffer.getSize() != currentBufferRequestSize) {
				availableBuffer.setSize(currentBufferRequestSize);
			}
			currentEnvelope.setBuffer(availableBuffer);
			currentDataBuffer = availableBuffer.getNioBuffer();
	...
	private DecoderState decodeEnvelope(ByteBuf in) throws IOException {
		...
		if (this.currentBufferRequestSize > 0) {
			...
			currentBufferFuture = requestBufferForTarget(jobId, sourceId, this.currentBufferRequestSize);

			if (currentBufferFuture == null) {
				return DecoderState.RECEIVER_DESTROYED;
			}
	...
	private BufferFuture requestBufferForTarget(JobID jobId, ChannelID sourceId, int size) throws IOException {
		...
		return currentBufferProvider.requestBuffer(size);
	}
