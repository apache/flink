...
/**
 *<b>IMPORTANT</b> {@link MemoryStateBackend} is deprecated in favor of
 * {@link org.apache.flink.runtime.state.hashmap.HashMapStateBackend} and
 * {@link org.apache.flink.runtime.state.storage.JobManagerCheckpointStorage}. This change does
 * not affect the runtime characteristics of your Jobs and is simply an API change to help better
 * communicate the ways Flink separates local state storage from fault tolerance. Jobs can be upgraded
 * without loss of state. If configuring your state backend via the {@code StreamExecutionEnvironment}
 ...
 * }</pre>
 *
 *<p>If you are configuring your state backend via the {@code flink-conf.yaml} please make
 ...
 * }</pre>
 *
 *<p>This state backend holds the working state in the memory (JVM heap) of the TaskManagers.
