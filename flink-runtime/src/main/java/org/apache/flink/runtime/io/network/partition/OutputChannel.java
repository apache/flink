...
public class OutputChannel extends Channel {
	...
	private final IntermediateResultPartition outputGate;
	 ...
	 */
	public OutputChannel(IntermediateResultPartition outputGate, int index, ChannelID id, ChannelID connectedId, ChannelType type) {
	...
	public void queueEnvelope(Envelope envelope) {
		...
		for (AbstractEvent event : envelope.deserializeEvents()) {
			if (event.getClass() == ChannelCloseEvent.class) {
				...
				this.outputGate.deliverEvent((TaskEvent) event);
			}
			else {
