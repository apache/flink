...
public class StateBackendLoader {
	 ...
	 * in {@link CoreOptions#STATE_BACKEND}. If no state backend is configured, this instantiates the
	...
	public static StateBackend fromApplicationOrConfigOrDefault(
		...
		if (fromApplication != null) {
			...
			if (fromApplication instanceof ConfigurableStateBackend) {
				...
				StateBackend configured = ((ConfigurableStateBackend) fromApplication).configure(config);
				...
				if (configured instanceof  MemoryStateBackend) {
					MemoryStateBackend memBackend = (MemoryStateBackend) configured;
					if (memBackend.getCheckpointPath() == null && HighAvailabilityMode.isHighAvailabilityModeActivated(config)) {
						...
						if (haStoragePath != null) {
							try {
								...
								tempConfig.setString(AbstractFileStateBackend.CHECKPOINT_PATH, checkpointDirPath.toString());
		...
		// (3) use the default
		final MemoryStateBackend defaultBackend = new MemoryStateBackendFactory().createFromConfig(config);
		if (logger != null) {
			logger.info("No state backend has been configured, using default (Memory / JobManager) {}", defaultBackend);
		}
