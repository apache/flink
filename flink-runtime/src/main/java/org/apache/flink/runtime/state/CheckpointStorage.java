 ...
 *
 *<p>For example, {@link org.apache.flink.runtime.state.storage.JobManagerCheckpointStorage JobManagerCheckpointStorage}
 * stores checkpoints in the memory of the JobManager. It is lightweight and without additional
 * dependencies but is not highly available and only supports small state sizes. This checkpoint
 * storage policy is convenient for local testing and development.
 *
 *<p>{@link org.apache.flink.runtime.state.storage.FileSystemCheckpointStorage FileSystemCheckpointStorage}
 * stores checkpoints in a filesystem. For systems like HDFS, NFS Drives, S3, and GCS, this storage
 * policy supports large state size, in the magnitude of many terabytes while providing a highly
 * available foundation for stateful applications. This checkpoint storage policy is recommended
 ...
 *
 * <p>Because of that, {@code CheckpointStorage} implementations are meant to be like <i>factories</i>
 * that create the proper states stores that provide access to the persistent. That way, the
 * Checkpoint Storage can be very lightweight (contain only configurations) which makes it easier
 * to be serializable.
 *
 * <h2>Thread Safety</h2>
 *
