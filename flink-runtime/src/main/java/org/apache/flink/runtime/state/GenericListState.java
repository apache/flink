 ...
 */
public class GenericListState<K, N, T, Backend extends PartitionedStateBackend<K>, W extends ValueState<ArrayList<T>> & KvState<K, N, ValueState<ArrayList<T>>, Backend>>
	implements ListState<T>, KvState<K, N, ListState<T>, Backend> {
	...
	@Override
	public KvStateSnapshot<K, N, ListState<T>, Backend> snapshot(
		long checkpointId,
		long timestamp) throws Exception {
		KvStateSnapshot<K, N, ValueState<ArrayList<T>>, Backend> wrappedSnapshot = wrappedState.snapshot(
			checkpointId,
			timestamp);
	...
	private static class Snapshot<K, N, T, Backend extends PartitionedStateBackend<K>> implements KvStateSnapshot<K, N, ListState<T>, Backend> {
		private static final long serialVersionUID = 1L;

		private final KvStateSnapshot<K, N, ValueState<ArrayList<T>>, Backend> wrappedSnapshot;

		public Snapshot(KvStateSnapshot<K, N, ValueState<ArrayList<T>>, Backend> wrappedSnapshot) {
			this.wrappedSnapshot = wrappedSnapshot;
		}

		@Override
		@SuppressWarnings("unchecked")
		public KvState<K, N, ListState<T>, Backend> restoreState(
