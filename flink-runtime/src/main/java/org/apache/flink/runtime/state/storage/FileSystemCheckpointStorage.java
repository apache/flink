 ...
 *
 * <p>This checkpoint storage stores small state chunks directly with the metadata, to avoid creating
 ...
 *
 * <p>Checkpoints from this checkpoint storage are as persistent and available as filesystem that
 * is written to. If the file system is a persistent distributed file system, this checkpoint storage
 ...
 * application (by creating the backend with the respective constructor parameters and setting it
...
public class FileSystemCheckpointStorage extends AbstractFileCheckpointStorage
	...
	/**
	 * Creates a new checkpoint storage that stores its checkpoint data in the file system and location
	 ...
	 *
	 * <p>For a Job targeting HDFS, this means that the URI must either specify the authority
	 ...
	 */
	private FileSystemCheckpointStorage(FileSystemCheckpointStorage original, ReadableConfig configuration, ClassLoader classLoader) {
		if (getValidFileStateThreshold(original.fileStateThreshold) >= 0) {
			...
			if (configuredStateThreshold >= 0) {
				...
				LoggerFactory.getLogger(AbstractFileCheckpointStorage.class).warn(
	...
	@Override
	public FileSystemCheckpointStorage configure(
