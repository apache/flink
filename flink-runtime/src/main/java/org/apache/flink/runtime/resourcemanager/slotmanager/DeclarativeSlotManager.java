...
public class DeclarativeSlotManager implements SlotManager {
    ...
    private void cancelAllocationFuture(SlotID slotId) {
        final CompletableFuture<Acknowledge> acknowledgeCompletableFuture =
                pendingSlotAllocationFutures.remove(slotId);
        // the future may be null if we are just re-playing the state transitions due to a slot
        // report
    ...
    private void checkResourceRequirements() {
        ...
        final ResourceCounter pendingSlots =
                new ResourceCounter(
                            ...
                            if (acknowledge != null) {
                                ...
                                if (throwable instanceof SlotOccupiedException) {
                                    ...
                                    if (throwable instanceof CancellationException) {
                                        LOG.debug(
                                                "Cancelled allocation of slot {} for job {}.",
                                                slotId,
                                                jobId,
                                                throwable);
                                    } else {
    ...
    private boolean tryAllocateWorkerAndReserveSlot(
        ...
        if (newlyFulfillableRequirements.isPresent()) {
            ...
            if (newSlots.getNumberOfRequiredSlots() > 1) {
                pendingSlots.incrementCount(
                        newSlots.getResourceProfile(), newSlots.getNumberOfRequiredSlots() - 1);
            }
            return true;
        } else {
            return false;
