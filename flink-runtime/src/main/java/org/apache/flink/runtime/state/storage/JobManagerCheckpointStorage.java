...
/**
 * The {@link CheckpointStorage} checkpoints state directly to the JobManager's memory (hence the name),
 * but savepoints will be persisted to a file system.
 *
 * <p>This checkpoint storage is primarily  for experimentation, quick local setups,
 ...
 * For any other setup, the {@link FileSystemCheckpointStorage} should be used. The
 * {@code FileSystemCheckpointStorage} but checkpoints state directly to files rather than
 * to the JobManager's memory, thus supporting larger state sizes and more highly available
 * recovery.
 *
 ...
 * does guarantee persistence for savepoints, externalized checkpoints (if configured), and checkpoints
 ...
 * application (by creating the storage with the respective constructor parameters and setting it
 * on the execution environment) or by specifying it in the Flink configuration.
 *
 * <p>If the checkpoint storage was specified in the application, it may pick up additional configuration
...
public class JobManagerCheckpointStorage extends AbstractFileCheckpointStorage
	...
	/**
	 * Creates a new JobManagerCheckpointStorage, setting optionally the paths to persist checkpoint metadata
	 ...
	 * and the JobManager needs to be able to hold all aggregated state in its memory.
	 *
	 * @param checkpointPath path to write checkpoint metadata to. If null, the value from
