...
public class FsStateBackend extends AbstractFileStateBackend implements ConfigurableStateBackend {
	 ...
	 * @throws IOException Thrown, if no file system can be found for the scheme in the URI.
	 */
	public FsStateBackend(URI checkpointDataUri, @Nullable URI defaultSavepointDirectory) throws IOException {
		this(checkpointDataUri, defaultSavepointDirectory, DEFAULT_FILE_STATE_THRESHOLD);
	}
	 ...
	 * @throws IllegalArgumentException Thrown, if the {@code fileStateSizeThreshold} is out of bounds.
	 */
	public FsStateBackend(URI checkpointDataUri, @Nullable URI defaultSavepointDirectory, int fileStateSizeThreshold) {
		super(checkNotNull(checkpointDataUri, "checkpoint directory is null"), defaultSavepointDirectory);
	...
	// ------------------------------------------------------------------------
	//  State Stores and State Holders
	// ------------------------------------------------------------------------
	...
	public CheckpointStreamFactory createStreamFactory(JobID jobId, String operatorIdentifier) throws IOException {
		...
		final Path checkpointDirectoryPath = getCheckpointDirectoryForJob(jobId);
		final FileSystem fs = checkpointDirectoryPath.getFileSystem();

		return new FsCheckpointStreamFactory(fs, checkpointDirectoryPath, fileStateThreshold);
	}

	@Override
	public CheckpointStreamFactory createSavepointStreamFactory(
			String operatorIdentifier,
			String savepointDirectory) throws IOException {

		checkNotNull(savepointDirectory, "savepoint directory is null");

		final Path savepointDirectoryPath = new Path(savepointDirectory);
		final FileSystem fs = savepointDirectoryPath.getFileSystem();

		return new FsSavepointStreamFactory(fs, savepointDirectoryPath, fileStateThreshold);
	}
	...
	public StateDisposeHook createCheckpointDisposeHook(
		...
		final Path dirToDelete;

		// if we have an external pointer, we use that to get the directory to drop
		final String pointer = checkpoint.getExternalPointer();
		if (pointer != null) {
			final FileStatus metadataFile = resolveCheckpointPointer(pointer, true);
			dirToDelete = metadataFile.getPath().getParent();
		}
		else {
			// not externalized, find the directory based on paths and IDs
			// we can skip this once all checkpoints record a target location
			dirToDelete = createSpecificCheckpointPath(
					getCheckpointDirectoryForJob(checkpoint.getJobId()), checkpoint.getCheckpointID());
		}
		
		// delete the directory in all cases, also when the metadata is not externalized
	...
	public String toString() {
		return "File State Backend (checkpoints: " + getCheckpointPath() + 
