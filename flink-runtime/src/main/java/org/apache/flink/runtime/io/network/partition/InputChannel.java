...
public class InputChannel<T extends IOReadableWritable> extends Channel implements BufferProvider {
	 ...
	 *
	 * @param inputGate          the input gate this channel is connected to
	 * @param channelIndex       the index of the channel in the input gate
	 * @param channelID          the ID of the channel
	 * @param connectedChannelID the ID of the channel this channel is connected to
	 */
		...
		if (this.dataBuffer == null) {
			...
			if (boe.isEvent()) {
				...
				if (evt.getClass() == ChannelCloseEvent.class) {
					...
					this.currentEvent = (TaskEvent) evt;
				...
				this.deserializer.setNextMemorySegment(this.dataBuffer.getMemorySegment(), this.dataBuffer.getSize());
			}
		}
	...
	private void releasedConsumedReadBuffer(Buffer buffer) {
		this.amountOfDataTransmitted += buffer.getSize();
		buffer.recycle();
	}
	...
	public BufferFuture requestBuffer() {
		return this.inputGate.requestBuffer();
	}
