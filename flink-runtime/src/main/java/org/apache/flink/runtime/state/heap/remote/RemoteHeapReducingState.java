 ...
 */
class RemoteHeapReducingState<K, N, V>
	extends AbstractRemoteHeapMergingState<K, N, V, V, V>
	 ...
	 */
	private RemoteHeapReducingState(
	...
	public void add(V value) throws Exception {
		...
		V newValue =
			oldValue == null ? value : reduceTransformation.reduceFunction.reduce(oldValue, value);
	...
	static <K, N, SV, S extends State, IS extends S> IS create(
		...
		RemoteHeapKeyedStateBackend.RemoteHeapKvStateInfo kvState = backend.getRemoteHeapKvStateInfo(
			stateDesc.getName());
		return (IS) new RemoteHeapReducingState<>(
			keySerializer,
