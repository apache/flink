...
public class IntermediateResultPartition extends Gate<IOReadableWritable> {
	 ...
	 */
	public IntermediateResultPartition(JobID jobId, GateID gateId, int index) {
	...
	public void broadcastBuffer(Buffer buffer) throws IOException, InterruptedException {
		for (int i = 1; i < this.channels.length; i++) {
			channels[i].sendBuffer(buffer.retain());
		}
