...
public class TieredStorageMasterClient {
    ...
    public CompletableFuture<Collection<PartitionWithMetrics>> getPartitionWithMetrics(
        ...
        List<CompletableFuture<Map<ResultPartitionID, TieredPartitionWithMetrics>>> futures =
                new ArrayList<>(tiers.size());
        for (Tuple2<String, TierMasterAgent> tier : tiers) {
            CompletableFuture<Map<ResultPartitionID, TieredPartitionWithMetrics>>
                ...
                .thenApply(
                        allPartitions -> {
                            ...
                            expectedPartitions.forEach(
                                    partitionId -> {
                                        List<ResultPartitionBytes> partitionWithMetrics =
                                                new ArrayList<>();
                                        // TierShuffleDescriptor shuffleDescriptor = null;
                                        for (Map<ResultPartitionID, TieredPartitionWithMetrics>
                                                partitionMap : allPartitions) {
                                            TieredPartitionWithMetrics partitionWithMetric =
                                                    partitionMap.get(partitionId);
                                            if (partitionWithMetric == null) {
                                                break;
                                            }
                                        ...
                                        if (partitionWithMetrics.size() == partitionNums) {
