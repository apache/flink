...
import org.apache.flink.runtime.io.disk.iomanager.BufferFileReaderBase;
...
public class SpillableBlockingQueue implements IntermediateResultPartitionQueue {
	...
	private class SpillableQueueIterator implements IntermediateResultPartitionQueueIterator {
		...
		private final Optional<BufferProvider> bufferProvider;

		private BufferFileReaderBase reader;
		...
		private SpillableQueueIterator(Optional<BufferProvider> bufferProvider) {
					...
					if (reader == null) {
						try {
							if (bufferProvider.isPresent()) {
								reader = ioManager.createBufferFileReader(ioChannel, bufferProvider.get(), position);
							}
							else {
								reader = ioManager.createBufferFileReader(ioChannel, position);
							}
						}
						catch (IOException e) {
			...
			synchronized (queueMonitor) {
				...
				if (hasSpilled) {
					...
					if (boe.isBuffer()) {
						...
						position += SerializationUtil.toSerializedEvent(boe.getEvent()).remaining() + 5;
					}
