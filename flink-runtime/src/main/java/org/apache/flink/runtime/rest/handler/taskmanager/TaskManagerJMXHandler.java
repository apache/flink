...
/**
 * class for get jmx host and port for tm.
 */
public class TaskManagerJMXHandler extends AbstractTaskManagerHandler<RestfulGateway, EmptyRequestBody, JMXInfo, TaskManagerMessageParameters> {
	...
	public TaskManagerJMXHandler(
		CompletableFuture<String> localRestAddress,
		GatewayRetriever<? extends RestfulGateway> leaderRetriever,
		Time timeout,
		Map<String, String> responseHeaders,
		MessageHeaders<EmptyRequestBody, JMXInfo, TaskManagerMessageParameters> messageHeaders,
	...
	@Override
	protected CompletableFuture<JMXInfo> handleRequest(
		...
		final ResourceID resourceID = request.getPathParameter(TaskManagerIdPathParameter
			.class);
		return getJmxPortFuture(resourceID, gateway).thenApply(jmxTuple -> new JMXInfo(jmxTuple.f0, jmxTuple.f1));
	}

	private CompletableFuture<Tuple2<String, Long>> getJmxPortFuture(ResourceID resourceID, ResourceManagerGateway gateway) {
		log.debug("get jmx information for resourceId:[{}].", resourceID.getResourceIdString());
		return gateway.requestJmx(resourceID, timeout);
	}
