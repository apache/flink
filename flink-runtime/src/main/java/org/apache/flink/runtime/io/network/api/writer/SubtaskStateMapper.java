...
/**
 * The {@code SubtaskStateMapper} narrows down the subtasks that need to be read during rescaling to recover from a
 * particular subtask when in-flight data has been stored in the checkpoint.
 *
 ...
 *
 * </p>Most {@code SubtaskStateMapper} will only produce unique assignments and are thus optimal. Some rescaler, such
...
public enum SubtaskStateMapper {
	...
	/**
	 * Extra state is redistributed to other subtasks without any specific guarantee (only that up- and downstream
	 * are matched).
	 */
	ARBITRARY {
		@Override
	...
	/**
	 * Discards extra state. Useful if all subtasks already contain the same information (broadcast).
	 */
	DISCARD_EXTRA_STATE {
		@Override
		public Set<Integer> getOldSubtasks(int newSubtaskIndex, int oldNumberOfSubtasks, int newNumberOfSubtasks) {
	...
	/**
	 * Redistributes subtask state in a round robin fashion. Returns a mapping of {@code newIndex -> oldIndexes}. The
	...
	/**
	 * Returns all old subtask indexes that need to be read to restore all buffers for the given new subtask index on
	 * rescale.
	 */
	public abstract Set<Integer> getOldSubtasks(int newSubtaskIndex, int oldNumberOfSubtasks, int newNumberOfSubtasks);
	...
	public Map<Integer, Set<Integer>> getNewToOldSubtasksMapping(int oldParallelism, int newParallelism) {
		return IntStream.range(0, newParallelism).boxed().
			collect(Collectors.toMap(
				Function.identity(),
				channelIndex -> getOldSubtasks(channelIndex, oldParallelism, newParallelism)));
	}
}
