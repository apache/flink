...
public class IOManager implements UncaughtExceptionHandler {
	...
	public BufferFileReader createBufferFileReader(Channel.ID id, BufferProvider bufferProvider) throws IOException {
		...
		return new BufferFileReader(id, readers[id.getThreadNum()].requestQueue, 0, bufferProvider);
	}
	...
	public BufferFileReader createBufferFileReader(Channel.ID id, BufferProvider bufferProvider, long initialPosition) throws IOException {
		...
		return new BufferFileReader(id, readers[id.getThreadNum()].requestQueue, initialPosition, bufferProvider);
	}

	public BufferFileSegmentReader createBufferFileReader(Channel.ID id) throws IOException {
		return createBufferFileReader(id, 0);
	}

	public BufferFileSegmentReader createBufferFileReader(Channel.ID id, long initialPosition) throws IOException {
		checkState(!isClosed, "I/O manager is already closed.");
