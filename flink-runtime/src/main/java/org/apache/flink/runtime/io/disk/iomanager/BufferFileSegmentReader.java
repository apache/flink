...
public class BufferFileSegmentReader extends ChannelAccess<FileSegment, ReadRequest> {
	...
	private final LinkedBlockingQueue<FileSegment> fileSegmentReturnQueue = new LinkedBlockingQueue<FileSegment>();
	...
	@Override
	protected void returnBuffer(FileSegment fileSegment) {
		if (!isFinished.get() && fileSegment != null) {
			fileSegmentReturnQueue.add(fileSegment);
		}
	...
	public boolean hasNext() {
		return (!isFinished.get() || !fileSegmentReturnQueue.isEmpty()) && !isClosed.get();
	}

	public FileSegment getNextFileSegment() throws IOException {
		FileSegment fileSegment = fileSegmentReturnQueue.poll();
		if (fileSegment != null) {
			return fileSegment;
		}

		while (fileSegmentReturnQueue.size() < 32 && !isFinished.get() && !isClosed.get()) {
			addRequest(new FileSegmentReadRequest(this, isFinished));
		}

		return fileSegmentReturnQueue.poll();
	}
	...
	static class FileSegmentReadRequest implements ReadRequest {
		...
		private final ChannelAccess<FileSegment, ?> ioChannel;

		private final AtomicBoolean isFinished;

		private FileSegment fileSegment;

		protected FileSegmentReadRequest(ChannelAccess<FileSegment, ?> targetChannel, AtomicBoolean isFinished) {
		...
		public void read() throws IOException {
			...
			fileSegment = new FileSegment(fileChannel, fileSegmentPosition, length, type);
		}

		@Override
		public void requestDone(IOException e) {
			ioChannel.handleProcessedBuffer(fileSegment, e);
		}
	}
}