...
/**
 * This class contains the user-defined accumulators of a task.
 * Given that Flink handles differently accumulators that are bigger than the
 * {@link org.apache.flink.runtime.akka.AkkaUtils#getLargeAccumulatorThreshold(
		org.apache.flink.configuration.Configuration)}-byte threshold, this class can
 * accommodate both accumulator types, i.e. big and small.
 *
 * In fact, given that the accumulators of a task can either be small or oversized, this
 * class will contain either one or the other type, as it can be seen by its constructors.
 * */
