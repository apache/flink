...
public class ProxyAggregatingState<K, N, IN, ACC, OUT> implements InternalAggregatingState<K, N, IN, ACC, OUT> {
	...
	@Override
	public StateIncrementalVisitor<K, N, ACC> getStateIncrementalVisitor(int recommendedMaxNumberOfReturnedRecords) {
	...
	static <T, K, N, SV, S extends State, IS extends S> IS create(InternalKvState<K, N, SV> aggregatingState) {
		return (IS) new ProxyAggregatingState<>((InternalAggregatingState<K, N, T, SV, ?>) aggregatingState);
	}
}
