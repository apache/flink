...
class TransientBlobCleanupTask extends TimerTask {
	 ...
	 **/
	private ConcurrentMap<Tuple2<JobID, BlobKey>, Long> blobExpiryTimes;
	...
	TransientBlobCleanupTask(
			ConcurrentMap<Tuple2<JobID, BlobKey>, Long> blobExpiryTimes, Lock writeLock,
	...
	public void run() {
		...
		Set<Map.Entry<Tuple2<JobID, BlobKey>, Long>> entries = blobExpiryTimes.entrySet();
		for (Map.Entry<Tuple2<JobID, BlobKey>, Long> entry : entries) {
