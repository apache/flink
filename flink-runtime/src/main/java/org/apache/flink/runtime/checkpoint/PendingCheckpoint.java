...
public class PendingCheckpoint {
							...
							Map<ExecutionAttemptID, ExecutionVertex> verticesToConfirm)
	{
		...
		this.taskGroupStates = new HashMap<>();
	}
		...
		synchronized (lock) {
			...
			if (vertex != null) {
				if (state != null || kvState != null) {
					...
					if (taskGroupStates.containsKey(jobVertexID)) {
						taskGroupState = taskGroupStates.get(jobVertexID);
					} else {
						taskGroupState = new StateForTaskGroup(jobVertexID, vertex.getTotalNumberOfParallelSubtasks());
	...
	private void dispose(ClassLoader userClassLoader, boolean releaseState) {
		synchronized (lock) {
			...
			if (releaseState) {
				for (StateForTaskGroup taskGroupState : taskGroupStates.values()) {
					taskGroupState.discard(userClassLoader);
				}
			}
