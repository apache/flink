...
public class NettyConnectionRaceTrack {
	...
	private final ConcurrentMap<RemoteAddress, Object> channels = new ConcurrentHashMap<RemoteAddress, Object>();

	public NettyConnectionRaceTrack(NettyClient client) {
		this.client = client;
	}

	Channel connectOrGet(RemoteAddress target) throws IOException {
		...
		if (entry != null) {
			...
			if (old == null) {
				client.connect(target.getAddress()).addListener(connectingChannel);
