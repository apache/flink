        ...
        try {
            if (UserGroupInformation.isSecurityEnabled()
                ...
                if (fileLocation != null) {
                    // Use reflection API since the API semantics are not available in Hadoop1
                    // profile. Below APIs are
                    ...
                    try {
                        ...
                        Credentials cred =
                                (Credentials)
                                        readTokenStorageFileMethod.invoke(
                                                null, new File(fileLocation), hadoopConfiguration);

                        // if UGI uses Kerberos keytabs for login, do not load HDFS delegation token
                        // since
                        // the UGI would prefer the delegation token instead, which eventually
                        // expires
                        ...
                        Collection<Token<? extends TokenIdentifier>> usrTok =
                                (Collection<Token<? extends TokenIdentifier>>)
                                        getAllTokensMethod.invoke(cred);
                        ...
                        Method addCredentialsMethod =
                                UserGroupInformation.class.getMethod(
                                        "addCredentials", Credentials.class);
