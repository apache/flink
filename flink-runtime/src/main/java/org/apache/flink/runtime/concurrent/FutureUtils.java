...
public class FutureUtils {
                    ...
                    (t, throwable) -> {
                        if (throwable != null) {
                            if (throwable instanceof CancellationException) {
                                ...
                                if (retries > 0) {
                                    retryOperation(resultFuture, operation, retries - 1, executor);
                                } else {
                                    resultFuture.completeExceptionally(
                                            new RetryException(
    ...
    public static <T> CompletableFuture<T> retryWithDelay(
        ...
        retryOperationWithDelay(
                resultFuture, operation, retries, retryDelay, retryPredicate, scheduledExecutor);
    ...
    public static <T> CompletableFuture<T> retryWithDelay(
        ...
        return retryWithDelay(
                operation, retries, retryDelay, (throwable) -> true, scheduledExecutor);
    }
    ...
    public static <T> CompletableFuture<T> orTimeout(
        ...
        if (!future.isDone()) {
            final ScheduledFuture<?> timeoutFuture =
                    Delayer.delay(
                            () -> timeoutFailExecutor.execute(new Timeout(future)),
                            timeout,
                            timeUnit);
