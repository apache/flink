...
public class ExecutionVertex {
	...
	TaskDeploymentDescriptor createDeploymentDescriptor(ExecutionAttemptID executionId, AllocatedSlot slot) {
		...
		for (ExecutionEdge[] resultEdges : inputEdges) {
			IntermediateDataSetID resultId = resultEdges[0].getSource().getIntermediateResult().getId();
			IntermediateResultPartitionInfo[] iccd = IntermediateResultPartitionInfo.fromEdges(resultEdges, slot);

			rdd.add(new ResultPartitionReaderDeploymentDescriptor(resultId, iccd));
		}
		...
		// --------------------------------------------------------------------
		List<IntermediateResultPartitionDeploymentDescriptor> irpdd =
				new ArrayList<IntermediateResultPartitionDeploymentDescriptor>(resultPartitions.length);

		for (IntermediateResultPartition partition : resultPartitions) {
			irpdd.add(IntermediateResultPartitionDeploymentDescriptor.fromIntermediateResultPartition(partition));
		}
		...
		return new TaskDeploymentDescriptor(getJobId(), getJobvertexId(), executionId, getTaskName(),
				...
				jobVertex.getJobVertex().getInvokableClassName(), irpdd, inputGates, jarFiles, slot.getSlotNumber());
	}
