...
public class LocalRecoveryConfig {
	...
	public enum LocalRecoveryMode {
		...
		/**
		 * Attempts to parses the given string to a {@link LocalRecoveryMode}.
		 *
		 * @param input string to parse
		 * @return the {@link LocalRecoveryMode} that is represented by the input string.
		 * @throws ParseException if the string cannot be parsed to a {@link LocalRecoveryMode}.
		 */
		@Nonnull
		public static LocalRecoveryMode fromString(String input) throws ParseException {
			if (input != null) {
				String trimInput = input.trim();
				if (DISABLED.toString().equalsIgnoreCase(trimInput)) {
					return DISABLED;
				} else if (ENABLE_FILE_BASED.toString().equalsIgnoreCase(trimInput)) {
		...
		public static LocalRecoveryMode fromConfig(@Nonnull Configuration configuration) {
			...
			try {
				return LocalRecoveryConfig.LocalRecoveryMode.fromString(localRecoveryConfString);
