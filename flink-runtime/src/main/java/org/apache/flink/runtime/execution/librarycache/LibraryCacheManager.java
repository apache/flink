...
/**
 * The LibraryCacheManager is responsible for creating and managing the user code
 * class loaders.
 *
 * <p>In order to obtain a user code class loader, one first needs to obtain a
 * {@link ClassLoaderLease} for a given {@link JobID}. At first, the {@link ClassLoaderLease}
 * is unresolved. In order to obtain the user class loader one needs to resolve it
 * by specifying the required jar files and class paths. The user code class loader
 * for a job is valid as long as there exists a valid {@link ClassLoaderLease}. A
...
public interface LibraryCacheManager {
	...
	interface ClassLoaderHandle {
		 ...
		 * has to specify the required jars and class paths. Upon calling this
		 * method first for a job, it will make sure that the required jars are
		 * present and potentially cache the created user code class loader.
		 * Every subsequent call to this method, will ensure that created
		 ...
		 * @param requiredClasspaths requiredClasspaths the user code class loader needs to be started with
	...
	interface ClassLoaderLease extends ClassLoaderHandle {
		...
		/**
		 * Closes the lease to the user code class loader for the associated job.
		 *
		 * <p>This method signals that the lease holder not longer needs the user
		 * code class loader for the associated job. Once all leases for a job a
		 * closed, the library cache manager is allowed to release the associated
