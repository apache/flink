...
public class ConnectionUtils {
	...
	private static InetAddress tryLocalHostBeforeReturning(InetAddress preliminaryResult, SocketAddress targetAddress, boolean logging) throws IOException {
		...
		if(tryToConnect(localhostName, targetAddress, AddressDetectionState.LOCAL_HOST.getTimeout(), logging)) {
			// success, we were able to use local host to connect
			LOG.debug("The network address detection determined {} for the local host, but" +
					" InetAddress.getLocalHost() returned {} which allows us to connect to " +
					"the JobManager as well", preliminaryResult, localhostName);
