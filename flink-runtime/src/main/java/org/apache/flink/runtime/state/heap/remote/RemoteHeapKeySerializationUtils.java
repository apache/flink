...
public class RemoteHeapKeySerializationUtils {
	...
	public static boolean isAmbiguousKeyPossible(
		...
		return (isSerializerTypeVariableSized(keySerializer) && isSerializerTypeVariableSized(
			namespaceSerializer));
	}
	...
	public static void serializeKeyGroup(int keyGroup, byte[] startKeyGroupPrefixBytes) {
		...
		for (int j = 0; j < keyGroupPrefixBytes; ++j) {
			startKeyGroupPrefixBytes[j] = extractByteAtPosition(
				keyGroup,
				keyGroupPrefixBytes - j - 1);
		}
	}
