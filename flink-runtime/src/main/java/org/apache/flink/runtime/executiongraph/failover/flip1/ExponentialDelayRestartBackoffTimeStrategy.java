 ...
 * The delay starts at initial value and keeps increasing (multiplying by backoff multiplier) until maximum delay is reached.
 *
...
public class ExponentialDelayRestartBackoffTimeStrategy implements RestartBackoffTimeStrategy {
	...
	ExponentialDelayRestartBackoffTimeStrategy(
		...
		checkArgument(initialBackoffMS <= maxBackoffMS, "Initial backoff cannot be higher than maximum backoff.");
		...
		checkArgument(
			0 <= jitterFactor && jitterFactor <= 1,
			"Jitter factor must be >= 0 and <= 1.");
	...
	/**
	 * Calculate jitter offset to avoid thundering herd scenario.
	 * The offset range increases with the number of restarts.
	 * <p>
	 * F.e. for backoff time 8 with jitter 0.25, it generates random number in range [-2, 2].
	 *
	...
	public static ExponentialDelayRestartBackoffTimeStrategyFactory createFactory(final Configuration configuration) {
		long initialBackoffMS = configuration
			.get(RestartStrategyOptions.RESTART_STRATEGY_EXPONENTIAL_DELAY_INITIAL_BACKOFF)
			.toMillis();
		long maxBackoffMS = configuration
			.get(RestartStrategyOptions.RESTART_STRATEGY_EXPONENTIAL_DELAY_MAX_BACKOFF)
			.toMillis();
		double backoffMultiplier = configuration
			.get(RestartStrategyOptions.RESTART_STRATEGY_EXPONENTIAL_DELAY_BACKOFF_MULTIPLIER);
		long resetBackoffThresholdMS = configuration
			.get(RestartStrategyOptions.RESTART_STRATEGY_EXPONENTIAL_DELAY_RESET_BACKOFF_THRESHOLD)
			.toMillis();
		double jitterFactor = configuration
			.get(RestartStrategyOptions.RESTART_STRATEGY_EXPONENTIAL_DELAY_JITTER_FACTOR);
