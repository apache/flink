...
public class JobResult implements Serializable {
     ...
     *
     * <p><b>Only call this when the origin of this {@link JobResult} is trusted</b> (for example,
     * a same-process execution such as {@code MiniCluster} or Application Mode, where this
     * process itself produced the {@link SerializedThrowable}). {@link
     * SerializedThrowable#deserializeError} can run arbitrary code if the bytes it deserializes
     * did not originate from a trusted source. A {@link JobResult} obtained from a client that
     * talks to a remote JobManager (for example any {@code RestClusterClient}-backed {@code
     * JobClient}) should use {@link #toSafeJobExecutionResult} instead.
     *
    ...
    public JobExecutionResult toJobExecutionResult(ClassLoader classLoader)
        ...
        return toJobExecutionResult(
                classLoader,
                serializedThrowable == null ? null : serializedThrowable.deserializeError(classLoader));
    }

    /**
     * Converts the {@link JobResult} to a {@link JobExecutionResult} without deserializing the
     * failure cause: the thrown exception's cause is the {@link SerializedThrowable} itself,
     * never the original, live exception object.
     *
     * <p>The robust choice when the origin of this {@link JobResult} isn't known to be
     * trustworthy - for example, one obtained from a {@code RestClusterClient}-backed {@code
     * JobClient} talking to a remote JobManager. {@code classLoader} is only used to deserialize
     * accumulators. A caller that needs the original exception object back must call {@link
    ...
    private JobExecutionResult toJobExecutionResult(ClassLoader classLoader, @Nullable Throwable cause)
            throws JobExecutionException, IOException, ClassNotFoundException {
