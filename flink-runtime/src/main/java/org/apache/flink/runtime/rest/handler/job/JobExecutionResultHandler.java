...
public class JobExecutionResultHandler
	...
	protected CompletableFuture<JobExecutionResultResponseBody> handleRequest(
		...
		return gateway.isJobExecutionResultPresent(jobId, timeout).thenCompose(present -> {
				if (!present) {
					...
					return gateway.getJobExecutionResult(jobId, timeout).thenApply((jobExecutionResult) -> {
						if (jobExecutionResult.isLeft()) {
							return JobExecutionResultResponseBody.created(
								JobExecutionResult.failure(jobId, jobExecutionResult.left()));
						} else {
							return JobExecutionResultResponseBody.created(
								JobExecutionResult.success(jobExecutionResult.right()));
						}
					});
				}
			}
	...
	private static CompletionException propagateException(Throwable throwable) {
		...
		if (throwable instanceof JobExecutionResultNotFoundException
			|| throwable instanceof FlinkJobNotFoundException) {
