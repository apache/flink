...
/**
 * Drop-in replacement for {@link org.apache.flink.runtime.iterative.task.IterationHeadPactTask} used for Stale Synchronous
 * Parallel iterations.
 * The head is responsible for coordinating an iteration and can run a {@link org.apache.flink.runtime.operators.PactDriver} inside. It will read
 ...
 * synchronization task and after each superstep, it will advance its task until its clock is farther than the cluster-
 * wide clock + the defined slack. It then waits for the cluster-wide clock to catch up before going on {@link org.apache.flink.runtime.iterative.event.ClockTaskEvent}.
 * Starting with the second iteration, the input for the head is the output of the tail, transmitted through the backchannel. Once the
 * iteration is done, the head will send a {@link org.apache.flink.runtime.iterative.event.TerminationEvent} to all it's connected tasks, signaling them to shutdown.
 * <p/>
