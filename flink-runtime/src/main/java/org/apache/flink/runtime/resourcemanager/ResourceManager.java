...
public abstract class ResourceManager<WorkerType extends Serializable>
		...
		if (isValid(resourceManagerLeaderId)) {
			if (!jobLeaderIdService.containsJob(jobId)) {
				try {
					...
					// This should actually never happen because, it should always be possible to add a new job
	...
	/**
	 * Checks whether the given resource manager leader id is matching the current leader id.
	 *
	...
	protected void removeJob(JobID jobId) {
		try {
			...
			onFatalError(new ResourceManagerException("Could not remove job " + jobId + '.', e));
		}
