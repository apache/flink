...
import org.apache.flink.runtime.io.network.partition.BoundedBlockingSubpartitionMemory.Reader;
...
/**
 * Tests for the {@link BoundedBlockingSubpartitionMemory}.
 */
public class BoundedBlockingSubpartitionMemoryTest {

	@Test
	public void testCompleteIsSameBufferAsOriginal() {
		final ByteBuffer bb = ByteBuffer.allocateDirect(128);
		final BoundedBlockingSubpartitionMemory.Writer writer = new BoundedBlockingSubpartitionMemory.Writer(bb);
	...
	private void testWriteAndReadMultipleBuffers(ByteBuffer buffer, int numIntsPerBuffer) {
		...
		int numBuffers = 0;
		while (writer.writeBuffer(BufferBuilderTestUtils.buildBufferWithAscendingInts(1024, numIntsPerBuffer))) {
			numBuffers++;
		}
