...
public class BackPressureRequestCoordinatorTest extends TestLogger {
    ...
    public void testBackPressureRequestTimeout() throws Exception {
        ExecutionVertex[] vertices =
                createExecutionVertices(ExecutionState.RUNNING, CompletionType.TIMEOUT);
    ...
    private static TaskManagerGateway createTaskManagerGateway(
        ...
        final CompletableFuture<TaskBackPressureResponse> responseFuture =
                new CompletableFuture<>();
        switch (completionType) {
            case SUCCESSFULLY:
                responseFuture.complete(
                        new TaskBackPressureResponse(0, executionAttemptId, backPressureRatio));
                break;
            case EXCEPTIONALLY:
    ...
    private static class MockBackPressureRequestTaskManagerGateway
            extends SimpleAckingTaskManagerGateway {
        private final CompletableFuture<TaskBackPressureResponse> responseFuture;

        private MockBackPressureRequestTaskManagerGateway(
                CompletableFuture<TaskBackPressureResponse> responseFuture) {
