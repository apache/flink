...
/**
 * Tests for the {@link SchedulerImpl} and {@link SlotPoolImpl} which verify the spread out of
 * slots.
 */
public class SlotPoolSlotSpreadOutTest extends TestLogger {
    ...
    public final SlotPoolResource slotPoolResource =
            new SlotPoolResource(LocationPreferenceSlotSelectionStrategy.createEvenlySpreadOut());
    ...
    public void allocateSingleSlot_withNoRequirements_selectsSlotSoThatWorkloadIsSpreadOut() {
        ...
        final TaskManagerLocation firstTaskManagerLocation =
                getTaskManagerLocation(firstSlotFuture);
        final TaskManagerLocation secondTaskManagerLocation =
                getTaskManagerLocation(secondSlotFuture);
    ...
    public void allocateSingleSlot_withInputPreference_inputPreferenceHasPrecedenceOverSpreadOut() {
        ...
        final TaskManagerLocation sourceTaskManagerLocation =
                getTaskManagerLocation(sourceSlotFuture);

        Collection<TaskManagerLocation> preferredLocations =
                Collections.singleton(sourceTaskManagerLocation);
        final CompletableFuture<LogicalSlot> sinkSlotFuture =
                allocateSlotWithInputPreference(sinkSlotRequest, preferredLocations);
    ...
    public void allocateSharedSlot_withNoRequirements_selectsSlotsSoThatWorkloadIsSpreadOut() {
        ...
        final List<ScheduledUnit> sourceScheduledUnits =
                IntStream.range(0, numberSlots)
                        .mapToObj(
                                ignored ->
                                        createSharedSlotRequest(
                                                sourceJobVertexId, slotSharingGroupId))
                        .collect(Collectors.toList());

        final List<ScheduledUnit> sinkScheduledUnits =
                IntStream.range(0, numberTaskExecutors)
                        .mapToObj(
                                ignored ->
                                        createSharedSlotRequest(
                                                sinkJobVertexId, slotSharingGroupId))
                        .collect(Collectors.toList());

        sourceScheduledUnits.forEach(this::allocateSlot);
        final Set<TaskManagerLocation> sinkLocations =
                sinkScheduledUnits.stream()
    ...
    private ScheduledUnit createSharedSlotRequest(
            JobVertexID jobVertexId, SlotSharingGroupId slotSharingGroupId) {
    ...
    private CompletableFuture<LogicalSlot> internalAllocateSlot(
            ScheduledUnit scheduledUnit, SlotProfile slotProfile) {
    ...
    private CompletableFuture<LogicalSlot> allocateSlotWithInputPreference(
            ScheduledUnit scheduledUnit, Collection<TaskManagerLocation> preferredLocations) {
        return internalAllocateSlot(
                scheduledUnit,
                SlotProfile.preferredLocality(ResourceProfile.UNKNOWN, preferredLocations));
    }

    private TaskManagerLocation getTaskManagerLocation(
            CompletableFuture<? extends LogicalSlot> slotFuture) {
    ...
    private void registerTaskExecutor(int numberSlotsPerTaskExecutor) {
        ...
        final Collection<SlotOffer> slotOffers =
                IntStream.range(0, numberSlotsPerTaskExecutor)
                        .mapToObj(
                                index ->
                                        new SlotOffer(
