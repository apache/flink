...
/**
 * {@code AbstractApplicationRunITCase} runs a single application in HA mode and provides {@code
 * abstract} methods for initializing a specific {@link FileSystem}.
 *
 * <p>Sub-classes must use a {@link
 * org.apache.flink.runtime.testutils.InternalMiniClusterExtension}.
 */
@ExtendWith(TestLoggerExtension.class)
public abstract class AbstractHAApplicationRunITCase {
    ...
    @Test
    void testApplicationExecutionInHaMode(@InjectMiniCluster MiniCluster flinkCluster)
            throws Exception {
        final JobGraph jobGraph = JobGraphTestUtils.singleNoOpJobGraph();
        final SingleJobApplication application = new SingleJobApplication(jobGraph);
        ...
        flinkCluster.submitApplication(application).get(30, TimeUnit.SECONDS);

        final Deadline deadline = Deadline.fromNow(Duration.ofSeconds(30));
        final ApplicationState applicationStatus =
                FutureUtils.retrySuccessfulWithDelay(
                                () -> flinkCluster.getApplicationStatus(applicationId),
                                Duration.ofMillis(10),
                                deadline,
                                status ->
                                        flinkCluster.isRunning()
                                                && status == ApplicationState.FINISHED,
                                new ScheduledExecutorServiceAdapter(
                                        EXECUTOR_RESOURCE.getExecutor()))
                        .get(deadline.timeLeft().toMillis(), TimeUnit.MILLISECONDS);

        assertThat(applicationStatus).isEqualTo(ApplicationState.FINISHED);

        runAfterApplicationTermination();
    }
}
