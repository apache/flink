...
public class TaskManagerCheckInSlotManagerTest extends TestLogger {
    ...
    /**
     * If there is no job running, no need to keep redundant taskManagers and release the timeout
     * taskManagers. This may happen in session mode without jobs running.
     */
    @Test
    ...
    /**
     * Register four taskManagers that all have two slots. For taskManager0, both slots are free.
     * For taskManager1, both slots are allocated. For taskManager2, One slot is allocated, the
     * other is free. For taskManager3, one slot is free, the other is allocated. If
     * redundantTaskManagerNum is 0, the idle taskManager should be released.
     *
     * @throws Exception
     */
    @Test
     ...
     * For taskManager1, both slots are allocated. For taskManager2, One slot is allocated, the
     * other is free. For taskManager3, one slot is free, the other is allocated. If
     * redundantTaskManagerNum is 1, two free slots are needed and the idle taskManager should be
     * released.
     *
     * @throws Exception
     */
     ...
     * For taskManager1, both slots are allocated. For taskManager2, One slot is allocated, the
     * other is free. For taskManager3, one slot is free, the other is allocated. If
     * redundantTaskManagerNum is 2, four free slots can satisfy the requirement.
     *
     * @throws Exception
     */
    @Test
     ...
     * For taskManager1, both slots are allocated. For taskManager2, One slot is allocated, the
     * other is free. For taskManager3, one slot is free, the other is allocated. If
     * redundantTaskManagerNum is 3, two more free slots are needed and another taskManager should
     * be allocated.
     *
     * @throws Exception
     */
    ...
    /**
     * Register four taskManagers that all have two slots. The difference between the taskManagers
     * is whether the slot is allocated. To maintain redundantTaskManagerNum, SlotManagerImpl may
     * release or allocate taskManagers.
     *
    ...
    private void registerTaskManagerWithTwoSlots(
            SlotManagerImpl slotManager, boolean slot0Free, boolean slot1Free) {
        canBeReleasedFuture.set(new CompletableFuture<>());
        ...
        SlotStatus slotStatus0 =
                slot0Free
                        ? new SlotStatus(slotId0, resourceProfile)
        ...
        SlotStatus slotStatus1 =
                slot1Free
                        ? new SlotStatus(slotId1, resourceProfile)
    ...
    private SlotManagerImpl createAndStartSlotManager(
            int redundantTaskManagerNum, int numSlotsPerWorker) {
    ...
    private void checkTaskManagerTimeoutWithCustomCanBeReleasedResponse(
        ...
        canBeReleasedFuture.set(new CompletableFuture<>());
        mainThreadExecutor.execute(
                slotManager::checkTaskManagerTimeoutsAndRedundancy); // trigger TM.canBeReleased
        // request
        mainThreadExecutor.triggerAll();
