...
/**
 * Tests for basic {@link SubmittedJobGraphs} contract.
 */
public class ZooKeeperSubmittedJobGraphsITCase extends TestLogger {
	...
	public void testPutAndRemoveJobGraph() throws Exception {
		ZooKeeperSubmittedJobGraphs jobGraphs = new ZooKeeperSubmittedJobGraphs(
	...
	public void testConcurrentAddJobGraph() throws Exception {
		ZooKeeperSubmittedJobGraphs jobGraphs = null;
		ZooKeeperSubmittedJobGraphs otherJobGraphs = null;

		try {
			jobGraphs = new ZooKeeperSubmittedJobGraphs(
					ZooKeeper.createClient(), "/testConcurrentAddJobGraph", StateHandleProvider);

			otherJobGraphs = new ZooKeeperSubmittedJobGraphs(
					ZooKeeper.createClient(), "/testConcurrentAddJobGraph", StateHandleProvider);
	...
	@Ignore
	@Test
	// Ignored, because this is very instable on Travis. The callbacks often don't happen.
	public void testConcurrentRemoveJobGraph() throws Exception {
		ZooKeeperSubmittedJobGraphs jobGraphs = new ZooKeeperSubmittedJobGraphs(
				ZooKeeper.createClient(), "/testConcurrentRemoveJobGraph", StateHandleProvider);

		try {
			SubmittedJobGraphListener listener = mock(SubmittedJobGraphListener.class);

			final AtomicReference<JobID> callbackJobId = new AtomicReference<>();
			final CountDownLatch removeCallbackLatch = new CountDownLatch(1);

			doAnswer(new Answer<Void>() {
				@Override
				public Void answer(InvocationOnMock invocation) throws Throwable {
					callbackJobId.compareAndSet(null, (JobID) invocation.getArguments()[0]);

					removeCallbackLatch.countDown();

					return null;
				}
			}).when(listener).onRemovedJobGraph(any(JobID.class));
			...
			// Wait for the job graph to be available
			// Everything is cool... not much happening ;)
			verify(listener, never()).onAddedJobGraph(any(JobID.class));
			verify(listener, never()).onRemovedJobGraph(any(JobID.class));

			// This looks unnecessary, but the ZooKeeper watch does not fire if creating and
			// deleting happens too fast w/o a get operation.
			SubmittedJobGraph recovered = jobGraphs.recoverJobGraph(jobGraph.getJobId());
			verifyJobGraphs(jobGraph, recovered);

			// Delete it
			ZooKeeper.getClient().delete().forPath("/testConcurrentRemoveJobGraph/" +
					jobGraph.getJobId());

			removeCallbackLatch.await();

			verify(listener, never()).onAddedJobGraph(any(JobID.class));
			verify(listener, atMost(1)).onRemovedJobGraph(any(JobID.class));

			assertEquals(jobGraph.getJobId(), callbackJobId.get());
		}
		finally {
			jobGraphs.stop();
		}
	...
	public void testUpdateJobGraphYouDidNotGetOrAdd() throws Exception {
		...
		ZooKeeperSubmittedJobGraphs otherJobGraphs = new ZooKeeperSubmittedJobGraphs(
				ZooKeeper.createClient(), "/testUpdateJobGraphYouDidNotGetOrAdd", StateHandleProvider);
