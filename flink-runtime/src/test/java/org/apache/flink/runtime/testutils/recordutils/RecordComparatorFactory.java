...
public class RecordComparatorFactory implements TypeComparatorFactory<Record> {
    ...
    public void writeParametersToConfig(Configuration config) {
        ...
        // write the config
        config.set(getIntegerConfigOption(NUM_KEYS), this.positions.length);
        for (int i = 0; i < this.positions.length; i++) {
            config.set(getIntegerConfigOption(KEY_POS_PREFIX + i), this.positions[i]);
    ...
    public void readParametersFromConfig(Configuration config, ClassLoader cl)
        ...
        final int numKeyFields = config.get(getIntegerConfigOption(NUM_KEYS), -1);
        if (numKeyFields < 0) {
        ...
        for (int i = 0; i < numKeyFields; i++) {
            // next key position
            final int p = config.get(getIntegerConfigOption(KEY_POS_PREFIX + i), -1);
            if (p >= 0) {
                positions[i] = p;
            } else {
                throw new IllegalConfigurationException(
