...
public class SavepointCoordinatorTest extends TestLogger {
	...
	public void testRollbackSetsCheckpointID() throws Exception {
		...
		when(savepoint.getTaskGroupStates()).thenReturn(Collections.<JobVertexID, StateForTaskGroup>emptyMap());
	...
	private static void verifyPendingCheckpoint(
		...
		for (StateForTaskGroup stateForTaskGroup : checkpoint.getTaskGroupStates().values()) {
			actualNumberOfCollectedStates += stateForTaskGroup.getNumberCollectedStates();
		}
	...
	private static void verifyCompletedCheckpoint(
		...
		for (ExecutionVertex vertex : expectedVertices) {
			...
			StateForTaskGroup stateForTaskGroup = checkpoint.getTaskGroupState(jobVertexID);

			assertNotNull(stateForTaskGroup);

			StateForTask stateForTask = stateForTaskGroup.getState(vertex.getParallelSubtaskIndex());

			assertNotNull(stateForTask);

			ExecutionAttemptID vertexAttemptId = vertex.getCurrentExecutionAttempt().getAttemptId();

			ExecutionAttemptID stateAttemptId = (ExecutionAttemptID) stateForTask.getState()
