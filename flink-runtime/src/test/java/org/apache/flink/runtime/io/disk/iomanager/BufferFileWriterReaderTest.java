...
public class BufferFileWriterReaderTest {
	...
	public void testWriteAndDelete() {
		try {
			...
			final BufferPool bufferPool = buffers.createBufferPool(512, true);
	...
	public void testWriteReadBufferEventSequence() {
		try {
			...
			final BufferPool outputBufferPool = buffers.createBufferPool(512, true);
			final BufferPool inputBufferPool = buffers.createBufferPool(128, true);
	...
	public void testWriteBufferEventSequenceReadFileSegment() {
		try {
			...
			final BufferFileWriter writer = ioManager.createBufferFileWriter(ioChannel);
			final BufferFileSegmentReader reader = ioManager.createBufferFileReader(ioChannel);
			...
			int serializedEventLength = SerializationUtil.toSerializedEvent(new TestEvent(0L)).remaining();

			while (reader.hasNext()) {
				BufferOrEvent bufferOrEvent = reader.getNextBufferOrEvent();

				if (bufferOrEvent != null) {
					assertTrue(bufferOrEvent.isFileSegment());
					FileSegment fileSegment = bufferOrEvent.getFileSegment();

					if (random.nextDouble() <= 0.9) {
						assertTrue(bufferOrEvent.isBuffer());

						assertEquals(random.nextInt(bufferSize), fileSegment.getLength());
					}
					else {
						assertTrue(bufferOrEvent.isEvent());
