...
public class SharedStateCollectorTest {
    ...
    public void testNoSharing() throws Exception {
        ...
        assertSharedStateEquals(emptyMap(), vertex);
    }
    ...
    public void testSimpleSharing() throws Exception {
        ...
        assertSharedStateEquals(singletonMap(stateId, asList(vertex.getTaskVertices())), vertex);
    }
    ...
    public void testCrossVertexSharing() throws Exception {
        ...
        assertSharedStateEquals(
                singletonMap(
                        stateId,
                        asList(vertex1.getTaskVertices()[0], vertex2.getTaskVertices()[0])),
                vertex1,
                vertex2);
    }
    ...
    private void assertSharedStateEquals(
            Map<String, List<ExecutionVertex>> expected, ExecutionJobVertex... vertex) {
        assertEquals(
                expected.entrySet().stream()
                        .collect(
                                Collectors.toMap(
                                        e -> StateObjectID.of(e.getKey()), Map.Entry::getValue)),
