...
public class TaskManagerLogListHandlerTest extends TestLogger {
	...
	public void testGetTaskManagerLogsList() throws Exception {
		...
		assertEquals(logListInfo.getLogInfos().size(), resourceManagerGateway.requestTaskManagerLogList(EXPECTED_TASK_MANAGER_ID, TestingUtils.TIMEOUT()).get().size());
	}
	...
	public void testGetTaskManagerLogsListForUnknownTaskExecutorException() {
		...
		try {
			...
			HttpResponseStatus httpResponseStatus = restHandlerException.getHttpResponseStatus();
			assertThat(httpResponseStatus, is(equalTo(HttpResponseStatus.NOT_FOUND)));
