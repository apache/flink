...
public class SpanningRecordSerializationTest extends TestLogger {
    ...
    /**
     * Iterates over the provided records and tests whether {@link SpanningRecordSerializer} and
    ...
    private static void testSerializationRoundTrip(
        ...
        for (SerializationTestType record : records) {
            ...
            if (serializer
                    .copyToBufferBuilder(serializationResult.getBufferBuilder())
                    .isFullBuffer()) {
                ...
                while ((serializationResult = setNextBufferForSerializer(serializer, segmentSize))
                        .isFullBuffer()) {
                    deserializer.setNextBuffer(serializationResult.buildBuffer());
                }
            }
    ...
    public void testSpanningRecordUnconsumedBuffer() throws Exception {
        ...
        testUnconsumedBuffer(
                serializer, deserializer, Util.randomRecord(SerializationTestTypeFactory.INT), 1);
    }
    ...
    private static BufferAndSerializerResult setNextBufferForSerializer(
            RecordSerializer<SerializationTestType> serializer, int segmentSize)
            throws IOException {
