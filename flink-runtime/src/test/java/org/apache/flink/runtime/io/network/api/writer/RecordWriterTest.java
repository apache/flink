...
public class RecordWriterTest {
	...
	public void testBroadcastEventNoRecords() throws Exception {
		...
		for (int i = 0; i < numberOfChannels; i++) {
			...
			BufferOrEvent boe = parseData(view.getNextData(), i);
	...
	public void testBroadcastEventMixedRecords() throws Exception {
		...
		if (isBroadcastWriter) {
			...
			for (int i = 0; i < numberOfChannels; i++) {
				...
				ResultSubpartitionView view = partition.createSubpartitionView(i, new NoOpBufferAvailablityListener());
				for (int j = 0; j < 3; j++) {
					assertTrue(parseData(view.getNextData(), 0).isBuffer());
				}
			...
			assertTrue(parseData(views[0].getNextData(), 0).isBuffer());
			...
			assertTrue(parseData(views[1].getNextData(), 1).isBuffer());
			assertTrue(parseData(views[1].getNextData(), 1).isBuffer());
			...
			assertTrue(parseData(views[2].getNextData(), 2).isBuffer());
			...
			for (int i = 0; i < numberOfChannels; i++) {
				BufferOrEvent boe = parseData(views[i].getNextData(), i);
				assertTrue(boe.isEvent());
				assertEquals(barrier, boe.getEvent());
			}
		}
	...
	public void testBroadcastEventBufferReferenceCounting() throws Exception {
		...
		PartitionData[] data = new PartitionData[numSubpartitions];

		// process all collected events (recycles the buffer)
		for (int i = 0; i < numSubpartitions; i++) {
			assertEquals(1, partition.getNumberOfQueuedBuffers(i));
			ResultSubpartitionView view = partition.createSubpartitionView(i, new NoOpBufferAvailablityListener());
			data[i] = view.getNextData();
			assertTrue(parseData(data[i], i).isEvent());
		}
	}
	...
	static BufferOrEvent parseData(PartitionData data, int targetChannel) throws IOException {
	...
	public static Buffer getBuffer(PartitionData data) throws IOException {
		assertNotNull(data);
		return data.buffer();
	}
