...
public class FsCheckpointMetadataOutputStreamTest {
	...
	public void testEmptyState() throws Exception {
		...
		try (FsCheckpointMetadataOutputStream stream = new FsCheckpointMetadataOutputStream(fs, path)) {
			handle = stream.closeAndGetPointerHandle();
		}

		// must have created a handle
		assertNotNull(handle);
		assertNotNull(handle.pointer());
		assertNotNull(handle.stateHandle());

		// the pointer path should exist as a directory
		assertTrue(fs.exists(new Path(handle.pointer())));
		assertFalse(fs.getFileStatus(new Path(handle.pointer())).isDir());

		// the contents should be empty
		try (FSDataInputStream in = handle.stateHandle().openInputStream()) {
	...
	public void testWriteAndRead() throws Exception {
		...
		try (FSDataInputStream in = fs.open(new Path(handle.pointer()))) {
			byte[] buffer = new byte[data.length];
			readFully(in, buffer);
			assertArrayEquals(data, buffer);
		}
	...
	public void testCleanupWhenFailingCloseAndGetHandle() throws IOException {
		...
		FsCheckpointMetadataOutputStream stream = new FsCheckpointMetadataOutputStream(fs, path);
		verify(fs).create(any(Path.class), any(WriteMode.class));

		stream.write(new byte[] {1,2,3,4,5});

		try {
			stream.closeAndGetPointerHandle();
	...
	public void testCloseDoesNotLock() throws Exception {
		...
		final FsCheckpointMetadataOutputStream checkpointStream = 
				new FsCheckpointMetadataOutputStream(fileSystem, path);
		...
		final CheckedThread thread = new CheckedThread() {
			...
			public void go() throws Exception {
				...
				checkpointStream.closeAndGetPointerHandle();
			}
		};
		thread.start();
