...
/**
 * Tests for the scheduling of the execution graph. This tests that for example the order of
 * deployments is correct and that bulk slot allocation works properly.
 */
public class ExecutionGraphSchedulingTest extends TestLogger {
    ...
    /**
     * Tests that with scheduling futures and pipelined deployment, the target vertex will not
     * deploy its task before the source vertex does.
     */
    @Test
    public void testScheduleSourceBeforeTarget() throws Exception {
        ...
        targetVertex.connectNewDataSetAsInput(
                sourceVertex, DistributionPattern.ALL_TO_ALL, ResultPartitionType.PIPELINED);
    ...
    private TestingLogicalSlot createTestingLogicalSlot(
            InteractionsCountingTaskManagerGateway gatewaySource) {
    ...
    /**
     * This test verifies that before deploying a pipelined connected component, the full set of
     * slots is available, and that not some tasks are deployed, and later the system realizes that
    ...
    public void testDeployPipelinedConnectedComponentsTogether() throws Exception {
        ...
        final InteractionsCountingTaskManagerGateway[] sourceTaskManagers =
                new InteractionsCountingTaskManagerGateway[parallelism];
        final InteractionsCountingTaskManagerGateway[] targetTaskManagers =
                new InteractionsCountingTaskManagerGateway[parallelism];
    ...
    /**
     * This tests makes sure that with eager scheduling no task is deployed if a single slot
     * allocation fails. Moreover we check that allocated slots will be returned.
     */
    @Test
    ...
    /**
     * Tests that an ongoing scheduling operation does not fail the {@link ExecutionGraph} if it
     * gets concurrently cancelled.
     */
    @Test
    public void testSchedulingOperationCancellationWhenCancel() throws Exception {
        ...
        slotProvider.addSlots(
                jobVertex.getID(), new CompletableFuture[] {slotFuture1, slotFuture2});
    ...
    /**
     * Tests that a partially completed eager scheduling operation fails if a completed slot is
     * released. See FLINK-9099.
     */
    @Test
    ...
    /**
     * Tests that all slots are being returned to the {@link SlotOwner} if the {@link
     * ExecutionGraph} is being cancelled. See FLINK-9908
     */
    @Test
    public void testCancellationOfIncompleteScheduling() throws Exception {
        ...
        final ConcurrentMap<SlotRequestId, Integer> slotRequestIds =
                        ...
                        (SlotRequestId slotRequestId) -> {
                            ...
                            return slotRequestIds.size() % 2 == 0
                                    ? CompletableFuture.completedFuture(
                                            createSingleLogicalSlot(
                                                    slotOwner, taskManagerGateway, slotRequestId))
        ...
        taskManagerGateway.setCancelConsumer(
                (ExecutionAttemptID executionAttemptId) -> {
                    ...
                    // if the execution was cancelled in state SCHEDULING, then it might already
                    // have been removed
                    if (execution != null) {
    ...
    private ExecutionGraph createExecutionGraph(JobGraph jobGraph, SlotProvider slotProvider)
            throws Exception {
        return createExecutionGraph(jobGraph, slotProvider, Time.minutes(10));
    }

    private ExecutionGraph createExecutionGraph(
            JobGraph jobGraph, SlotProvider slotProvider, Time timeout) throws Exception {
    ...
    private static void verifyNothingDeployed(
            ExecutionGraph eg, InteractionsCountingTaskManagerGateway[] taskManagers) {
