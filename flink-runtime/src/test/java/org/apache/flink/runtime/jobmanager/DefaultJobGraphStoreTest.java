...
/**
 * Tests for {@link DefaultJobGraphStore} with {@link TestingJobGraphStoreWatcher}, {@link TestingStateHandleStore},
...
public class DefaultJobGraphStoreTest extends TestLogger {
	...
	public void testRecoverJobGraph() throws Exception {
		...
		final TestingStateHandleStore<JobGraph> stateHandleStore = builder
	...
	public void testRecoverJobGraphFailedShouldReleaseHandle() throws Exception {
		...
		try {
			...
			fail("recoverJobGraph should fail when there is exception in getting the state handle.");
		} catch (Exception ex) {
	...
	public void testPutJobGraphWhenNotExist() throws Exception {
		...
		final TestingStateHandleStore<JobGraph> stateHandleStore = builder
			.setExistsFunction(ignore -> IntegerResourceVersion.notExisting())
	...
	public void testRemoveJobGraphWithNonExistName() throws Exception {
		...
		try {
			...
			fail("We should get an expected timeout because we are removing a non-existed job graph.");
	...
	public void testOnRemovedJobGraphShouldOnlyProcessKnownJobGraphs() throws Exception {
		...
		assertThat(testingJobGraphListener.getRemovedJobGraphs(), contains(testingJobGraph.getJobID()));
	}
	...
	public void testStoppingJobGraphStoreShouldReleaseAllHandles() throws Exception {
		...
		final TestingStateHandleStore<JobGraph> stateHandleStore = builder
			.setReleaseAllHandlesRunnable(() -> completableFuture.complete(null))
			.build();
		final JobGraphStore jobGraphStore = createAndStartJobGraphStore(stateHandleStore);
		jobGraphStore.stop();
	...
	private JobGraphStore createAndStartJobGraphStore(TestingStateHandleStore<JobGraph> stateHandleStore) throws Exception {
		final JobGraphStore jobGraphStore = new DefaultJobGraphStore<>(
			stateHandleStore,
