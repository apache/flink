...
public class SpanningRecordSerializerTest {
	...
	public void testHasData() {
		...
		final Buffer buffer = new Buffer(new MemorySegment(new byte[SEGMENT_SIZE]), SEGMENT_SIZE, Mockito.mock(BufferRecycler.class));
	...
	private void test(Util.MockRecords records, int segmentSize) throws Exception {
		...
		final SpanningRecordSerializer<SerializationTestType> serializer = new SpanningRecordSerializer<SerializationTestType>();
		final Buffer buffer = new Buffer(new MemorySegment(new byte[segmentSize]), segmentSize, Mockito.mock(BufferRecycler.class));
