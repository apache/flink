...
class DefaultVertexParallelismAndInputInfosDeciderTest {
    ...
    void testAllEdgesAllToAllAndOneIsBroadcast(boolean consumeAllSubpartitions) {
        AllToAllBlockingResultInfo resultInfo1 =
                createAllToAllBlockingResultInfo(
                        new long[] {10L, 15L, 13L, 12L, 1L, 10L, 8L, 20L, 12L, 17L});
        AllToAllBlockingResultInfo resultInfo2;
        if (consumeAllSubpartitions) {
            // create three subpartitions
            resultInfo2 = createAllToAllBlockingResultInfo(new long[] {0L, 5L, 10L}, true, true);
        } else {
        ...
        if (consumeAllSubpartitions) {
            checkAllToAllJobVertexInputInfo(
                    ...
                    Arrays.asList(
                            new IndexRange(0, 2), new IndexRange(0, 2), new IndexRange(0, 2)));
        } else {
            checkAllToAllJobVertexInputInfo(
                    parallelismAndInputInfos
                            .getJobVertexInputInfos()
    ...
    @ParameterizedTest
    @ValueSource(booleans = {true, false})
    void testAllEdgesBroadcast(boolean consumeAllSubpartitions) {
        AllToAllBlockingResultInfo resultInfo1;
        AllToAllBlockingResultInfo resultInfo2;
        if (consumeAllSubpartitions) {
            // create three subpartitions
            resultInfo1 = createAllToAllBlockingResultInfo(new long[] {0L, 5L, 10L}, true, true);
            resultInfo2 = createAllToAllBlockingResultInfo(new long[] {0L, 5L, 10L}, true, true);
        } else {
        ...
        List<IndexRange> expectedSubpartitionRanges =
                consumeAllSubpartitions
                        ? Collections.singletonList(new IndexRange(0, 2))
