...
/** Test base for scheduler related test cases. The test are executed with the {@link SlotPool}. */
public abstract class SchedulerTestBase extends TestLogger {
    ...
    protected final class TestingSlotPoolSlotProvider implements SlotProvider {
        ...
        public TaskManagerLocation addTaskManager(int numberSlots) {
            ...
            try {
                ...
                throw new RuntimeException(
                        "Unexpected exception occurred. This indicates a programming bug.", e);
            }
            ...
            for (int i = 0; i < numberSlots; i++) {
                final SlotOffer slotOffer =
                        new SlotOffer(new AllocationID(), i, ResourceProfile.ANY);
        ...
        public CompletableFuture<LogicalSlot> allocateSlot(
            ...
            return supplyInMainThreadExecutor(
                    () ->
                            scheduler
                                    .allocateSlot(task, slotProfile, allocationTimeout)
                                    .thenApply(
                                            (LogicalSlot logicalSlot) -> {
                                                switch (logicalSlot.getLocality()) {
                                                    case LOCAL:
                                                        numberOfLocalizedAssignments
                                                                .incrementAndGet();
                                                        break;
                                                    case UNCONSTRAINED:
                                                        numberOfUnconstrainedAssignments
                                                                .incrementAndGet();
                                                        break;
                                                    case NON_LOCAL:
                                                        numberOfNonLocalizedAssignments
                                                                .incrementAndGet();
                                                        break;
                                                    case HOST_LOCAL:
                                                        numberOfHostLocalizedAssignments
                                                                .incrementAndGet();
                                                        break;
                                                    default:
                                                        // ignore
