...
# Disable Maven HTTP repo blocking (needed for legacy pyx4me.com repo used by maven-junction-plugin)
mkdir -p ~/.m2
...
# Skip flink-dist (uses maven-junction-plugin which depends on dead pyx4me.com repo)
mvn clean install -B -DskipTests -Dfast -pl '!flink-dist'
