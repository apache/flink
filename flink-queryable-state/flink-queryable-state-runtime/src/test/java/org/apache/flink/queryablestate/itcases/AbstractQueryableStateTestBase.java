...
public abstract class AbstractQueryableStateTestBase extends TestLogger {
	...
	public void testDuplicateRegistrationFailsJob() throws Exception {
		...
		CompletableFuture<JobStatus> jobStatusFuture = FutureUtils.retrySuccesfulWithDelay(
			() -> clusterClient.getJobStatus(jobGraph.getJobID()),
			Time.milliseconds(50),
			deadline,
			(jobStatus) -> jobStatus.equals(JobStatus.FAILED),
			TestingUtils.defaultScheduledExecutor());

		assertEquals(JobStatus.FAILED, jobStatusFuture.get(deadline.timeLeft().toMillis(), TimeUnit.MILLISECONDS));

		String failureCause = null;
		if (clusterClient instanceof StandaloneClusterClient) {
			// we are in legacy mode
			ActorGateway jobManagerGateway = clusterClient.getJobManagerGateway();


			// Get the job and check the cause
			JobManagerMessages.JobFound jobFound = FutureUtils.toJava(
				jobManagerGateway
					.ask(new JobManagerMessages.RequestJob(jobId), new FiniteDuration(deadline.timeLeft().toMillis(), TimeUnit.MILLISECONDS))
					.mapTo(ClassTag$.MODULE$.<JobManagerMessages.JobFound>apply(JobManagerMessages.JobFound.class)))
				.get(deadline.timeLeft().toMillis(), TimeUnit.MILLISECONDS);

			failureCause = jobFound.executionGraph().getFailureInfo().getExceptionAsString();
		} else if (clusterClient instanceof RestClusterClient) {
			JobResult jobResult =
				((RestClusterClient<?>) clusterClient).requestJobResult(jobId).get(deadline.timeLeft().toMillis(), TimeUnit.MILLISECONDS);
			failureCause = jobResult.getSerializedThrowable().get().getFullStringifiedStackTrace();
		} else {
			fail("Unknown ClusterClient implementation: " + clusterClient.getClass());
		}
