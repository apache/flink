...
public abstract class Transformation<T> {
    ...
    public void setResources(ResourceSpec minResources, ResourceSpec preferredResources) {
        OperatorValidationUtils.validateResourceRequirements(
    ...
    /**
     * Set the managed memory weight which indicates how much this transformation relies on managed
     * memory, so that a transformation highly relies on managed memory would be able to acquire
     * more managed memory in runtime (linear association). The default weight value is 1. Note that
     * currently it's only allowed to set the weight in cases of UNKNOWN resources.
     *
     * @param managedMemoryWeight The managed memory weight of this transformation
     * @throws IllegalArgumentException Thrown, if non-UNKNOWN resources are already set to this
     *     transformation
     */
    ...
    /**
     * Get the managed memory weight which indicates how much this transformation relies on managed
     * memory, so that a transformation highly relies on managed memory would be able to acquire
     * more managed memory in runtime (linear association). The default weight value is 1. Note that
     * it only works in cases of UNKNOWN resources.
     *
