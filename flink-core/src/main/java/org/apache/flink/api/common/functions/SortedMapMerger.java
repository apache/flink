...
/**
 * A {@link Merger} for sorted maps which merges maps by putting the pairs in the
 * second map into  the first one and merging the values under the same key.
 *
 * @param <K> The type of the keys in the maps to be merged.
 * @param <V> The type of the values in the maps to be merged.
 */
public final class SortedMapMerger<K, V> implements Merger<SortedMap<K, V>> {
	 ...
	 */
	public SortedMapMerger(final Merger<V> valueMerger) {
	...
	@Override
	public SortedMap<K, V> merge(
		final SortedMap<K, V> map1,
		final SortedMap<K, V> map2
	) {
