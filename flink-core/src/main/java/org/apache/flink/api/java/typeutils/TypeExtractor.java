...
public class TypeExtractor {
    ...
    private static Map<Type, Class<? extends TypeInfoFactory>> registeredTypeInfoFactories =
            new HashMap<>();

    /**
     * Registers a type information factory globally for a certain type. Every following type
     * extraction operation will use the provided factory for this type. The factory will have
    ...
    public static <IN, OUT> TypeInformation<OUT> getFoldReturnTypes(
            FoldFunction<IN, OUT> foldInterface, TypeInformation<IN> inType) {
    ...
    private <IN1, IN2, OUT> TypeInformation<OUT> createTypeInfoWithTypeHierarchy(
        ...
        // check if type is a subclass of tuple
            ...
            final TypeInformation<?>[] subTypesInfo =
                    createSubTypesInfo(
                            t, (ParameterizedType) curT, typeHierarchy, in1Type, in2Type, false);
    ...
    private <IN1, IN2> TypeInformation<?> createTypeInfoFromInputs(
            ...
            ArrayList<Type> returnTypeHierarchy,
        ...
        for (Type t : returnTypeHierarchy) {
            if (isClassType(t)
                    && Function.class.isAssignableFrom(typeToClass(t))
                    && typeToClass(t) != Function.class) {
        ...
        if (in1TypeInfo != null) {
            ...
            info =
                    createTypeInfoFromInput(
                            returnTypeVar,
                            new ArrayList<Type>(inputTypeHierarchy),
                            in1Type,
                            in1TypeInfo);
        }
    ...
    private <IN1> TypeInformation<?> createTypeInfoFromInput(
            TypeVariable<?> returnTypeVar,
            ArrayList<Type> inputTypeHierarchy,
            Type inType,
        ...
        // the input is a pojo
            ...
            for (Field field : fields) {
                ...
                if (fieldType instanceof TypeVariable
                    ...
                    ArrayList<Type> typeHierarchyWithFieldType =
    ...
    private static Type getParameterType(
            Class<?> baseClass, ArrayList<Type> typeHierarchy, Class<?> clazz, int pos) {
    ...
    private static Type getParameterTypeFromGenericType(
            Class<?> baseClass, ArrayList<Type> typeHierarchy, Type t, int pos) {
        // base class
        if (t instanceof ParameterizedType
    ...
    private static void validateInfo(
            ArrayList<Type> typeHierarchy, Type type, TypeInformation<?> typeInfo) {
        if (type == null) {
    ...
    private static void validateInputContainsExecutable(
            LambdaExecutable exec, TypeInformation<?> typeInfo) {
    ...
    private static Type materializeTypeVariable(
            ArrayList<Type> typeHierarchy, TypeVariable<?> typeVar) {
    ...
    private <X> TypeInformation<X> privateGetForClass(
            Class<X> clazz, ArrayList<Type> typeHierarchy) {
            ...
            for (Method m : clazz.getMethods()) {
                ...
                if ((methodNameLow.equals("set" + fieldNameLow)
                        ...
                        (m.getGenericParameterTypes()[0].equals(fieldType)
                                ...
                                || (fieldTypeGeneric != null
                                        && m.getGenericParameterTypes()[0].equals(
                                                fieldTypeGeneric)))
                        &&
    ...
    protected <OUT, IN1, IN2> TypeInformation<OUT> analyzePojo(
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                pojoFields.add(
                        new PojoField(field, new GenericTypeInfo<OUT>((Class<OUT>) genericClass)));
            }
        }
