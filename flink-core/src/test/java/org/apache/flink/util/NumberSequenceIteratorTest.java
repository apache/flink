...
public class NumberSequenceIteratorTest {
    ...
    private static void testSplitting(NumberSequenceIterator iter, int numSplits) {
        ...
        assertThat(numSplits, splits.length);

        // test start and end of range
        assertThat(iter.getCurrent(), splits[0].getCurrent());
        assertThat(iter.getTo(), splits[numSplits - 1].getTo());

        // test continuous range
        for (int i = 1; i < splits.length; i++) {
            assertThat(splits[i - 1].getTo() + 1, splits[i].getCurrent());
        }
