...
public class ReduceWrappingFunctionTest {
	...
	public void testWrappedReduceObject() {
		try {
			...
			AbstractFunction reducer = (AbstractFunction) reduceOp.getUserCodeWrapper().getUserCodeObject();
			...
			// test combine
			((GenericCombine<Record>) reducer).combine(source, collector);
			assertEquals(2, target.size());
			assertEquals(new IntValue(42), target.get(0).getField(0, IntValue.class));
			assertEquals(new LongValue(11), target.get(0).getField(1, LongValue.class));
			assertEquals(new IntValue(13), target.get(1).getField(0, IntValue.class));
			...
			SerializationUtils.clone(reducer);
		}
		catch (Exception e) {
			e.printStackTrace();
			fail(e.getMessage());
	...
	public void testWrappedReduceClass() {
		try {
			...
			UserCodeWrapper<GenericGroupReduce<Record, Record>> udf = reduceOp.getUserCodeWrapper();
			UserCodeWrapper<GenericGroupReduce<Record, Record>> copy = SerializationUtils.clone(udf);
			GenericGroupReduce<Record, Record> reducer = copy.getUserCodeObject();
