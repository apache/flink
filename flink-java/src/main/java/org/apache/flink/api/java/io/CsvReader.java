...
public class CsvReader {
	 ...
	 * @param fieldsOrder The order information between CSV data and POJO fields.
	 * @return The DataSet representing the parsed CSV data.
	 */
	public <T> DataSource<T> pojoType(Class<T> targetType, String[] fieldsOrder) {
		Preconditions.checkNotNull(targetType, "The target type class must not be null.");
		Preconditions.checkNotNull(fieldsOrder, "The fields order cannot be null if output type is a POJO.");

		@SuppressWarnings("unchecked")
		PojoTypeInfo<T> typeInfo = (PojoTypeInfo<T>) TypeExtractor.createTypeInfo(targetType);
		CsvInputFormat<T> inputFormat = new CsvInputFormat<T>(path, typeInfo);

		Class<?>[] classes = new Class<?>[fieldsOrder.length];
		for (int i = 0; i < fieldsOrder.length; i++) {
			int pos = typeInfo.getFieldIndex(fieldsOrder[i]);
			classes[i] = typeInfo.getPojoFieldAt(pos).type.getTypeClass();
		}

		configureInputFormat(inputFormat, classes);
		inputFormat.setOrderOfPOJOFields(fieldsOrder);
