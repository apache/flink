...
public class ReduceOperator<IN> extends SingleInputUdfOperator<IN, IN, ReduceOperator<IN>> {
	...
	public ReduceOperator(Grouping<IN> input, ReduceFunction<IN> function, IN initialValue) {
		super(input.getDataSet(), input.getDataSet().getType());
	...
	protected org.apache.flink.api.common.operators.SingleInputOperator<?, IN, ?> translateToDataFlow(Operator<IN> input) {
		...
		if (grouper == null) {
			...
			if (initialValue != null) {
				po.setInitialValue(getInputType().createSerializer(), initialValue);
			}
	...
	private static <T, K> MapOperatorBase<Tuple2<K, T>, T, ?> translateSelectorFunctionReducer(
			Keys.SelectorFunctionKeys<T, ?> rawKeys, ReduceFunction<T> function, TypeInformation<T> inputType,
			String name, Operator<T> input, int dop) {
		@SuppressWarnings("unchecked")
