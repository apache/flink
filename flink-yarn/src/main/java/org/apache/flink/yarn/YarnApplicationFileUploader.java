...
class YarnApplicationFileUploader implements AutoCloseable {
        ...
        if (Utils.isRemotePath(resourcePath.toString())) {
            ...
            final YarnLocalResourceDescriptor descriptor =
                    YarnLocalResourceDescriptor.fromFileStatus(
                            key, fileStatus, LocalResourceVisibility.APPLICATION);
    ...
    List<String> registerMultipleLocalResources(
            final Collection<Path> shipFiles, final String localResourcesDirectory)
            throws IOException {
            ...
            if (!isFlinkDistJar(relativePath.getName())) {
                ...
                final YarnLocalResourceDescriptor resourceDescriptor =
                        registerSingleLocalResource(
                                key, localPath, relativePath.getParent().toString(), true, true);
    ...
    public YarnLocalResourceDescriptor uploadFlinkDist(final Path localJarPath)
        ...
        flinkDist =
                registerSingleLocalResource(localJarPath.getName(), localJarPath, "", true, false);
        return flinkDist;
    }
    ...
    List<String> registerProvidedLocalResources() {
        ...
        providedSharedLibs.forEach(
                (fileName, fileStatus) -> {
                    ...
                    final YarnLocalResourceDescriptor descriptor =
                            YarnLocalResourceDescriptor.fromFileStatus(
                                    fileName, fileStatus, LocalResourceVisibility.PUBLIC);
