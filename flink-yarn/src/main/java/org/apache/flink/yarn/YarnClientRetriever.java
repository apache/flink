...
/**
 * Retriever is used to get yarn client for {@link ApplicationReportProviderImpl}. When external
 * yarn client is closed, it will create new yarn client that its lifecycle will be managed in this
 * retriever.
 */
@Internal
public final class YarnClientRetriever implements AutoCloseable {
    private static final Logger LOG = LoggerFactory.getLogger(YarnClientRetriever.class);

    private final YarnClient externalYarnClient;
    private final YarnConfiguration yarnConfiguration;
    private YarnClient createdYarnClient;

    private YarnClientRetriever(YarnClient yarnClient, YarnConfiguration yarnConfiguration) {
    ...
    public static YarnClientRetriever from(
            YarnClient yarnClient, YarnConfiguration yarnConfiguration) {
        return new YarnClientRetriever(yarnClient, yarnConfiguration);
    }
}
