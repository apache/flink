...
public class JoinTranslationTest extends CompilerTestBase {
	...
	public void testBroadcastHashFirstTest() {
		try {
			DualInputPlanNode node = createPlanAndGetJoinNode(JoinOperatorBase.JoinHint.BROADCAST_HASH_FIRST);
	...
	public void testBroadcastHashSecondTest() {
		try {
			DualInputPlanNode node = createPlanAndGetJoinNode(JoinOperatorBase.JoinHint.BROADCAST_HASH_SECOND);
	...
	public void testPartitionHashFirstTest() {
		try {
			DualInputPlanNode node = createPlanAndGetJoinNode(JoinOperatorBase.JoinHint.REPARTITION_HASH_FIRST);
	...
	public void testPartitionHashSecondTest() {
		try {
			DualInputPlanNode node = createPlanAndGetJoinNode(JoinOperatorBase.JoinHint.REPARTITION_HASH_SECOND);
			assertEquals(ShipStrategyType.PARTITION_HASH, node.getInput1().getShipStrategy());
			assertEquals(ShipStrategyType.PARTITION_HASH, node.getInput2().getShipStrategy());
			assertEquals(DriverStrategy.HYBRIDHASH_BUILD_SECOND, node.getDriverStrategy());
		}
	...
	public void testPartitionSortMergeTest() {
		try {
			DualInputPlanNode node = createPlanAndGetJoinNode(JoinOperatorBase.JoinHint.REPARTITION_SORT_MERGE);
			assertEquals(ShipStrategyType.PARTITION_HASH, node.getInput1().getShipStrategy());
			assertEquals(ShipStrategyType.PARTITION_HASH, node.getInput2().getShipStrategy());
	...
	public void testOptimizerChoosesTest() {
		try {
			DualInputPlanNode node = createPlanAndGetJoinNode(JoinOperatorBase.JoinHint.OPTIMIZER_CHOOSES);
			assertEquals(ShipStrategyType.PARTITION_HASH, node.getInput1().getShipStrategy());
			assertEquals(ShipStrategyType.PARTITION_HASH, node.getInput2().getShipStrategy());
