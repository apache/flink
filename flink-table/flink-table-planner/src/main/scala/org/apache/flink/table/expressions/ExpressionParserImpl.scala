 ...
 *
 * See ImplicitExpressionConversions and ImplicitExpressionOperations for the constructs available
 * in the Scala Expression DSL. This parser must be kept in sync with the Scala DSL lazy valined in
 * the above files.
 */
object ExpressionParserImpl extends JavaTokenParsers with PackratParsers with ExpressionParser {
  ...
  lazy val timeIntervalUnit: PackratParser[Expression] = TimeIntervalUnit.values
    .map { unit: TimeIntervalUnit => literal(unit.toString) ^^^ valueLiteral(unit) }
    .reduceLeft(_ | _)

  lazy val timePointUnit: PackratParser[Expression] = TimePointUnit.values
    .map { unit: TimePointUnit => literal(unit.toString) ^^^ valueLiteral(unit) }
    .reduceLeft(_ | _)
  ...
  lazy val suffixAsc: PackratParser[Expression] = composite <~ "." ~ ASC ~ opt("()") ^^ {
    e => unresolvedCall(BuiltInFunctionDefinitions.ORDER_ASC, e)
  }

  lazy val suffixDesc: PackratParser[Expression] = composite <~ "." ~ DESC ~ opt("()") ^^ {
    e => unresolvedCall(BuiltInFunctionDefinitions.ORDER_DESC, e)
  }
  ...
  lazy val suffixFlattening: PackratParser[Expression] =
    composite <~ "." ~ FLATTEN ~ opt("()") ^^ {
      e => unresolvedCall(BuiltInFunctionDefinitions.FLATTEN, e)
    }

  lazy val suffixDistinct: PackratParser[Expression] =
    composite <~ "." ~ DISTINCT ~ opt("()") ^^ {
      e => unresolvedCall(BuiltInFunctionDefinitions.DISTINCT, e)
    }
  ...
  lazy val unaryNot: PackratParser[Expression] = "!" ~> composite ^^ {
    e => unresolvedCall(BuiltInFunctionDefinitions.NOT, e)
  }

  lazy val unaryMinus: PackratParser[Expression] = "-" ~> composite ^^ {
    e => unresolvedCall(BuiltInFunctionDefinitions.MINUS_PREFIX, e)
  }
  ...
  private def throwError(msg: String, next: Input): Nothing = {
    ...
    throw new ExpressionParserException(s"""Could not parse expression at column ${
                                            next.pos.column
                                          }: $improvedMsg
