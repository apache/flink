    ...
    val tableSource = TableFactoryUtil.findAndCreateTableSource(
      ...
      new ResolvedCatalogTable(
        CatalogTable.newBuilder()
          .schema(
            Schema.newBuilder().fromResolvedSchema(resolvedSchemaWithRemovedTimeAttribute).build())
          .comment(tableToFind.getComment)
          .partitionKeys(tableToFind.getPartitionKeys)
          .options(tableToFind.getOptions).build(),
          ...
          if (FlinkTypeFactory.isTimeIndicatorType(f.getType)) {
            f.getType match {
              case ts: TimestampType =>
                new TimestampType(ts.isNullable, TimestampKind.REGULAR, ts.getPrecision)
              case ltz: LocalZonedTimestampType =>
                new LocalZonedTimestampType(ltz.isNullable, TimestampKind.REGULAR, ltz.getPrecision)
              case _ =>
                throw new ValidationException(
                  "The supported time indicator type" +
