...
object ScalarOperatorGens {
  ...
  def generateIn(
      ...
      haystack: Seq[GeneratedExpression]): GeneratedExpression = {
    ...
    if (haystack.forall(_.literal)) {
      ...
      val castNumeric = widerType match {
        case Some(t) =>
          (value: GeneratedExpression) => numericCasting(ctx, value.resultType, t)(value.resultTerm)
  ...
  def generateEquals(
    ...
    // generic types of same type
      ...
      val code = s"""
                    ...
                    |  $resultTerm =
                    |    ${left.resultTerm}.getBinarySection().equals(${
                     right.resultTerm
                   }.getBinarySection());
                    |}
                    |""".stripMargin
