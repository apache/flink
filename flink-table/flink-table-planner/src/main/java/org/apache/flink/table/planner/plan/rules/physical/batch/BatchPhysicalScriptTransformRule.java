...
public class BatchPhysicalScriptTransformRule extends ConverterRule {

    public static final RelOptRule INSTANCE =
            new BatchPhysicalScriptTransformRule(
                    Config.INSTANCE
                            .withConversion(
                                    FlinkLogicalScriptTransform.class,
                                    FlinkConventions.LOGICAL(),
                                    FlinkConventions.BATCH_PHYSICAL(),
                                    "BatchExecScriptTrans")
                            .withRuleFactory(BatchPhysicalScriptTransformRule::new));

    protected BatchPhysicalScriptTransformRule(Config config) {
        super(config);
    }

    @Override
    public RelNode convert(RelNode rel) {
        FlinkLogicalScriptTransform logicalScriptTransform = (FlinkLogicalScriptTransform) rel;

        RelTraitSet newTrait = rel.getTraitSet().replace(FlinkConventions.BATCH_PHYSICAL());
        RelNode newInput =
                RelOptRule.convert(
                        logicalScriptTransform.getInput(), FlinkConventions.BATCH_PHYSICAL());

        return new BatchPhysicalScriptTransformationExec(
                rel.getCluster(),
                newTrait,
                newInput,
