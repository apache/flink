...
/**
 * This rules is copied from Calcite's {@link org.apache.calcite.rel.rules.FilterJoinRule}. NOTES:
 * This file should be deleted when CALCITE-3170 is fixed, and please make sure to synchronize with
 * FlinkFilterJoinRule in blink planner when changing this class. Modification: - Handles the ON
 * condition of anti-join can not be pushed down, lines added (192-198)
 */
...
public abstract class FlinkFilterJoinRule extends RelOptRule {
    ...
    public static final FlinkFilterJoinRule FILTER_ON_JOIN =
            new FlinkFilterIntoJoinRule(true, RelFactories.LOGICAL_BUILDER, TRUE_PREDICATE);

    /**
     * Dumber version of {@link #FILTER_ON_JOIN}. Not intended for production use, but keeps some
     * tests working for which {@code FILTER_ON_JOIN} is too smart.
     */
    public static final FlinkFilterJoinRule DUMB_FILTER_ON_JOIN =
            new FlinkFilterIntoJoinRule(false, RelFactories.LOGICAL_BUILDER, TRUE_PREDICATE);
    ...
    /** Creates a FlinkFilterJoinRule with an explicit root operand and factories. */
    ...
    protected void perform(RelOptRuleCall call, Filter filter, Join join) {
        ...
        final List<RexNode> aboveFilters =
                filter != null ? RelOptUtil.conjunctions(filter.getCondition()) : new ArrayList<>();
