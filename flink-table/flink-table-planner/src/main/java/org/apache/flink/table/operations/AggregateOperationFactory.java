...
public class AggregateOperationFactory {
	...
	private ResolvedGroupWindow validateAndCreateSlideWindow(
		...
		if (windowSizeType != INTERVAL_ROWS && windowSizeType != INTERVAL_MILLIS) {
			throw new ValidationException(
				"Sliding window expects size literal of type Interval of Milliseconds or Interval of Rows.");
		}
	...
	private ResolvedGroupWindow validateAndCreateSessionWindow(
			...
			String windowName,
			FieldReferenceExpression timeField) {
		ValueLiteralExpression windowGap = getAsValueLiteral(window.getGap(), "Window expects a gap value literal.");

		if (windowGap.getType() != INTERVAL_MILLIS) {
			throw new ValidationException("Session window expects gap literal of type Interval of Milliseconds.");
		}
