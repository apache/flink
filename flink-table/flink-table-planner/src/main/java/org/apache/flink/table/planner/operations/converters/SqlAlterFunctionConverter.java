...
public class SqlAlterFunctionConverter implements SqlNodeConverter<SqlAlterFunction> {
    @Override
    public Operation convertSqlNode(SqlAlterFunction sqlAlterFunction, ConvertContext context) {
        if (sqlAlterFunction.isSystemFunction()) {
            throw new ValidationException("Alter temporary system function is not supported");
        }

        FunctionLanguage language =
                OperationConverterUtils.parseLanguage(sqlAlterFunction.getFunctionLanguage());
