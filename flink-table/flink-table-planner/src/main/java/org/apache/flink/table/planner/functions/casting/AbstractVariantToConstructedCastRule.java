...
/**
 * Base class for the rules that cast a {@link LogicalTypeRoot#VARIANT} to a constructed target,
 * imposing a schema on a variant. A constructed cast is the scalar cast applied to every leaf plus
 ...
 * or a VARIANT {@code null} in a {@code NOT NULL} position, so {@code TRY_CAST} wraps the whole
...
abstract class AbstractVariantToConstructedCastRule<OUT>
    ...
    /**
     * Treats a variant that stores a {@code null} as a {@code NULL} input, so a top-level VARIANT
