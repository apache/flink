 ...
 * than by position, because a VARIANT object is unordered. Name matching is case sensitive. A
 * target field absent from the object fails the cast. A field present but set to a VARIANT null
 * maps to SQL {@code NULL} when it is nullable and fails the cast when it is {@code NOT NULL}. A
 * {@link LogicalTypeRoot#VARIANT} target field is the exception: there the field cast is the
 * identity, so a VARIANT null is kept as a variant null rather than downgraded to SQL {@code NULL}.
 * Object fields the target does not name are dropped, so the row is a projection. A {@code
 * STRUCTURED} target shares the {@code RowData} representation and is served by the same rule.
 */
