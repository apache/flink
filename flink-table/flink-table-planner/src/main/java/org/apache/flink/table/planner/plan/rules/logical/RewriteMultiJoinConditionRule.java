...
public class RewriteMultiJoinConditionRule
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        scala.collection.mutable.Map<RexNode, List<RexNode>> equiJoinFilterMap =
        ...
        equiJoinFilters.stream()
                ...
                .forEach(
                        rexNode -> {
                            ...
                            equiJoinFilterMap
                                    .getOrElseUpdate(left, () -> new ArrayList<>())
                                    .add(right);
                            equiJoinFilterMap
                                    .getOrElseUpdate(right, () -> new ArrayList<>())
                                    .add(left);
                        });

        List<List<RexNode>> candidateJoinFilters =
                scala.collection.JavaConverters.bufferAsJavaList(
                        equiJoinFilterMap.values().filter(list -> list.size() > 1).toBuffer());
