...
/**
 * Accumulator for computing embedding model evaluation metrics. This accumulator calculates the
 * following similarity/distance metrics between embeddings: 1. Cosine Similarity: cos(θ) = (v1 ·
 * v2) / (||v1|| ||v2||) where: - v1 · v2 is the dot product: Σ(v1_i * v2_i) - ||v|| is the L2 norm:
 * √(Σv_i²) - Ranges from -1 (opposite direction) to 1 (same direction) - Value of 0 indicates
 * orthogonality (90° angle) 2. Jaccard Similarity: J(v1,v2) = Σ_min(v1_i, v2_i) / Σ_max(v1_i, v2_i)
 * where: - Treats vectors as fuzzy sets - Numerator represents intersection - Denominator
 * represents union - Ranges from 0 (no overlap) to 1 (identical) 3. Euclidean Distance: d(v1,v2) =
 * √(Σ(v1_i - v2_i)²) where: - Measures straight-line distance in vector space - Always non-negative
 * - Value of 0 indicates identical vectors - Sensitive to magnitude differences The accumulator
 * supports both accumulation of new values and retraction of existing values, making it suitable
 * for streaming scenarios with updates and retractions.
 */
@Internal
public class EmbeddingEvaluatorAccumulator extends ModelEvaluatorAccumulator {
    ...
    public void accumulate(Object... args) {
        ...
        Float[] actual = (Float[]) args[0];
        Float[] predicted = (Float[]) args[1];
