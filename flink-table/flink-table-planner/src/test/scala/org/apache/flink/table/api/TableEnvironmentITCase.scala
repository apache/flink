...
class TableEnvironmentITCase(tableEnvName: String) {
  ...
  def testSqlUpdateAndToDataStream(): Unit = {
    ...
    streamTableEnv.asInstanceOf[TableEnvironmentInternal].registerTableSource(
      "MyTable", getPersonCsvTableSource)
  ...
  def testExecuteSqlWithInsertOverwrite(): Unit = {
    ...
    tEnv.asInstanceOf[TableEnvironmentInternal].registerTableSink("MySink", configuredSink)
  ...
  def testStatementSetWithOverwrite(): Unit = {
    ...
    val configuredSink1 = new TestingOverwritableTableSink(sink1Path)
      .configure(Array("first"), Array(STRING))
    tEnv.asInstanceOf[TableEnvironmentInternal].registerTableSink("MySink1", configuredSink1)
    checkEmptyFile(sink1Path)
    ...
    tEnv.asInstanceOf[TableEnvironmentInternal].registerTableSink("MySink2", configuredSink2)
    checkEmptyFile(sink2Path)
  ...
  private def registerCsvTableSink(
    ...
    tEnv.asInstanceOf[TableEnvironmentInternal].registerTableSink(tableName, configuredSink)
