...
class SubplanReuseTest extends TableTestBase {
  ...
  def testDisableSubplanReuse(): Unit = {
    util.tableEnv.getConfig.getConfiguration
      .setBoolean(OptimizerConfigOptions.TABLE_OPTIMIZER_REUSE_SUB_PLAN_ENABLED, false)
    val sqlQuery =
      """
        |WITH r AS (
  ...
  def testSubplanReuseOnSourceWithLimit(): Unit = {
    ...
    util.tableEnv.getConfig.getConfiguration
      .setBoolean(OptimizerConfigOptions.TABLE_OPTIMIZER_REUSE_SOURCE_ENABLED, true)
    util.tableEnv.getConfig.getConfiguration
      .setString(ExecutionConfigOptions.TABLE_EXEC_DISABLED_OPERATORS, "HashJoin,SortMergeJoin")
    val sqlQuery =
      """
  ...
  def testSubplanReuseOnSortMergeJoin(): Unit = {
    ...
    util.tableEnv.getConfig.getConfiguration
      .setBoolean(BatchPhysicalSortMergeJoinRule.TABLE_OPTIMIZER_SMJ_REMOVE_SORT_ENABLED, true)
    val sqlQuery =
      """
