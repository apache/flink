...
class FlinkRelMdColumnUniquenessTest extends FlinkRelMdHandlerTestBase {
  ...
  def testAreColumnsUniqueOnTablePartiallyProjectedKey(): Unit = {
    Array(
      ...
      .foreach { scan =>
        assertNull(mq.areColumnsUnique(scan, ImmutableBitSet.of(0)))
        assertNull(mq.areColumnsUnique(scan, ImmutableBitSet.of(0, 1)))
        assertNull(mq.areColumnsUnique(scan, ImmutableBitSet.of(0, 1, 2)))
        assertNull(mq.areColumnsUnique(scan, ImmutableBitSet.of(0, 1, 2, 3)))
      }
  }
