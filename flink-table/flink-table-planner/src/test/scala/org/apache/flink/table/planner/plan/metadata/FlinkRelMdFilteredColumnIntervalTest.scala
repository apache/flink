...
class FlinkRelMdFilteredColumnIntervalTest extends FlinkRelMdHandlerTestBase {
  ...
  private var expr1, expr2, expr3, expr4, expr5, expr6, expr7, expr8, expr9, expr10, expr11, expr12,
      expr13: RexNode = _
  ...
  def testGetColumnIntervalOnProject(): Unit = {
    ...
    assertEquals(
      ValueInterval(bd(0d), bd(1.1d), includeUpper = false),
      mq.getFilteredColumnInterval(p, 1, 11))
  ...
  def testGetColumnIntervalOnAggregate(): Unit = {
    Array(
      ...
      streamGlobalAggWithLocal).foreach {
      agg =>
    ...
    Array(streamLocalAgg, batchLocalAgg).foreach {
      agg =>
        assertEquals(ValueInterval(bd(12), bd(18)), mq.getFilteredColumnInterval(agg, 0, -1))
        assertNull(mq.getFilteredColumnInterval(agg, 1, -1))
    ...
    Array(
      ...
      batchGlobalAggWithLocalWithAuxGroup).foreach {
      agg =>
  ...
  def testGetColumnIntervalOnAggregateWithFilter(): Unit = {
    ...
    Array(
      ...
      streamGlobalAggWithoutLocalWithFilter).foreach {
      agg =>
        assertEquals(ValueInterval(bd(12), bd(18)), mq.getFilteredColumnInterval(agg, 0, -1))
        assertNull(mq.getFilteredColumnInterval(agg, 1, -1))
        ...
        assertEquals(
          ValueInterval(bd(12), bd(18)),
          mq.getFilteredColumnInterval(agg.getInput, 3, 8))
    ...
    Array(batchGlobalAggWithLocalWithFilter, streamGlobalAggWithLocalWithFilter).foreach {
      agg =>
        assertEquals(ValueInterval(bd(12), bd(18)), mq.getFilteredColumnInterval(agg, 0, -1))
        assertNull(mq.getFilteredColumnInterval(agg, 1, -1))
    ...
    Array(streamLocalAggWithFilter).foreach {
      agg =>
        assertEquals(ValueInterval(bd(12), bd(18)), mq.getFilteredColumnInterval(agg, 0, -1))
        assertNull(mq.getFilteredColumnInterval(agg, 1, -1))
        assertEquals(
