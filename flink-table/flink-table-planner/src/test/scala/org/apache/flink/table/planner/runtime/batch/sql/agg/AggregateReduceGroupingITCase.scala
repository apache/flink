...
class AggregateReduceGroupingITCase extends BatchTestBase {
  ...
  override def before(): Unit = {
    ...
    registerCollection(
      "T2",
      Seq(row(1, 1, "X"), row(1, 2, "Y"), row(2, 3, null), row(2, 4, "Z")),
      new RowTypeInfo(Types.INT, Types.INT, Types.STRING),
      "a2, b2, c2",
      Array(true, true, true),
      FlinkStatistic
        .builder()
        .uniqueKeys(Set(Set("b2").asJava, Set("a2", "b2").asJava).asJava)
        .build()
    )
    ...
    registerCollection(
      "T4",
      Seq(
        row(1, 1, "A", localDateTime("2018-06-01 10:05:30"), "Hi"),
      ...
      (0 until 50000).map(
        i =>
          row(
            i,
            1L,
            if (i % 500 == 0) null else s"Hello$i",
            "Hello world",
            10,
  ...
  def testSingleAggOnTable_SortAgg(): Unit = {
    ...
    checkResult(
      "SELECT a6, b6, max(c6), count(d6), sum(e6) FROM T6 GROUP BY a6, b6",
  ...
  private def testSingleAggOnTable(): Unit = {
    // group by fix length
    checkResult(
      "SELECT a1, b1, count(c1) FROM T1 GROUP BY a1, b1",
      Seq(row(2, 1, 1), row(3, 2, 1), row(5, 2, 1), row(6, 3, 1)))
    // group by string
    checkResult(
      "SELECT a1, c1, count(d1), avg(b1) FROM T1 GROUP BY a1, c1",
      Seq(row(2, "A", 0, 1), row(3, "A", 1, 2), row(5, "B", 1, 2), row(6, "C", 1, 3)))
    checkResult(
      "SELECT c5, d5, avg(b5), avg(a5) FROM T5 WHERE d5 IS NOT NULL GROUP BY c5, d5",
      Seq(
        row("B", "Hi", 2, 3),
        row("C", "Hello", null, 1),
        row("D", "Hello world", 3, 4),
        row("E", "Hello world, how are you?", 1, 3),
        row("I", "hahaha", 2, 7),
        row("J", "I am fine.", 1, 6)
      )
    )
    ...
    checkResult(
      "SELECT a1, d1, count(d1) FROM T1 GROUP BY a1, d1",
      Seq(row(2, null, 0), row(3, "Hi", 1), row(5, "Hello", 1), row(6, "Hello world", 1)))
    checkResult(
      "SELECT c5, d5, avg(b5), avg(a5) FROM T5 GROUP BY c5, d5",
      Seq(
        row("A", null, 1, 2),
        row("B", "Hi", 2, 3),
        row("C", "Hello", null, 1),
    ...
    checkResult(
      "SELECT a3, b3, count(c3) FROM T3 GROUP BY a3, b3",
    ...
    checkResult(
      "SELECT a6, c6, avg(b6), count(d6), avg(e6) FROM T6 GROUP BY a6, c6",
      (0 until 50000).map(i => row(i, if (i % 500 == 0) null else s"Hello$i", 1, 1L, 10)))
    checkResult(
      "SELECT a6, d6, avg(b6), count(c6), avg(e6) FROM T6 GROUP BY a6, d6",
      (0 until 50000).map(i => row(i, "Hello world", 1, if (i % 500 == 0) 0L else 1L, 10)))
    checkResult(
      "SELECT a6, f6, avg(b6), count(c6), avg(e6) FROM T6 GROUP BY a6, f6",
      (0 until 50000).map(
        i => row(i, new Date(i + 1531820000000L), 1, if (i % 500 == 0) 0L else 1L, 10)))
  }

  @Test
  def testMultiAggs(): Unit = {
    checkResult(
      "SELECT a1, b1, c1, d1, m, COUNT(*) FROM " +
      ...
      Seq(
        ...
        row(3, 2, "A", 1, "Hi", 1),
        row(5, 2, "B", 1, "Hello", 1),
        row(6, 3, "C", 1, "Hello world", 1))
    )

    checkResult(
      "SELECT a3, b3, c, s, COUNT(*) FROM " +
  ...
  def testAggOnInnerJoin(): Unit = {
    ...
    checkResult(
      "SELECT a2, b2, a3, b3, COUNT(c2) FROM " +
  ...
  def testAggOnLeftJoin(): Unit = {
    ...
    checkResult(
      "SELECT a1, b1, a3, b3, COUNT(c1) FROM " +
    ...
    checkResult(
      "SELECT a3, b3, a1, b1, COUNT(c1) FROM " +
        "(SELECT * FROM T3 LEFT JOIN T1 ON a1 = a3) t GROUP BY a3, b3, a1, b1",
      Seq(
        row(1, 10, null, null, 0),
        row(2, 20, 2, 1, 2),
        row(3, 10, 3, 2, 1),
        row(4, 20, null, null, 0),
        row(4, null, null, null, 0))
    )
  }
  ...
  def testAggOnRightJoin(): Unit = {
    ...
    checkResult(
      ...
      Seq(
        row(2, 1, 2, 20, 2),
        row(3, 2, 3, 10, 1),
        row(null, null, 1, 10, 0),
        row(null, null, 4, 20, 0),
  ...
  def testAggOnWindow(): Unit = {
    ...
    checkResult(
      ...
      Seq(
        row(1, "Hi", LocalDateTime.of(2018, 6, 1, 10, 0, 0), 1, 1),
    ...
    checkResult(
      ...
      Seq(
        row(1, "A", LocalDateTime.of(2018, 6, 1, 10, 0, 0), 1),
  ...
  def testAggWithGroupingSets(): Unit = {
    checkResult(
      "SELECT a1, b1, c1, COUNT(d1) FROM T1 " +
  ...
  def testAggWithRollup(): Unit = {
    checkResult(
      "SELECT a1, b1, c1, COUNT(d1) FROM T1 GROUP BY ROLLUP (a1, b1, c1)",
      Seq(
        row(2, 1, "A", 0),
        row(2, 1, null, 0),
        row(2, null, null, 0),
  ...
  def testAggWithCube(): Unit = {
    checkResult(
      "SELECT a1, b1, c1, COUNT(d1) FROM T1 GROUP BY CUBE (a1, b1, c1)",
      Seq(
        row(2, 1, "A", 0),
        row(2, 1, null, 0),
        row(2, null, "A", 0),
  ...
  def testSingleDistinctAgg(): Unit = {
    checkResult(
      "SELECT a1, COUNT(DISTINCT c1) FROM T1 GROUP BY a1",
      Seq(row(2, 1), row(3, 1), row(5, 1), row(6, 1)))

    checkResult(
      "SELECT a1, b1, COUNT(DISTINCT c1) FROM T1 GROUP BY a1, b1",
      Seq(row(2, 1, 1), row(3, 2, 1), row(5, 2, 1), row(6, 3, 1)))
  }
  ...
  def testSingleDistinctAgg_WithNonDistinctAgg(): Unit = {
    checkResult(
      "SELECT a1, COUNT(DISTINCT c1), SUM(b1) FROM T1 GROUP BY a1",
      Seq(row(2, 1, 1), row(3, 1, 2), row(5, 1, 2), row(6, 1, 3)))

    checkResult(
      "SELECT a1, c1, COUNT(DISTINCT c1), SUM(b1) FROM T1 GROUP BY a1, c1",
    ...
    checkResult(
      "SELECT a1, d1, COUNT(DISTINCT c1), SUM(b1) FROM T1 GROUP BY a1, d1",
      Seq(
        row(2, null, 1, 1),
        row(3, "Hi", 1, 2),
        row(5, "Hello", 1, 2),
  ...
  def testMultiDistinctAggs(): Unit = {
    checkResult(
      "SELECT a1, COUNT(DISTINCT b1), SUM(DISTINCT b1) FROM T1 GROUP BY a1",
      Seq(row(2, 1, 1), row(3, 1, 2), row(5, 1, 2), row(6, 1, 3)))

    checkResult(
      "SELECT a1, d1, COUNT(DISTINCT c1), SUM(DISTINCT b1) FROM T1 GROUP BY a1, d1",
      Seq(
        row(2, null, 1, 1),
        row(3, "Hi", 1, 2),
        row(5, "Hello", 1, 2),
    ...
    checkResult(
      "SELECT a1, b1, COUNT(DISTINCT c1), COUNT(DISTINCT d1) FROM T1 GROUP BY a1, b1",
