...
abstract class TableTestUtil(verifyCatalogPath: Boolean = false) {
  ...
  protected def verifyString(expected: String, optimized: RelNode) {
    ...
    val acutalWithAdjustedPath = if (!verifyCatalogPath) {
      actualNoCharset.replaceAll("default-catalog, default-database, ", "")
    } else {
      actualNoCharset
    }

    val expectedLines = expected.split("\n").map(_.trim)
    val actualLines = acutalWithAdjustedPath.split("\n").map(_.trim)
...
object TableTestUtil {
  ...
  def extractBuiltinPath(config: TableConfig, catalogManager: CatalogManager): TableConfig = {
    config.setBultinCatalogName(catalogManager.getCurrentCatalog)
    config.setBultinDatabaseName(catalogManager.getCurrentDatabase)
    config
  }
