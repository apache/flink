...
import org.apache.flink.table.api.DataTypes.{BIGINT, DOUBLE, INT, STRING, TIMESTAMP}
...
class DagOptimizationTest extends TableTestBase {
  ...
  def testSingleSink1(): Unit = {
    ...
    val appendSink = createBlackHoleSinkDescriptor(("c", STRING), ("cnt", BIGINT))
  ...
  def testSingleSink2(): Unit = {
    ...
    val appendSink = createBlackHoleSinkDescriptor(("a1", INT), ("b", BIGINT), ("c1", STRING))
  ...
  def testSingleSink3(): Unit = {
    ...
    val appendSink = createBlackHoleSinkDescriptor(("a1", INT), ("b1", BIGINT))
  ...
  def testSingleSinkWithUDTF(): Unit = {
    ...
    val sink = createBlackHoleSinkDescriptor(
      ("a", INT),
      ("b", BIGINT),
      ("c", STRING),
      ("d", INT),
  ...
  def testSingleSinkSplitOnUnion(): Unit = {
    ...
    val table = util.tableEnv.sqlQuery(sqlQuery)
    val sink = createBlackHoleSinkDescriptor(("total_sum", INT))
    util.verifyExecPlanInsert(table, sink, "sink")
  }
  ...
  def testMultiSinks1(): Unit = {
    ...
    val sink1 = createBlackHoleSinkDescriptor(("total_sum", INT))
    util.tableEnv.createTemporaryTable("sink1", sink1)
    stmtSet.addInsert("sink1", table2)

    val sink2 = createBlackHoleSinkDescriptor(("total_min", INT))
  ...
  def testMultiSinks2(): Unit = {
    ...
    val sink1 = createBlackHoleSinkDescriptor(("a", INT), ("b1", BIGINT))
    util.tableEnv.createTemporaryTable("sink1", sink1)
    stmtSet.addInsert("sink1", table3)

    val sink2 = createBlackHoleSinkDescriptor(("a", INT), ("b1", BIGINT))
    util.tableEnv.createTemporaryTable("sink2", sink2)
    stmtSet.addInsert("sink2", table3)
  ...
  def testMultiSinks4(): Unit = {
    ...
    val sink1 = createBlackHoleSinkDescriptor(("a1", INT), ("b", BIGINT), ("c2", STRING))
    util.tableEnv.createTemporaryTable("sink1", sink1)
    stmtSet.addInsert("sink1", table5)

    val sink2 = createBlackHoleSinkDescriptor(("a1", INT), ("b", BIGINT), ("c1", STRING))
    util.tableEnv.createTemporaryTable("sink2", sink2)
  ...
  def testMultiLevelViews(): Unit = {
    ...
    val sink1 = createBlackHoleSinkDescriptor(("a", INT), ("b", BIGINT), ("c", STRING))
    util.tableEnv.createTemporaryTable("sink1", sink1)
    ...
    val sink2 = createBlackHoleSinkDescriptor(("b", BIGINT), ("cnt", BIGINT))
    util.tableEnv.createTemporaryTable("sink2", sink2)
    stmtSet.addInsert("sink2", table4)

    val table5 = util.tableEnv.sqlQuery("SELECT b, cnt FROM TempTable3 WHERE b >=4 AND b < 6")
    val sink3 = createBlackHoleSinkDescriptor(("b", BIGINT), ("cnt", BIGINT))
    util.tableEnv.createTemporaryTable("sink3", sink3)
  ...
  def testMultiSinksWithUDTF(): Unit = {
    ...
    val sink1 = createBlackHoleSinkDescriptor(("a", INT), ("total_c", BIGINT))
    util.tableEnv.createTemporaryTable("sink1", sink1)
    stmtSet.addInsert("sink1", table5)

    val sqlQuery6 = "SELECT * FROM table4 WHERE a < 50"
    val table6 = util.tableEnv.sqlQuery(sqlQuery6)
    val sink2 = createBlackHoleSinkDescriptor(("a", INT), ("total_c", BIGINT))
    util.tableEnv.createTemporaryTable("sink2", sink2)
    stmtSet.addInsert("sink2", table6)
  ...
  def testMultiSinksSplitOnUnion2(): Unit = {
    ...
    val sink3 = createBlackHoleSinkDescriptor(("a", INT))
    util.tableEnv.createTemporaryTable("sink3", sink3)
  ...
  def testMultiSinksSplitOnUnion3(): Unit = {
    ...
    val sink1 = createBlackHoleSinkDescriptor(("a", INT), ("c", STRING))
    util.tableEnv.createTemporaryTable("sink1", sink1)
    ...
    val sink2 = createBlackHoleSinkDescriptor(("total_sum", INT))
    util.tableEnv.createTemporaryTable("sink2", sink2)
    stmtSet.addInsert("sink2", table2)

    val table3 = util.tableEnv.sqlQuery("SELECT MIN(a) AS total_min FROM TempTable1")
    val sink3 = createBlackHoleSinkDescriptor(("total_min", INT))
    util.tableEnv.createTemporaryTable("sink3", sink3)
    stmtSet.addInsert("sink3", table3)
  ...
  def testMultiSinksWithWindow(): Unit = {
    ...
    val table1 = util.tableEnv.sqlQuery(sqlQuery1)
    val sink1 = createBlackHoleSinkDescriptor(
      ("a", INT),
      ("sum_c", DOUBLE),
      ("time", TIMESTAMP(3)),
    ...
    val table2 = util.tableEnv.sqlQuery(sqlQuery2)
    val sink2 = createBlackHoleSinkDescriptor(("a", INT), ("sum_c", DOUBLE), ("time", TIMESTAMP(3)))
    util.tableEnv.createTemporaryTable("sink2", sink2)
    stmtSet.addInsert("sink2", table2)
