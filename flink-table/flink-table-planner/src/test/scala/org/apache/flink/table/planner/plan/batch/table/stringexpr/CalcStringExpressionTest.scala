...
class CalcStringExpressionTest extends TableTestBase {
  ...
  def testFilterOnCustomType(): Unit = {
    val util = batchTestUtil()
    val t = util
      .addTableSource[CustomType]("Table3", 'myInt, 'myLong, 'myString)
      .as("i", "l", "s")
  ...
  def testAdvancedDataTypes(): Unit = {
    ...
    val t1 = t.select(
      ...
      "1984-07-12".cast(Types.SQL_DATE),
      "14:34:24".cast(Types.SQL_TIME),
      "1984-07-12 14:34:24".cast(Types.SQL_TIMESTAMP)
    )
    val t2 = t.select(
      "a, b, c, d, e, 11.2p, 11.2p," +
  ...
  def testNumericAutoCastInComparison() {
    val util = batchTestUtil()
    val table =
      util.addTableSource[(Byte, Short, Int, Long, Float, Double)]("Table", 'a, 'b, 'c, 'd, 'e, 'f)
  ...
  def testCasting() {
    val util = batchTestUtil()
    val table = util.addTableSource[(Int, Double, Long, Boolean)]("Table", '_1, '_2, '_3, '_4)
  ...
  def testCastFromString() {
    val util = batchTestUtil()
    val table = util.addTableSource[(String, String, String)]("Table", '_1, '_2, '_3)
