...
class FiniteTestSource(elements: Iterable[Row], watermarkGenerator: Row => Long)
  ...
  override def run(ctx: SourceFunction.SourceContext[Row]): Unit = {
    ...
    lock.synchronized {
      for (t <- elements) {
        ctx.collect(t)
        ctx.emitWatermark(WatermarkUtils.createWatermarkEventFromTimestamp(watermarkGenerator(t)))
      }
    }

    ctx.emitWatermark(WatermarkUtils.createWatermarkEventFromTimestamp(Long.MaxValue))
