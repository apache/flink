...
public class PathResolutionTest {
	...
	public void testStreamSqlPathResolution() {
		...
		testSpec.getDefaultDatabasePath()
			.ifPresent(dbPath -> tEnv.useDatabase(dbPath.getCatalogName(), dbPath.getDatabaseName()));

		util.verifyJavaSql(
			String.format("SELECT * FROM %s", testSpec.getSqlPathToLookup()),
			String.format(
	...
	static class TestSpec {
		...
		public TestSpec tableApiLookupPath(String... path) {
			this.tableApiLookupPath = Arrays.asList(path);
			return this;
		}
		...
		public TestSpec expectPath(String... expectedPath) {
			...
			this.expectedPath = Arrays.asList(expectedPath);
			return this;
		}
		...
		public Optional<DatabasePath> getDefaultDatabasePath() {
			if (defaultDatabase != null && defaultCatalog != null) {
				return Optional.of(new DatabasePath(defaultCatalog, defaultDatabase));
			} else {
		...
		public String toString() {
			...
			stringBuilder.append(String.format("%s=[%s]", label, String.join(", ", properties)));
