...
public class OverAggregateTestPrograms {
    ...
    /** Regression test for FLINK-25802: RANGE OVER with TIMESTAMP(3) ORDER BY failed to compile. */
    public static final TableTestProgram OVER_AGGREGATE_RANGE_TIMESTAMP =
            TableTestProgram.of(
                            "over-aggregate-batch-range-timestamp",
                            "RANGE OVER with TIMESTAMP(3) ORDER BY")
                    .setupTableSource(
                            SourceTestStep.newBuilder("timestamp_range_source")
                                    .addSchema("ts TIMESTAMP(3)", "val INT")
                                    .producedValues(
                                            Row.of(LocalDateTime.of(2021, 1, 1, 0, 0, 0), 1),
                                            Row.of(LocalDateTime.of(2021, 1, 1, 0, 0, 5), 2),
                                            Row.of(LocalDateTime.of(2021, 1, 1, 0, 0, 12), 3))
                                    .build())
                    .setupTableSink(
