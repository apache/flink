...
class SubQueryValuesTest extends TableTestBase {
    ...
    void testBatch(String predicate) {
        javaBatchTestUtil().verifyExecPlan(query(predicate));
    }

    @ParameterizedTest(name = "{0}")
    @ValueSource(strings = {"EXISTS", "NOT EXISTS"})
    void testStreaming(String predicate) {
        javaStreamTestUtil().verifyExecPlan(query(predicate));
    }

    @ParameterizedTest(name = "{0}")
    @ValueSource(strings = {"EXISTS", "NOT EXISTS"})
    void testBatchWithProjectAndFilter(String predicate) {
        javaBatchTestUtil().verifyExecPlan(queryWithProjectAndFilter(predicate));
    }

    @ParameterizedTest(name = "{0}")
    @ValueSource(strings = {"EXISTS", "NOT EXISTS"})
    void testStreamingWithProjectAndFilter(String predicate) {
        javaStreamTestUtil().verifyExecPlan(queryWithProjectAndFilter(predicate));
    }
