 ...
 * class tests `accumulate` method without order but with ignore null argument.
 */
public abstract class FirstLastValueAggFunctionWithoutOrderWithIgnoreNullTestBase<T, ACC>
    ...
    protected ACC accumulateValues(List<T> values)
        ...
        for (int i = 0; i < values.size(); ++i) {
            accumulateFunc.invoke(aggregator, accumulator, values.get(i), false);
        }
        return accumulator;
    }
    ...
    protected void retractValues(ACC accumulator, List<T> values)
        ...
        for (int i = 0; i < values.size(); ++i) {
            retractFunc.invoke(aggregator, accumulator, values.get(i), false);
        }
    }
}
