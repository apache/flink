 ...
 * class tests `accumulate` method with order and respect nulls.
 */
public abstract class FirstLastValueRespectNullsAggFunctionWithOrderTestBase<T, ACC>
    ...
    protected ACC accumulateValues(List<T> values, List<Long> orders)
        ...
        for (int i = 0; i < values.size(); ++i) {
            accumulateFunc.invoke(aggregator, accumulator, values.get(i), orders.get(i), false);
        }
        return accumulator;
    }
    ...
    protected void retractValues(ACC accumulator, List<T> values, List<Long> orders)
        ...
        for (int i = 0; i < values.size(); ++i) {
            retractFunc.invoke(aggregator, accumulator, values.get(i), orders.get(i), false);
        }
    }
}
