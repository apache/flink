...
/** Base test class for ML_PREDICT table function. */
public abstract class MLPredictTableFunctionTestBase extends TableTestBase {
    ...
    public void testTooFewArguments() {
        ...
        assertThatThrownBy(() -> util.verifyRelPlan(sql))
                .hasMessageContaining("No match found for function signature ML_PREDICT");
    }
    ...
    public void testNonExistModel() {
        String sql =
                "SELECT *\n"
                        + "FROM TABLE(ML_PREDICT(TABLE MyTable, MODEL NonExistModel, DESCRIPTOR(a, b)))";
        assertThatThrownBy(() -> util.verifyRelPlan(sql))
    ...
    public void testNonExistColumn() {
        ...
        if (util.isBounded()) {
            assertThatThrownBy(() -> util.verifyRelPlan(sql))
                    .isInstanceOf(ValidationException.class)
                    .hasRootCauseMessage(
                            "Descriptor column 'no_col' not found in table columns. Available columns: a, b, c, d.");
        } else {
            assertThatThrownBy(() -> util.verifyRelPlan(sql))
                    .isInstanceOf(ValidationException.class)
                    .hasRootCauseMessage(
    ...
    public void testInputTableIsCdcStream() {
        if (util.isBounded()) {
            // CDC table is unbounded stream, skip the bounded test
            return;
        }
        util.tableEnv()
                .executeSql(
    ...
    protected static Stream<Arguments> compatibleTypeProvider() {
        return Stream.of(
    ...
    protected static Stream<Arguments> incompatibleTypeProvider() {
        return Stream.of(
