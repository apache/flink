 ...
 * class tests `accumulate` method without order and respect nulls.
 */
public abstract class FirstLastValueRespectNullsAggFunctionWithoutOrderTestBase<T, ACC>
    ...
    protected ACC accumulateValues(List<T> values)
        ...
        for (T value : values) {
            accumulateFunc.invoke(aggregator, accumulator, value, false);
        }
        return accumulator;
    }
    ...
    protected void retractValues(ACC accumulator, List<T> values)
        ...
        for (T value : values) {
            retractFunc.invoke(aggregator, accumulator, value, false);
        }
    }
}
