...
package org.apache.flink.table.planner.runtime.batch.jsonplan;
...
public class CalcAndAggJsonPlanITCase extends JsonPlanTestBase {

    @Override
    protected void setup() throws Exception {
        tableEnv = TableEnvironment.create(EnvironmentSettings.inBatchMode());
        tableEnv.getConfig().set(ExecutionConfigOptions.TABLE_EXEC_DISABLED_OPERATORS, "");
    }
    ...
    @Test
    public void testGbAggSortAgg() throws Exception {
        tableEnv.getConfig().set(ExecutionConfigOptions.TABLE_EXEC_DISABLED_OPERATORS, "HashAgg");

        List<String> data = Arrays.asList("1,1,hi", "2,1,hi", "3,2,hi");
        createTestCsvSourceTable("MyTable", data, "a bigint", "b int not null", "c varchar");
        File sinkPath = createTestCsvSinkTable("MySink", "c varchar", "a bigint");
        ...
        assertResult(Collections.singletonList("hi,6"), sinkPath);
    }

    @Test
    public void testGbAggHashAgg() throws Exception {
        tableEnv.getConfig().set(ExecutionConfigOptions.TABLE_EXEC_DISABLED_OPERATORS, "SortAgg");
