...
public class FileSystemTableSourceWithPartitionsITCase extends BatchTestBase {
    ...
    private void writePartitionedTestFiles(Integer partitionCount) throws IOException {
        ...
        for (Row row : TEST_DATA) {
            ...
            Files.write(
                    Paths.get(TEMPORARY_FOLDER.newFolder(partitionPath).getPath(), "part0.csv"),
    ...
    public void testPartitions(Integer partitionCount) throws IOException {
        ...
        assertThat(actual).containsExactlyInAnyOrder(Row.of(1, 4, 7, 10));
        actual = executeAndCollectResults("SELECT * FROM " + TABLE_NAME + " WHERE f0=2 AND f1=5;");
        assertThat(actual).containsExactlyInAnyOrder(Row.of(2, 5, 8, 11));
    ...
    @ParameterizedTest(name = "Partition count: {0}")
    @ValueSource(ints = {1, 2, 3, 4})
    public void testPartitionsWithMetadataFields(Integer partitionCount) throws IOException {
        ...
        List<Row> actual = executeAndCollectResults("SELECT * FROM " + TABLE_NAME + " WHERE f0=1;");
        assertThat(actual).containsExactlyInAnyOrder(Row.of(1, 4, 7, 10, "part0.csv", 1));
    }
}
