...
class BuiltInFunctionsITCase extends BatchTestBase {
  ...
  @Test
  def testLogicalOperateFunction(): Unit = {
    checkResult(
  ...
  @Test
  def testConditionFunction(): Unit = {
    checkResult(
      "SELECT b," +
  ...
  def testStringOperateFunction(): Unit = {
    checkResult("SELECT b," +
        ...
        "LPAD('ab', 5, 'cd'), LPAD('ab', -8, 'cde'), LPAD('ab', 5, '')," +
      ...
      Seq(row(3, "abcd", 3, 3, "ABC2", "abc3", 2, 0,
        ...
        "abdef", "a|b|def", "cdcab", null, null, "abcde", null, "abcdabcdabcd", "", "dcba",
        "puorG ababilA", 2987074, 0, "e2fc714c4727ee9395f324cd2e7f331f",
        "d41d8cd98f00b204e9800998ecf8427e", "abcd")))
  }

  @Test
  def testStringOperateFunction2(): Unit = {
    checkResult("SELECT b," +
        ...
        "JSONVALUE('[10, 20, [30, 40]]', '$[2][*]')," +
        "JSONVALUE('[10, 20, [30, [40, 50, 60]]]', '$[2][*][1][*]')," +
        "JSONVALUE('{xx]', '$[2][*]')," +
  ...
  @Test
  def testStringOperateFunction3(): Unit = {
    checkResult("SELECT " +
  ...
  @Test
  def testStringOperateFunction4(): Unit = {
    checkResult("SELECT " +
  ...
  @Test
  def testStringOperateFunction5(): Unit = {
    checkResult("SELECT " +
        "locate('a', 'abcdabcd', 3), " +
  ...
  @Test
  def testStringOperateFunction6(): Unit = {
    checkResult("SELECT length(uuid()) FROM testTable WHERE a = TRUE",
      Seq(row(36)))
  }

  @Test
  def testStringOperateFunction7(): Unit = {
    checkResult("SELECT " +
        "ascii('val_238'), " +
        "ascii('val_239'), " +
  ...
  @Test
  def testStringOperateFunction8(): Unit = {
  ...
  @Test
  def testStringOperateFunction9(): Unit = {
    checkResult("SELECT " +
        "instr('val_238', '_'), " +
        "instr('val_239', '_'), " +
  ...
  @Test
  def testTimeOperateFunction(): Unit = {
    checkResult("SELECT b," +
