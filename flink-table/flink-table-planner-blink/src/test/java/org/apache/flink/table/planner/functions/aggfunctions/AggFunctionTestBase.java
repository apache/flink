    ...
    protected <E> void validateResult(E expected, E result, TypeInformation<?> typeInfo) {
        if (expected instanceof BigDecimal && result instanceof BigDecimal) {
            ...
            assertThat(
                    (RawValueData) result,
                    equivalent(
                            (RawValueData<?>) expected, new RawValueDataSerializer<>(serializer)));
        } else if (expected instanceof GenericRowData && result instanceof GenericRowData) {
            validateGenericRow(
                    (GenericRowData) expected, (GenericRowData) result, (RowDataTypeInfo) typeInfo);
        } else {
            assertEquals(expected, result);
        }
    }

    private void validateGenericRow(
            GenericRowData expected, GenericRowData result, RowDataTypeInfo typeInfo) {
