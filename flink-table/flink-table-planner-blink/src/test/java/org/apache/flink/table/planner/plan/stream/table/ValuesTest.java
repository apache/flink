...
public class ValuesTest extends TableTestBase {
	...
	public void testValuesWithComplexNesting() {
		...
		Table t = util.getTableEnv().fromValues(
			DataTypes.ROW(
				...
				DataTypes.FIELD("row",
					DataTypes.ROW(
						DataTypes.FIELD("string", DataTypes.CHAR(5)),
						DataTypes.FIELD("decimal", DataTypes.DECIMAL(10, 2)),
						DataTypes.FIELD("nestedRow", DataTypes.ROW(
							DataTypes.FIELD("time", DataTypes.TIME(4))
						))
					)
				),
	...
	public void testNoCommonType() {
		...
		thrown().expectMessage("Types in fromValues(...) must have a common super type. Could not" +
			" find a common type for all rows at column 1.");
		JavaStreamTableTestUtil util = javaStreamTestUtil();
		util.getTableEnv().fromValues(
			row("ABC", 1L),
	...
	public void testCannotCast() {
		thrown().expect(ValidationException.class);
		thrown().expectMessage("Could not cast the value of the 0 column: [ 4 ] of a row: [ 4 ]" +
