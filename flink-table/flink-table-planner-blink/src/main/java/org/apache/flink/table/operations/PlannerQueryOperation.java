...
public class PlannerQueryOperation implements QueryOperation {
	...
	public PlannerQueryOperation(RelNode calciteTree) {
		...
		TypeInformation[] fieldTypes = FlinkTypeFactory.toLogicalRowType(rowType).getChildren().stream().map(
				TypeInfoLogicalTypeConverter::fromLogicalTypeToTypeInfo).toArray(TypeInformation[]::new);
