...
public class ExecGraphGenerator {
	...
	private ExecNode<?> generate(FlinkPhysicalRel rel) {
		...
		if (rel instanceof LegacyExecNodeBase) {
			...
			execNode = createBatchExecExchange((BatchPhysicalExchange) rel, inputNodes.get(0));
		} else if (rel instanceof StreamPhysicalExchange) {
			execNode = createStreamExecExchange((StreamPhysicalExchange) rel, inputNodes.get(0));
		} else {
	...
	private BatchExecExchange createBatchExecExchange(BatchPhysicalExchange exchange, ExecNode<?> inputNode) {
	...
	private StreamExecExchange createStreamExecExchange(StreamPhysicalExchange exchange, ExecNode<?> inputNode) {
		final RequiredShuffle requiredShuffle = getRequiredShuffle(exchange.getDistribution());
