...
abstract class FirstLastValueWithRetractAggFunctionBase<T>
        extends InternalAggregateFunction<
                T,
                FirstLastValueWithRetractAggFunctionBase.FirstLastValueWithRetractAccumulator<T>> {
    ...
    public DataType getAccumulatorDataType() {
        return DataTypes.STRUCTURED(
                ...
                DataTypes.FIELD("value", valueDataType.nullable()),
                DataTypes.FIELD("order", DataTypes.BIGINT()),
                DataTypes.FIELD(
                        "valueToOrderMap",
    ...
    public static class FirstLastValueWithRetractAccumulator<T> {
        ...
        public boolean equals(Object o) {
            ...
            FirstLastValueWithRetractAccumulator<?> that =
                    (FirstLastValueWithRetractAccumulator<?>) o;
    ...
    @SuppressWarnings("unchecked")
    public void retract(FirstLastValueWithRetractAccumulator<T> acc, Object value)
            throws Exception {
        if (value != null) {
            T v = (T) value;
            List<Long> orderList = acc.valueToOrderMap.get(v);
    ...
    @SuppressWarnings("unchecked")
    public void retract(FirstLastValueWithRetractAccumulator<T> acc, Object value, Long order)
            throws Exception {
        if (value != null) {
            T v = (T) value;
    ...
    public void merge(
        ...
        for (FirstLastValueWithRetractAccumulator<T> otherAcc : accIt) {
            if (acc.order == null
                    || (otherAcc.order != null && compare.apply(acc.order, otherAcc.order))) {
    ...
    private void mergeOrderToValueMap(
            FirstLastValueWithRetractAccumulator<T> acc,
            FirstLastValueWithRetractAccumulator<T> otherAcc)
            throws Exception {
