 ...
 * - Unsupported keys in SEMI/ANTI Join condition are from both Join's left and Join's right
...
public class FlinkSemiAntiJoinJoinTransposeRule extends RelOptRule {
	...
	private FlinkSemiAntiJoinJoinTransposeRule() {
		super(operand(LogicalJoin.class,
				some(operand(LogicalJoin.class, any()))));
	}
	...
	public void onMatch(RelOptRuleCall call) {
		...
		List<RelDataTypeField> joinFields =
				semiAntiJoin.getRowType().getFieldList();
		...
		if (nKeysFromX > 0) {
			...
			newSemiAntiJoinFilter =
					semiAntiJoin.getCondition().accept(
