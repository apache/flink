 ...
 * {@link UnresolvedFieldReferenceExpression}, and make sure the name of your attributes are unique within
...
public abstract class DeclarativeAggregateFunction extends UserDefinedFunction {
	...
	private Set<String> getAggBufferNames() {
		if (aggBufferNamesCache == null) {
			aggBufferNamesCache = Arrays.stream(aggBufferAttributes())
				.map(UnresolvedFieldReferenceExpression::getName)
	 ...
	 */
	public final UnresolvedFieldReferenceExpression[] operands() {
		int operandCount = operandCount();
		Preconditions.checkState(operandCount >= 0, "inputCount must be greater than or equal to 0.");
		UnresolvedFieldReferenceExpression[] ret = new UnresolvedFieldReferenceExpression[operandCount];
		for (int i = 0; i < operandCount; i++) {
			String name = String.valueOf(i);
			validateOperandName(name);
			ret[i] = new UnresolvedFieldReferenceExpression(name);
		}
		return ret;
	}
	 ...
	 */
	public final UnresolvedFieldReferenceExpression operand(int i) {
		String name = String.valueOf(i);
		if (getAggBufferNames().contains(name)) {
			throw new IllegalStateException(
				String.format("Agg buffer name(%s) should not same to operands.", name));
	 ...
	 */
	public final UnresolvedFieldReferenceExpression mergeOperand(UnresolvedFieldReferenceExpression aggBuffer) {
	 ...
	 */
	public final UnresolvedFieldReferenceExpression[] mergeOperands() {
		UnresolvedFieldReferenceExpression[] aggBuffers = aggBufferAttributes();
		UnresolvedFieldReferenceExpression[] ret = new UnresolvedFieldReferenceExpression[aggBuffers.length];
