...
public class MultipleInputNodeCreationProcessor implements DAGProcessor {
        ...
        if (!isStreaming) {
            ...
            InputPriorityConflictResolver resolver =
                    new InputPriorityConflictResolver(
                            roots, ExecEdge.DamBehavior.BLOCKING, ShuffleMode.PIPELINED);
                            ...
                            .anyMatch(
                                    inputWrapper ->
                                            inputWrapper.execNode instanceof Exchange
                                                    && ((Exchange) inputWrapper.execNode)
    ...
    static boolean isChainableSource(ExecNode<?, ?> node, DAGProcessContext context) {
        if (node instanceof BatchExecBoundedStreamScan) {
            ...
            return scan.boundedStreamTable().dataStream().getTransformation()
                    instanceof SourceTransformation;
        } else if (node instanceof StreamExecDataStreamScan) {
            StreamExecDataStreamScan scan = (StreamExecDataStreamScan) node;
            return scan.dataStreamTable().dataStream().getTransformation()
                    instanceof SourceTransformation;
            ...
            Transformation<?> transformation =
                    ((ExecNode) node)
                            .translateToPlan(Preconditions.checkNotNull(context).getPlanner());
    ...
    private ExecNode<?, ?> getMultipleInputNode(
        ...
        for (int i = 0; i < wrapper.inputs.size(); i++) {
            wrapper.execNode.replaceInputNode(
                    i, (ExecNode) getMultipleInputNode(wrapper.inputs.get(i), visitedMap));
        }
    ...
    private StreamExecMultipleInput createStreamMultipleInputNode(
        ...
        return new StreamExecMultipleInput(
                outputRel.getCluster(), outputRel.getTraitSet(), inputRels, outputRel);
    }
    ...
    private BatchExecMultipleInput createBatchMultipleInputNode(
        ...
        return new BatchExecMultipleInput(
                outputRel.getCluster(), outputRel.getTraitSet(), inputRels, outputRel, inputEdges);
    }
