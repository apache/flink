...
class StreamPhysicalTemporalJoin(
  ...
  override def translateToExecNode(): ExecNode[_] = {
    ...
    val remainingNonEquiJoinPredicates =
      temporalJoinConditionExtractor.apply(joinSpec.getNonEquiCondition.orElse(null)) match {
        case condition if condition != null => Optional.of(condition)
        case _ => Optional.empty().asInstanceOf[Optional[RexNode]]
      }
