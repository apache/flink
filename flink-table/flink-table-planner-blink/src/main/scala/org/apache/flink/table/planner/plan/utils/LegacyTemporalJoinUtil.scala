...
/**
  * Utilities for legacy temporal table join from temporal table function.
  */
object LegacyTemporalJoinUtil {
  ...
  /**
    * [[LEGACY_TEMPORAL_JOIN_CONDITION]] is a specific condition which correctly defines
    * references to rightTimeAttribute, rightPrimaryKeyExpression and leftTimeAttribute.
    * The condition is used to mark this is a temporal table function join.
    * Later rightTimeAttribute, rightPrimaryKeyExpression and leftTimeAttribute will be
    * extracted from the condition.
    */
  val LEGACY_TEMPORAL_JOIN_CONDITION = new SqlFunction(
    "__LEGACY_TEMPORAL_JOIN_CONDITION",
  ...
  def isRowtimeCall(call: RexCall): Boolean = {
    checkArgument(call.getOperator == LEGACY_TEMPORAL_JOIN_CONDITION)
  ...
  def makeRowTimeTemporalJoinConditionCall(
    ...
    rexBuilder.makeCall(
      LEGACY_TEMPORAL_JOIN_CONDITION,
      leftTimeAttribute,
      rightTimeAttribute,
  ...
  def containsTemporalJoinCondition(condition: RexNode): Boolean = {
    ...
    condition.accept(new RexVisitorImpl[Void](true) {
      override def visitCall(call: RexCall): Void = {
        if (call.getOperator != LEGACY_TEMPORAL_JOIN_CONDITION) {
