...
import org.apache.flink.table.api.{OptimizationConfigOptions, TableConfig, TableException}
import org.apache.flink.table.plan.nodes.calcite.Sink
import org.apache.flink.table.plan.nodes.logical.FlinkLogicalTableSourceScan
import org.apache.flink.table.plan.nodes.physical.PhysicalTableSourceScan
import org.apache.flink.table.plan.util.{DefaultRelShuttle, RelDigestUtil}
...
object SubplanReuser {
  ...
  def reuseDuplicatedSubplan(rels: Seq[RelNode], tableConfig: TableConfig): Seq[RelNode] = {
    if (!tableConfig.getConf.getBoolean(
      OptimizationConfigOptions.SQL_OPTIMIZER_REUSE_SUB_PLAN_ENABLED)) {
      return rels
    }
    val tableSourceReuseEnabled = tableConfig.getConf.getBoolean(
      OptimizationConfigOptions.SQL_OPTIMIZER_REUSE_TABLE_SOURCE_ENABLED)
