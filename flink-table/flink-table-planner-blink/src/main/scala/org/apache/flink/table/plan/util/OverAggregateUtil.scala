...
/**
  * Utility methods for over aggregate operator.
  */
object OverAggregateUtil {
  ...
  def orderingToString(
      ...
      orderFields: util.List[RelFieldCollation]): String = {
    val inFields = inputType.getFieldList
    val orderingString = orderFields.map {
  ...
  def createFlinkRelCollation(group: Group): RelCollation = {
    ...
    if (groupSet.nonEmpty || orderKeyIdxs.nonEmpty) {
      ...
      val fields = new util.ArrayList[RelFieldCollation]()
