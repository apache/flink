...
class FlinkBatchCost(
  ...
  /**
    ...
    *         sites
    */
  override def isInfinite: Boolean = (this eq FlinkBatchCost.Infinity) || hasInfinity
  ...
  override def equals(other: RelOptCost): Boolean = {
    (this eq other) ||
      (other.isInstanceOf[FlinkBatchCost] && isEqualTo(other.asInstanceOf[FlinkBatchCost]))
  }
  ...
  override def isEqWithEpsilon(other: RelOptCost): Boolean = {
    other match {
      case that: FlinkBatchCost => isEqualToWithEpsilon(that, RelOptUtil.EPSILON)
      case _ => false
    }
  }
  ...
  override def isLe(other: RelOptCost): Boolean = {
    isLessThanOrEqualTo(other.asInstanceOf[FlinkBatchCost])
  }
  ...
  /**
    ...
    */
  override def divideBy(cost: RelOptCost): Double = computeRatio(cost.asInstanceOf[FlinkBatchCost])
