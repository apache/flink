 ...
 *
 * <p>The harness runs no planner, so it reproduces the planner's inference protocol directly. It
 * probes the function up to three times with different required-mode hints, following the steps
 * documented on {@link ChangelogFunction#getChangelogMode}, to settle whether the output emits
 * updates, whether those updates carry UPDATE_BEFORE, and whether deletes are key-only. The result
 * is the mode the harness then enforces on the PTF's emitted rows.
 */
@Internal
