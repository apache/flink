...
public class ProcessTableFunctionTestHarness<OUT> implements AutoCloseable {
    ...
    /**
     * Table semantics as seen during type inference, where the changelog mode and upsert keys have
     * not yet been resolved. Both are therefore withheld (null mode, no upsert keys) so that {@link
     * TableSemantics#changelogMode()} reports empty here, matching the planner's {@code
    ...
    private Row stripNonKeyFieldsForKeyOnlyDelete(TableArgumentInfo tableArg, Row row) {
        ...
        Set<Integer> keyIndices =
                Arrays.stream(resolveColumnNamesToIndices(tableArg, keyColumns, "Partition"))
                        .boxed()
                        .collect(Collectors.toSet());

        // A new Row starts all-null, so copying back only the key columns leaves the rest stripped.
    ...
    public static class Builder<OUT> {
        ...
        public ProcessTableFunctionTestHarness<OUT> build() throws Exception {
            ...
            // Reject unknown argument names before resolving the output mode. Otherwise the
            // resolver's per-argument lookups would silently ignore them.
            List<TableArgumentInfo> tableArgInfos = ArgumentInfo.filterTableArguments(arguments);
            Set<String> configuredArgumentNames = new HashSet<>();
            configuredArgumentNames.addAll(tableArgumentChangelogModes.keySet());
            configuredArgumentNames.addAll(tableArgumentUpsertKeys.keySet());
            PtfChangelogModeValidator changelogValidator =
                    new PtfChangelogModeValidator(
                            tableArgInfos, configuredArgumentNames, onTimeColumnName);
