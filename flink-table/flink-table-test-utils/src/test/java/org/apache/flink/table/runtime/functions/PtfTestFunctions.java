...
class PtfTestFunctions {
    ...
    /**
     * Counts rows per partition key in keyed state and emits the running count as an upsert
     * changelog. A null value (as a key-only DELETE arrives once the harness has stripped its
     * non-key fields) evicts the key and emits a DELETE, but only when the key was present.
     * Exercises keyed state together with updating input and upsert output.
     */
    @DataTypeHint("ROW<key INT, count INT>")
    public static class KeyedCountingPTF extends ProcessTableFunction<Row>
            implements ChangelogFunction {
        /** Per-key running count held in harness-managed keyed state. */
        ...
        public void eval(
            ...
            if (value == null) {
                ...
                state.count++;
                collect(Row.ofKind(RowKind.UPDATE_AFTER, key, state.count));
            }
        }
