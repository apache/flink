...
public class LocalExecutorITCase extends TestLogger {
	...
	public void testStreamQueryExecutionSink() throws Exception {
		...
		try {
			...
			executeAndVerifySinkResult(executor, sessionId, statement1, () -> verifySinkResult(csvOutputPath));
			...
			final String statement2 = "insert Into TableSourceSink \n "
					...
					+ "FROM TableNumber1";
			executeAndVerifySinkResult(executor, sessionId, statement2, () -> verifySinkResult(csvOutputPath));
			...
			executeAndVerifySinkResult(executor, sessionId, statement3, () -> {
				TestBaseUtils.compareResultCollections(
						SimpleCatalogFactory.TABLE_CONTENTS.stream().map(Row::toString).collect(Collectors.toList()),
						TestCollectionTableFactory.getResult().stream().map(Row::toString).collect(Collectors.toList()),
	...
	private void executeAndVerifySinkResult(
			...
			String sessionId,
			String statement,
			RunnableWithException verifyResult) throws Exception {
