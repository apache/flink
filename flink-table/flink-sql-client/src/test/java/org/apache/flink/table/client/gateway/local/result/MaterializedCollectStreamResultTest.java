...
public class MaterializedCollectStreamResultTest {
    ...
    public void testSnapshot() throws Exception {
        ...
        try (TestMaterializedCollectStreamResult result =
            ...
            result.processRecord(converter.apply(Row.ofKind(RowKind.INSERT, "A", 1)));
            result.processRecord(converter.apply(Row.ofKind(RowKind.INSERT, "B", 1)));
            result.processRecord(converter.apply(Row.ofKind(RowKind.INSERT, "A", 1)));
            result.processRecord(converter.apply(Row.ofKind(RowKind.INSERT, "C", 2)));
            ...
            result.processRecord(converter.apply(Row.ofKind(RowKind.UPDATE_BEFORE, "A", 1)));
            ...
            result.processRecord(converter.apply(Row.ofKind(RowKind.UPDATE_BEFORE, "C", 2)));
            result.processRecord(converter.apply(Row.ofKind(RowKind.UPDATE_BEFORE, "A", 1)));
            result.processRecord(converter.apply(Row.ofKind(RowKind.UPDATE_AFTER, "D", 1)));
            ...
            assertEquals(
                    ...
                    result.retrievePage(1));
            assertEquals(
                    Collections.singletonList(converter.apply(Row.of("D", 1))),
                    result.retrievePage(2));
        }
    }
    ...
    public void testLimitedSnapshot() throws Exception {
        ...
        try (TestMaterializedCollectStreamResult result =
            ...
            result.processRecord(converter.apply(Row.ofKind(RowKind.INSERT, "D", 1)));
            result.processRecord(converter.apply(Row.ofKind(RowKind.INSERT, "A", 1)));
            result.processRecord(converter.apply(Row.ofKind(RowKind.INSERT, "B", 1)));
            result.processRecord(converter.apply(Row.ofKind(RowKind.INSERT, "A", 1)));
            ...
            assertEquals(
                    Arrays.asList(
                            null,
                            null,
                            converter.apply(Row.ofKind(RowKind.INSERT, "B", 1)),
                            converter.apply(
            ...
            assertEquals(
                    Collections.singletonList(converter.apply(Row.ofKind(RowKind.INSERT, "B", 1))),
                    result.retrievePage(1));
            assertEquals(
                    Collections.singletonList(converter.apply(Row.ofKind(RowKind.INSERT, "A", 1))),
                    result.retrievePage(2));

            result.processRecord(converter.apply(Row.ofKind(RowKind.INSERT, "C", 1)));

            assertEquals(
                    Arrays.asList(
                            converter.apply(Row.ofKind(RowKind.INSERT, "A", 1)),
                            converter.apply(
                                    Row.ofKind(
                                            RowKind.INSERT,
                                            "C",
                                            1))), // limit clean up has taken place
                    result.getMaterializedTable());

            result.processRecord(converter.apply(Row.ofKind(RowKind.DELETE, "A", 1)));
                    ...
                    Collections.singletonList(
                            converter.apply(
                                    Row.ofKind(
                                            RowKind.INSERT,
                                            "C",
                                            1))), // regular clean up has taken place
                    result.getMaterializedTable());
        }
    }
