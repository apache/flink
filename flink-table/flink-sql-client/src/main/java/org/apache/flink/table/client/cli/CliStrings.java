...
public final class CliStrings {
	...
	public static final AttributedString MESSAGE_HELP = new AttributedStringBuilder()
		...
		.append(formatCommand(SqlCommand.CREATE_VIEW, "Creates a virtual table from an SQL query. Syntax: 'CREATE VIEW <name> AS <query>;'"))
		...
		.append(formatCommand(SqlCommand.INSERT_INTO, "Inserts the results of an SQL SELECT query into a declared table sink."))
		...
		.append(formatCommand(SqlCommand.SELECT, "Executes an SQL SELECT query on the Flink cluster."))
		...
		.append(formatCommand(SqlCommand.SOURCE, "Reads an SQL SELECT query from a file and executes it on the Flink cluster."))
