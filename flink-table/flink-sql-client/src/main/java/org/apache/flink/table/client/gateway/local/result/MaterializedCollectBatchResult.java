...
public class MaterializedCollectBatchResult<C> extends BasicResult<C>
    ...
    public MaterializedCollectBatchResult(
            TableSchema tableSchema, ExecutionConfig config, ClassLoader classLoader) {
    ...
    public void startRetrieval(JobClient jobClient) {
        jobClient
                .getJobExecutionResult(classLoader)
                            ...
                            if (throwable != null) {
                                executionException.compareAndSet(
                                        ...
                                        new SqlExecutionException(
                                                "Error while retrieving result.", throwable));
                            }
                        });
    }
    ...
    public List<Row> retrievePage(int page) {
        synchronized (resultLock) {
            ...
            return resultTable.subList(
                    pageSize * (page - 1), Math.min(resultTable.size(), page * pageSize));
        }
    }
