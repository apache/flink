    ...
    public Optional<DataType> inferType(CallContext callContext) {
        ...
        if (argCount < 1 || argCount > 3) {
            throw new IllegalArgumentException(
                    "TO_TIMESTAMP_LTZ requires 1 to 3 arguments, but " + argCount + "arguments were provided.");
        }

        if (argCount == 1) {
            // TO_TIMESTAMP_LTZ(numeric) -- For epoch milliseconds
            // TO_TIMESTAMP_LTZ(string) -- Parse timestamp with default format
            return Optional.of(DataTypes.TIMESTAMP_LTZ(DEFAULT_PRECISION));
        } else if (argCount == 2) {
            // Legacy case: TO_TIMESTAMP_LTZ(timestamp, precision)
            ...
            // TO_TIMESTAMP_LTZ(string, format) -- Parse timestamp with custom format
            return Optional.of(DataTypes.TIMESTAMP_LTZ(DEFAULT_PRECISION));
        } else {
            // argCount == 3
