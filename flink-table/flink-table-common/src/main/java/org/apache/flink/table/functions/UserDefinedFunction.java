...
public abstract class UserDefinedFunction implements FunctionDefinition, Serializable {
	 ...
	 *
	 * <p>If the function has only one constructor without any parameter (a default constructor
	 * is automatically generated by the compiler if a class contains no constructor declarations),
	 * identifier is the class name, else identifier is the class name and instance's md5.
	 */
	public final String functionIdentifier() {
		final Class<?> clazz = getClass();
		if (clazz.getDeclaredConstructors().length == 1 &&
				clazz.getDeclaredConstructors()[0].getParameterCount() == 0) {
			return clazz.getName().replace('.', '$').concat("$");
		} else {
			final String md5 = EncodingUtils.hex(EncodingUtils.md5(EncodingUtils.encodeObjectToString(this)));
			return clazz.getName().replace('.', '$').concat("$").concat(md5);
		}
	}
