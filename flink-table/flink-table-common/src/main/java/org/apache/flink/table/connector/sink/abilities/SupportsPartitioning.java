 ...
 * clause or with a constant SELECT clause, the operation is considered an "insertion into a dynamic
 * partition". In the above example, the static partition part is {@code region='europe'} which will
 * be passed by the planner into {@link #applyStaticPartition(Map)}. The remaining values for
 * partition keys should be obtained from each individual record by the sink during runtime. In the
 * two examples above, the {@code month} field is the dynamic partition key.
 *
