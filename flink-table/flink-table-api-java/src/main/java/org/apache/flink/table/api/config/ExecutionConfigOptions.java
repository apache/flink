...
public class ExecutionConfigOptions {
	...
	private static final String MEMORY_DESCRIPTION = "Note: memory size is only a weight hint," +
			" it will affect the weight of memory that can be applied by a single operator in the task," +
			" the actual memory used depends on the running environment.";

	private static String memoryDescription(String description) {
		return description + " " + MEMORY_DESCRIPTION;
	}
	...
	public static final ConfigOption<String> TABLE_EXEC_RESOURCE_EXTERNAL_BUFFER_MEMORY =
		key("table.exec.resource.external-buffer-memory")
			.defaultValue("10 mb")
			.withDescription(memoryDescription(
