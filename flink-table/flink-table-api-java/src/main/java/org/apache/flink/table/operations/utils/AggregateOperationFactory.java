...
final class AggregateOperationFactory {
    ...
    private Stream<String> extractAggregateNames(Expression expression, List<String> groupNames) {
        if (isFunctionOfKind(expression, TABLE_AGGREGATE)) {
            ...
            return Arrays.stream(
                    FieldInfoUtils.getFieldNames(definition.getResultTypeInfo(), groupNames));
        } else {
    ...
    private static class TableAggFunctionCallResolver
        ...
        private void validateAlias(
                List<String> aliases, TableAggregateFunctionDefinition aggFunctionDefinition) {
            ...
            if (aliasesSize > 0 && aliasesSize != callArity) {
                throw new ValidationException(
                        String.format(
                                ...
                                aggFunctionDefinition, callArity, aliasesSize));
            }
        }
