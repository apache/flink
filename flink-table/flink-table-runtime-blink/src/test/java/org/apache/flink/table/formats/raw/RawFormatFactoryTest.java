...
public class RawFormatFactoryTest extends TestLogger {
	...
	public void testSeDeSchema() {
		...
		final RawFormatDeserializationSchema expectedDeser =
			new RawFormatDeserializationSchema(
				...
				"UTF-8",
				true);
		DeserializationSchema<RowData> actualDeser = createDeserializationSchema(SCHEMA, tableOptions);
	...
	public void testCharsetAndEndiannessOption() {
		final Map<String, String> tableOptions = getModifiedOptions(
			options -> {
	...
	public void testInvalidSchema() {
		TableSchema invalidSchema = TableSchema.builder()
			.field("f0", DataTypes.STRING())
			.field("f1", DataTypes.BIGINT())
			.build();
		String expectedError = "The 'raw' format only supports single physical column. " +
	...
	public void testInvalidCharset() {
		...
		try {
			...
			assertThat(
				e.getCause().getCause(),
				hasMessage(equalTo(expectedError)));
		}
	...
	public void testInvalidEndianness() {
		...
		String expectedError = "Unsupported endianness name: BIG_ENDIAN. " +
	...
	public void testInvalidFieldTypes() {
		try {
			...
			assertThat(
				e,
				hasMessage(equalTo("The 'raw' format doesn't supports 'TIMESTAMP(3)' as column type.")));
		}
		...
		try {
			...
			assertThat(
				e,
				hasMessage(equalTo("The 'raw' format doesn't supports 'MAP<INT, STRING>' as column type.")));
		}
	}
