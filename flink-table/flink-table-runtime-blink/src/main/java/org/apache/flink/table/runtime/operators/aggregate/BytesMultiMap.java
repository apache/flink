 ...
 * </pre>
 */
public class BytesMultiMap extends BytesMap<Iterator<RowData>> {
    private static final Logger LOG = LoggerFactory.getLogger(BytesMultiMap.class);
    ...
    public void append(LookupInfo<Iterator<RowData>> info, BinaryRowData value) throws IOException {
        try {
            if (info.found) {
                ...
                int pointerToAppended = recordArea.appendRecord(info, value);
                bucketSegments
    ...
    public KeyValueIterator<RowData, Iterator<RowData>> getEntryIterator() {
    ...
    private final class RecordArea implements BytesMap.RecordArea<Iterator<RowData>> {
        ...
        @Override
        public int appendRecord(LookupInfo<Iterator<RowData>> info, BinaryRowData value)
                throws IOException {
            int lastPosition = (int) keyOutView.getCurrentOffset();
            // write key to keyOutView
            int skip = keySerializer.serializeToPages(info.key, keyOutView);
        ...
        KeyValueIterator<RowData, Iterator<RowData>> entryIterator() {
            return new EntryIterator();
        }

        final class EntryIterator implements KeyValueIterator<RowData, Iterator<RowData>> {
            private int count;
