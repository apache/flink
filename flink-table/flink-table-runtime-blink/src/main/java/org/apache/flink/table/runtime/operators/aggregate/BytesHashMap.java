 ...
 *
 * <p>- Bucket area: this contains: pointer + hashcode. Bytes 0 to 8: a pointer to the record in the
 * record area Bytes 8 to 16: a holds key's full 32-bit hashcode
 *
 * <p>- Record area: this contains the actual data in linked list records. A BytesHashMap's record
 * has four parts: Bytes 0 to 4: len(k) Bytes 4 to 4 + len(k): key data Bytes 4 + len(k) to 8 +
 * len(k): len(v) Bytes 8 + len(k) to 8 + len(k) + len(v): value data
 *
...
public class BytesHashMap {
    ...
    public BinaryRowData append(LookupInfo info, BinaryRowData value) throws IOException {
        try {
            ...
            bucketSegments
                    ...
                    .putLong(info.bucketOffset, pointerToAppended);
            bucketSegments
                    .get(info.bucketSegmentIndex)
        ...
        for (MemorySegment memorySegment : bucketSegments) {
            for (int j = 0; j < numBucketsPerSegment; j++) {
                ...
                if (recordPointer != END_OF_LIST) {
                    ...
                    int step = STEP_INCREMENT;
                    long hashCode2 = 0;
                    while (newBucketSegments.get(bucketSegmentIndex).getLong(bucketOffset)
                            != END_OF_LIST) {
                        if (step == 1) {
                            hashCode2 = calcSecondHashCode(hashCode1);
                        }
    ...
    public static final class LookupInfo {
         ...
         * for that key. The same purpose with bucketSegmentIndex, bucketOffset.
         */
