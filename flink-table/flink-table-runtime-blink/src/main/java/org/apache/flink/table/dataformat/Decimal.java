...
public abstract class Decimal implements Comparable<Decimal> {
	...
	public int compareTo(Decimal that) {
		if ((this instanceof CompactDecimal) && (that instanceof CompactDecimal) && this.scale == that.scale) {
			return Long.compare(((CompactDecimal) this).longVal, ((CompactDecimal) that).longVal);
		}
	...
	public static Decimal fromBigDecimal(BigDecimal bd, int precision, int scale) {
		...
		return new LooseDecimal(precision, scale, bd);
	}

	public static Decimal zero(int precision, int scale) {
		if (precision <= MAX_COMPACT_PRECISION) {
			return new CompactDecimal(precision, scale, 0);
		} else {
			return LooseDecimal.fromBigDecimal(BigDecimal.ZERO, precision, scale);
		}
	}

	public abstract Decimal copy();

	public abstract byte[] toUnscaledBytes();
	...
	public static Decimal add(Decimal v1, Decimal v2, int precision, int scale) {
		if ((v1 instanceof CompactDecimal) && (v2 instanceof CompactDecimal) && v1.scale == v2.scale) {
			assert scale == v1.scale; // no need to rescale
			try {
				long ls = Math.addExact(((CompactDecimal) v1).longVal, ((CompactDecimal) v2).longVal); // checks overflow
				return new CompactDecimal(precision, scale, ls);
	...
	public static Decimal subtract(Decimal v1, Decimal v2, int precision, int scale) {
		if ((v1 instanceof CompactDecimal) && (v2 instanceof CompactDecimal) && v1.scale == v2.scale) {
			assert scale == v1.scale; // no need to rescale
			try {
				long ls = Math.subtractExact(((CompactDecimal) v1).longVal, ((CompactDecimal) v2).longVal); // checks overflow
				return new CompactDecimal(precision, scale, ls);
			} catch (ArithmeticException e) {
				// overflow, fall through
			}
	...
	public static Decimal sign(Decimal b0) {
		if (b0.isCompact()) {
			return new CompactDecimal(b0.precision, b0.scale, b0.signum() * POW10[b0.scale]);
		} else {
	...
	public static int compare(Decimal b1, long n2) {
		if (b1 instanceof LooseDecimal) {
			return ((LooseDecimal) b1).decimalVal.compareTo(BigDecimal.valueOf(n2));
		}
		if (b1.scale == 0) {
			return Long.compare(((CompactDecimal) b1).longVal, n2);
		}

		long i1 = ((CompactDecimal) b1).longVal / POW10[b1.scale];
		if (i1 == n2) {
			long l2 = n2 * POW10[b1.scale]; // won't overflow
			return Long.compare(((CompactDecimal) b1).longVal, l2);
		} else {
			return i1 > n2 ? +1 : -1;
		}
	}
