...
/**
 * Committer for {@link StreamingFileWriter}. This is the single (non-parallel) task. It collects
...
public class StreamingFileCommitter extends AbstractStreamOperator<Void>
        ...
        try (TableMetaStoreFactory.TableMetaStore metaStore =
            ...
            for (String partition : partitions) {
                ...
                PartitionCommitPolicy.Context context =
                        new PolicyContext(new ArrayList<>(partSpec.values()), path);
    ...
    private static class TaskTracker {
        ...
        private boolean add(long checkpointId, int task) {
            Set<Integer> tasks =
                    notifiedTasks.computeIfAbsent(checkpointId, (k) -> new HashSet<>());
            tasks.add(task);
