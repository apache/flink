...
import org.apache.flink.table.runtime.orderedmultisetstate.OrderedMultiSetState.RemovalResultType;
import org.apache.flink.table.runtime.orderedmultisetstate.OrderedMultiSetStateParameters;
...
public class SinkUpsertMaterializerV2 extends TableStreamOperator<RowData>
    ...
    private final OrderedMultiSetStateParameters stateParameters;
    ...
    private transient OrderedMultiSetState<RowData> orderedMultiSetState;
    private final boolean hasUpsertKey;

    public SinkUpsertMaterializerV2(
            boolean hasUpsertKey, OrderedMultiSetStateParameters stateParameters) {
    ...
    public void processElement(StreamRecord<RowData> element) throws Exception {
        ...
        switch (row.getRowKind()) {
            ...
            case UPDATE_BEFORE:
            case DELETE:
                Tuple3<RemovalResultType, Optional<RowData>, OrderedMultiSetState.SizeChangeInfo>
                        removalResult = orderedMultiSetState.remove(row);
                switch (removalResult.f0) {
                    case REMOVED_OTHER:
                        // do nothing;
                        break;
                    case NOTHING_REMOVED:
                        ...
                        break;
                    case ALL_REMOVED:
                        collect(removalResult.f1.get(), RowKind.DELETE);
                        break;
                    case REMOVED_LAST_ADDED:
                        collect(removalResult.f1.get(), RowKind.UPDATE_AFTER);
                        break;
                    default:
                        throw new IllegalArgumentException(
                                "Unexpected removal result type: " + removalResult.f0);
                }
        }
    }
    ...
    public static SinkUpsertMaterializerV2 create(
        ...
        return new SinkUpsertMaterializerV2(
                hasUpsertKey,
                new OrderedMultiSetStateParameters(
                        checkNotNull(
                                hasUpsertKey
