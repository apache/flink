...
public class LinkedMultiSetState implements OrderedMultiSetState<RowData> {
    ...
    // todo: replace tuple with class? that would make it easier to add more fields
    // in particular, lowestSqn is needed for iterator
    private final ValueState<Tuple2<Long, Long>> highestSqnStateAndSize;
    ...
    private LinkedMultiSetState(
            ...
            ValueState<Tuple2<Long, Long>> highestSqnStateAndSize,
        ...
        this.highestSqnStateAndSize = checkNotNull(highestSqnStateAndSize);
    ...
    public static OrderedMultiSetState<RowData> create(
            OrderedMultiSetStateParameters p, RuntimeContext ctx) {
        ...
        return new LinkedMultiSetState(
                ...
                keyHashFunction,
                p.keyExtractor,
                TimeSelector.getTimeDomain(p.stateSettings.getTtlTimeDomain()));
    }
    ...
    public SizeChangeInfo add(RowData row, long timestamp) throws Exception {
        ...
        final Tuple2<Long, Long> highSqnAndSize = highestSqnStateAndSize.value();
        ...
        highestSqnStateAndSize.update(Tuple2.of(newSqn, newSize));
        if (append) {
    ...
    public Tuple3<RemovalResultType, Optional<RowData>, SizeChangeInfo> remove(RowData row)
        ...
        final RowDataKey key = toKey(row);
        final Long rowSqn = rowToSqnState.get(key);
        final Tuple2<Long, Long> highSqnStateAndSize = highestSqnStateAndSize.value();
    ...
    public boolean isEmpty() throws IOException {
        return highestSqnStateAndSize.value() == null;
    }
