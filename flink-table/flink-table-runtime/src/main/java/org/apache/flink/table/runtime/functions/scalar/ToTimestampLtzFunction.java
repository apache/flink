 ...
 *   TO_TIMESTAMP_LTZ(numeric, precision) -> TIMESTAMP_LTZ(3)
 *   TO_TIMESTAMP_LTZ(string) -> TIMESTAMP_LTZ(3)
 *   TO_TIMESTAMP_LTZ(string, format) -> TIMESTAMP_LTZ(3)
 *   TO_TIMESTAMP_LTZ(string, format, timezone) -> TIMESTAMP_LTZ(3)
...
public class ToTimestampLtzFunction extends BuiltInScalarFunction {
    ...
    public TimestampData eval(long epoch, int precision) {
        return DateTimeUtils.toTimestampData(epoch, precision);
    }

    public TimestampData eval(double epoch, int precision) {
        return DateTimeUtils.toTimestampData(epoch, precision);
    }

    public TimestampData eval(DecimalData epoch, int precision) {
        return DateTimeUtils.toTimestampData(epoch, precision);
    }
    ...
    public TimestampData eval(StringData timestamp) {
        ...
        return parseTimestamp(timestamp.toString(), DEFAULT_FORMAT, UTC);
    }
    ...
    public TimestampData eval(StringData timestamp, StringData format) {
        ...
        return parseTimestamp(timestamp.toString(), format.toString(), UTC);
    }
