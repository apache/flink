...
public abstract class BaseAdaptiveJoinOperatorOptimizationStrategy
    ...
    protected void visitDownstreamAdaptiveJoinNode(
        ...
        Map<ImmutableStreamNode, List<ImmutableStreamEdge>> nodesWithOutEdges = new HashMap<>();
        for (Integer finishedStreamNodeId : finishedStreamNodeIds) {
            for (ImmutableStreamEdge streamEdge :
                ...
                if (context.getOperatorFactory(downstreamNode.getId()) instanceof AdaptiveJoin) {
                    nodesWithOutEdges
        ...
        for (ImmutableStreamNode joinNode : nodesWithOutEdges.keySet()) {
            tryOptimizeAdaptiveJoin(
                    operatorsFinished,
                    context,
                    joinNode,
                    nodesWithOutEdges.get(joinNode),
                    (AdaptiveJoin) context.getOperatorFactory(joinNode.getId()));
        }
    }
