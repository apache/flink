...
class UrlDecodeFunctionTest {
    ...
    @Test
    void testUrlDecodeRecursiveMaxDepthZero() {
        UrlDecodeFunction func = createFunction(true);
        // maxDepth <= 0 should use default max depth (10), so double-encoded string is fully
        // decoded
        StringData result =
                func.eval(StringData.fromString("https%253A%252F%252Fflink.apache.org%252F"), 0);
        assertThat(result).isNotNull();
        assertThat(result.toString()).isEqualTo("https://flink.apache.org/");
    }

    @Test
    void testUrlDecodeRecursiveMaxDepthNegative() {
        UrlDecodeFunction func = createFunction(true);
        // maxDepth < 0 should use default max depth (10)
        StringData result =
                func.eval(StringData.fromString("https%253A%252F%252Fflink.apache.org%252F"), -1);
        assertThat(result).isNotNull();
        assertThat(result.toString()).isEqualTo("https://flink.apache.org/");
    }
