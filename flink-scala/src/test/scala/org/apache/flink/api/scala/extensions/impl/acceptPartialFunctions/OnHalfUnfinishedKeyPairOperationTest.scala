...
class OnHalfUnfinishedKeyPairOperationTest extends AcceptPFTestBase {
  ...
  def testInnerJoinIsEqualToOnTuple(): Unit = {
    val test =
      tuples.join(tuples).whereClause { case (id, _) => id }.isEqualTo { case (id, _) => id }
    assert(
      test.javaSet.isInstanceOf[EquiJoin[_, _, _]],
  ...
  def testInnerJoinIsEqualToOnCaseClass(): Unit = {
    val test =
      caseObjects.join(caseObjects).whereClause { case KeyValuePair(id, _) => id }.isEqualTo {
  ...
  def testRightOuterJoinIsEqualToOnTuple(): Unit = {
    val test =
      tuples.rightOuterJoin(tuples).whereClause { case (id, _) => id }.isEqualTo {
        case (id, _) => id
      }
    assert(
      test.isInstanceOf[JoinFunctionAssigner[_, _]],
  ...
  def testRightOuterJoinIsEqualToOnCaseClass(): Unit = {
    val test =
      caseObjects
        .rightOuterJoin(caseObjects)
        .whereClause { case KeyValuePair(id, _) => id }
        .isEqualTo { case KeyValuePair(id, _) => id }
    assert(
      test.isInstanceOf[JoinFunctionAssigner[_, _]],
  ...
  def testLeftOuterJoinIsEqualToOnTuple(): Unit = {
    val test =
      tuples.leftOuterJoin(tuples).whereClause { case (id, _) => id }.isEqualTo {
        case (id, _) => id
      }
    assert(
      test.isInstanceOf[JoinFunctionAssigner[_, _]],
  ...
  def testFullOuterJoinIsEqualToOnTuple(): Unit = {
    val test =
      tuples.fullOuterJoin(tuples).whereClause { case (id, _) => id }.isEqualTo {
        case (id, _) => id
      }
    assert(
      test.isInstanceOf[JoinFunctionAssigner[_, _]],
  ...
  def testCoGroupIsEqualToOnTuple(): Unit = {
    val test =
      tuples.coGroup(tuples).whereClause { case (id, _) => id }.isEqualTo { case (id, _) => id }
    assert(
  ...
  def testCoGroupIsEqualToOnCaseClass(): Unit = {
    val test =
      caseObjects.coGroup(caseObjects).whereClause { case KeyValuePair(id, _) => id }.isEqualTo {
        case KeyValuePair(id, _) => id
      }
    assert(
      test.javaSet.isInstanceOf[CoGroupOperator[_, _, _]],
