...
class SelectByFunctionTest {
  ...
  def testMaxByComparison(): Unit = {
    ...
    try {
      Assertions.assertSame(bigger, maxByTuple.reduce(smaller, bigger), "SelectByMax must return bigger tuple")
      Assertions.assertSame(bigger, maxByTuple.reduce(bigger, smaller), "SelectByMax must return bigger tuple")
    } catch {
      case e: Exception =>
  ...
  def testMaxByComparisonSpecialCase1(): Unit = {
    ...
    try {
      Assertions.assertSame(specialCaseBigger, maxByTuple.reduce(specialCaseBigger, bigger), "SelectByMax must return the first given tuple")
      Assertions.assertSame(bigger, maxByTuple.reduce(bigger, specialCaseBigger), "SelectByMax must return the first given tuple")
    } catch {
      case e: Exception => Assertions.fail("No exception should be thrown " +
  ...
  def testMaxByComparisonSpecialCase2(): Unit = {
    ...
    val maxByTuple = new SelectByMaxFunction(tupleTypeInfo, a1)
    try {
      Assertions.assertSame(bigger, maxByTuple.reduce(specialCaseBigger, bigger), "SelectByMax must return bigger tuple")
      Assertions.assertSame(bigger, maxByTuple.reduce(bigger, specialCaseBigger), "SelectByMax must return bigger tuple")
    } catch {
      case e: Exception => Assertions.fail("No exception should be thrown" +
        " while comparing both tuples")
    }
  ...
  def testMaxByComparisonMustReturnATuple(): Unit = {
    ...
    try {
      Assertions.assertSame(bigger, maxByTuple.reduce(bigger, bigger), "SelectByMax must return bigger tuple")
      Assertions.assertSame(smaller, maxByTuple.reduce(smaller, smaller), "SelectByMax must return smaller tuple")
    } catch {
      case e: Exception => Assertions.fail("No exception should be thrown" +
        " while comparing both tuples")
    }
  ...
  def testMinByComparison(): Unit = {
    ...
    try {
      Assertions.assertSame(smaller, minByTuple.reduce(smaller, bigger), "SelectByMin must return smaller tuple")
      Assertions.assertSame(smaller, minByTuple.reduce(bigger, smaller), "SelectByMin must return smaller tuple")
    } catch {
      case e: Exception => Assertions.fail("No exception should be thrown " +
        "while comparing both tuples")
    }
  ...
  def testMinByComparisonSpecialCase1(): Unit = {
    ...
    try {
      Assertions.assertSame(specialCaseBigger, minByTuple.reduce(specialCaseBigger, bigger), "SelectByMin must return the first given tuple")
      Assertions.assertSame(bigger, minByTuple.reduce(bigger, specialCaseBigger), "SelectByMin must return the first given tuple")
    } catch {
      case e: Exception => Assertions.fail("No exception should be thrown " +
        "while comparing both tuples")
    }
  ...
  def testMinByComparisonSpecialCase2(): Unit = {
    ...
    try {
      Assertions.assertSame(smaller, minByTuple.reduce(specialCaseSmaller, smaller), "SelectByMin must return smaller tuple")
      Assertions.assertSame(smaller, minByTuple.reduce(smaller, specialCaseSmaller), "SelectByMin must return smaller tuple")
    } catch {
      case e: Exception => Assertions.fail("No exception should be thrown" +
        " while comparing both tuples")
    }
