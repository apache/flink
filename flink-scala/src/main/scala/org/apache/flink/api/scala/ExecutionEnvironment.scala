 ...
 * be created.
 */
class ExecutionEnvironment(javaEnvironment: JavaEnv) {
    ...
    def setParallelism(parallelism: Int): Unit = {
        javaEnvironment.setParallelism(parallelism)
    }
     ...
     */
    @deprecated
    def getDegreeOfParallelism = javaEnvironment.getParallelism

    /**
     * Returns the default parallelism for this execution environment. Note that this
     * value can be overridden by individual operations using [[DataSet.setParallelism]]
     */
    def getParallelism = javaEnvironment.getParallelism
    ...
    def setNumberOfExecutionRetries(numRetries: Int): Unit = {
        javaEnvironment.setNumberOfExecutionRetries(numRetries)
    }
     ...
     * should be used.
     */
    def getNumberOfExecutionRetries = javaEnvironment.getNumberOfExecutionRetries
    ...
    def registerTypeWithKryoSerializer(clazz: Class[_], serializer: Serializer[_]): Unit = {
        javaEnvironment.registerTypeWithKryoSerializer(clazz, serializer)
    }
    ...
    def addDefaultKryoSerializer(clazz: Class[_], serializer: Class[_ <: Serializer[_]]) {
        javaEnvironment.addDefaultKryoSerializer(clazz, serializer)
    }
    ...
    def registerType(typeClass: Class[_]) {
        javaEnvironment.registerType(typeClass)
    }
    ...
    def readTextFile(filePath: String, charsetName: String = "UTF-8"): DataSet[String] = {
        ...
        val source = new DataSource[String](javaEnvironment, format, BasicTypeInfo.STRING_TYPE_INFO,
            getCallLocationName())
        wrap(source)
    }
    ...
    def readCsvFile[T: ClassTag : TypeInformation](
        ...
        wrap(new DataSource[T](javaEnvironment, inputFormat, typeInfo, getCallLocationName()))
    }
    ...
    private def createInput[T: ClassTag](
        ...
        wrap(new DataSource[T](javaEnvironment, inputFormat, producedType, getCallLocationName()))
    }
    ...
    def registerCachedFile(filePath: String, name: String, executable: Boolean = false): Unit = {
        javaEnvironment.registerCachedFile(filePath, name, executable)
    }
    ...
    def execute(jobName: String): JobExecutionResult = {
        javaEnvironment.execute(jobName)
    }
    ...
    def getExecutionPlan() = {
        javaEnvironment.getExecutionPlan
    }
    ...
    def createProgramPlan(jobName: String = "") = {
        if (jobName.isEmpty) {
            javaEnvironment.createProgramPlan()
        } else {
            javaEnvironment.createProgramPlan(jobName)
        }
    }
}
