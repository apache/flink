...
import org.apache.flink.api.common.typeinfo.{PrimitiveArrayTypeInfo, BasicArrayTypeInfo, 
...
private[flink] trait TypeInformationGen[C <: Context] {
  ...
  def mkTypeInfo[T: c.WeakTypeTag](desc: UDTDescriptor): c.Expr[TypeInformation[T]] = desc match {
    ...
    case l : ListDescriptor if l.tpe <:< typeOf[Array[_]] => mkArrayTypeInfo(l)

    case l : ListDescriptor => mkCollectionTypeInfo(l)
  ...
  def mkCollectionTypeInfo[T: c.WeakTypeTag](desc: ListDescriptor): c.Expr[TypeInformation[T]] = {
    ...
    val result = q"""
      ...
      import org.apache.flink.api.scala.typeutils.ScalaCollectionTypeInfo
      import org.apache.flink.api.scala.typeutils.ScalaCollectionSerializer

      val elementTpe = $elementTypeInfo
      new ScalaCollectionTypeInfo($collectionClass, elementTpe) {
        def createSerializer() = {
          new ScalaCollectionSerializer[${desc.tpe}, ${desc.elem.tpe}](
  ...
  def mkArrayTypeInfo[T: c.WeakTypeTag](desc: ListDescriptor): c.Expr[TypeInformation[T]] = {
