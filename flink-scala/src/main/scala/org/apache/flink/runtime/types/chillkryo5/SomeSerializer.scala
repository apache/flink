...
class SomeSerializer[T] extends Serializer[Some[T]] {
  override def write(kryo: Kryo, output: Output, item: Some[T]): Unit = {
    kryo.writeClassAndObject(output, item.get)
  }

  override def read(kryo: Kryo, input: Input, cls: Class[_ <: Some[T]]): Some[T] = {
