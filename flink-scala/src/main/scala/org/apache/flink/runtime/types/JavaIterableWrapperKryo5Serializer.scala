 ...
 */
private class JavaIterableWrapperKryo5Serializer[T] extends Serializer[JIterable[T]] {

  import JavaIterableWrapperKryo5Serializer._

  override def write(kryo: Kryo, out: Output, obj: JIterable[T]): Unit = {
  ...
  override def read(kryo: Kryo, input: Input, clz: Class[_ <: JIterable[T]]): JIterable[T] = {
    kryo.readClassAndObject(input) match {
      case scalaIterable: Iterable[T] =>
        scala.collection.JavaConversions.asJavaIterable(scalaIterable)
      case javaIterable: JIterable[T] =>
        javaIterable
    }
  }
}

private object JavaIterableWrapperKryo5Serializer {
