...
import org.apache.flink.api.java.typeutils.runtime.kryo5.FlinkChillPackageRegistrar
import org.apache.flink.runtime.types.chillkryo5.{ClassManifestSerializer, EnumerationSerializer, IKryo5Registrar, LeftSerializer, ManifestSerializer, RegexSerializer, RichKryo, RightSerializer, ScalaBitSetSerializer, ScalaTupleKryo5Serialization, SingletonSerializer, SomeSerializer, SortedMapSerializer, SortedSetSerializer, SymbolSerializer, WrappedArraySerializer}
 ...
 */
class EmptyFlinkScalaKryo5Instantiator {
  def newKryo = {
    val k = new Kryo()
    k.setRegistrationRequired(false)
    // 2023-05-13: Flink Kryo 2 was using StdInstantiatorStrategy
    // Tests such as AvroRecordInputFormatTest.testDeserializeToGenericType were failing
    // with that.
    // It is better to use DefaultInstantiatorStrategy. That's what Flink
    // code in KryoSerializer.java uses when this class is not  available.
...
object FlinkScalaKryo5Instantiator extends Serializable {
...
class FlinkScalaKryo5Instantiator extends EmptyFlinkScalaKryo5Instantiator {
  override def newKryo = {
    val k = super.newKryo
    val reg = new AllScalaKryo5Registrar
    reg(k)
    k
  }
}

class ScalaCollectionsKryo5Registrar extends IKryo5Registrar {
  def apply(newK: Kryo) {
    ...
    val newRichK = new RichKryo(newK)
    /*
...
class JavaWrapperCollectionKryo5Registrar extends IKryo5Registrar {
  def apply(newK: Kryo) {
    val newRichK = new RichKryo(newK)
    newK.register(
      JavaIterableWrapperKryo5Serializer.wrapperClass,
      new JavaIterableWrapperKryo5Serializer)
  }
}

/** Registers all the scala (and java) serializers we have */
class AllScalaKryo5Registrar extends IKryo5Registrar {
  def apply(k: Kryo) {
    val col = new ScalaCollectionsKryo5Registrar
    col(k)

    val jcol = new JavaWrapperCollectionKryo5Registrar
    jcol(k)

    val richKryo = new RichKryo(k)

    // Register all 22 tuple serializers and specialized serializers
    ScalaTupleKryo5Serialization.register(k)
    richKryo
      .forClass[Symbol](new SymbolSerializer)
