...
readFromConfig() {
    ...
    local value=$(echo "$configuration" | awk -F': ' -v k="$key" 'BEGIN{value=""} $1==k{value=$2} END{print value}')
...
YAML_CONF=$(parseConfigurationAndExportLogs "${FLINK_CONF_DIR}" "${FLINK_BIN_DIR}" ${FLINK_LIB_DIR})
