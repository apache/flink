...
TMSlaves() {
    ...
    if [ ${SLAVES_ALL_LOCALHOST} = true ] ; then
        ...
        if [[ $? -ne 0 ]]; then
            for slave in ${SLAVES[@]}; do
                ssh -n $FLINK_SSH_OPTS $slave -- "export FLINK_CONF_DIR=\"${FLINK_CONF_DIR}\"; nohup /bin/bash -l \"${FLINK_BIN_DIR}/taskmanager.sh\" \"${CMD}\" \"${FLIP6}\" &"
            done
        else
            PDSH_SSH_ARGS="" PDSH_SSH_ARGS_APPEND=$FLINK_SSH_OPTS pdsh -w $(IFS=, ; echo "${SLAVES[*]}") \
                "export FLINK_CONF_DIR=\"${FLINK_CONF_DIR}\"; nohup /bin/bash -l \"${FLINK_BIN_DIR}/taskmanager.sh\" \"${CMD}\" \"${FLIP6}\""
        fi
    fi
}
