...
if [[ $STARTSTOP == "start" ]]; then
    ...
    if [[ ! ${FLINK_TM_MEM_SIZE} =~ ${IS_NUMBER} ]] || [[ "${FLINK_TM_MEM_SIZE}" -le "0" ]]; then
        echo "[ERROR] Configured TaskManager JVM heap size is not a valid value. Please set '${KEY_TASKM_MEM_SIZE}' in ${FLINK_CONF_FILE}."
        exit 1
    fi

    if [[ ! ${FLINK_TM_NET_MEM_SIZE} =~ ${IS_NUMBER} ]] || [[ "${FLINK_TM_NET_MEM_SIZE}" -le "0" ]]; then
        echo "[ERROR] Configured TaskManager Network memory size ('$FLINK_TM_NET_MEM_SIZE') is not a valid value. Please set '${KEY_TASKM_MEM_NETWORK_SIZE}' in ${FLINK_CONF_FILE}."
        exit 1
    fi

    if [[ "${FLINK_TM_NET_MEM_SIZE}" -gt "$((FLINK_TM_MEM_SIZE - FLINK_TM_MEM_MANAGED_SIZE))" ]]; then
        echo "[ERROR] Configured TaskManager Network memory size ('${KEY_TASKM_MEM_SIZE}') is too large. Consider increasing '${KEY_TASKM_MEM_SIZE}'."
        exit 1
    fi

    TM_HEAP_SIZE=${FLINK_TM_MEM_SIZE}
    TM_OFFHEAP_SIZE=0
    # Netty needs some memory for buffer initialization
    NETTY_MEMORY=1

    if useOffHeapMemory; then
        if [[ "${FLINK_TM_MEM_MANAGED_SIZE}" -gt "0" ]]; then
            # We split up the total memory in heap and off-heap memory
            if [[ "${FLINK_TM_MEM_SIZE}" -le "${FLINK_TM_MEM_MANAGED_SIZE}" ]]; then
            ...
            TM_HEAP_SIZE=$((FLINK_TM_MEM_SIZE - FLINK_TM_MEM_MANAGED_SIZE))
        else
            # We calculate the memory using a fraction of the total memory
            if [[ `bc -l <<< "${FLINK_TM_MEM_MANAGED_FRACTION} >= 1.0"` != "0" ]]; then
            ...
            TM_OFFHEAP_SIZE=`printf '%.0f\n' $(bc -l <<< "${FLINK_TM_MEM_SIZE} * ${FLINK_TM_MEM_MANAGED_FRACTION}")`
            TM_HEAP_SIZE=$((FLINK_TM_MEM_SIZE - TM_OFFHEAP_SIZE))
        fi
    fi

    export JVM_ARGS="${JVM_ARGS} -Xms${TM_HEAP_SIZE}M -Xmx${TM_HEAP_SIZE}M -XX:MaxDirectMemorySize=$((TM_OFFHEAP_SIZE + FLINK_TM_NET_MEM_SIZE + NETTY_MEMORY))M"
