...
case $STARTSTOP in
    ...
    (start)
        ...
        if [[ -z "$JOBMANAGER_ADDRESS" ]]; then
            ...
            $JAVA_RUN $JVM_ARGS ${FLINK_ENV_JAVA_OPTS} "${log_setting[@]}" -classpath "$FLINK_TM_CLASSPATH" org.apache.flink.runtime.taskmanager.TaskManager -jobmanagerAdd "$JOBMANAGER_ADDRESS" -configDir "$FLINK_CONF_DIR" > "$out" 2>&1 < /dev/null &
        fi
        echo $! > $pid
    ;;
