...
public class AsyncBatchWaitOperator<IN, OUT> extends AbstractStreamOperator<OUT>
    ...
    /** Maximum number of concurrent async batch operations. */
    ...
    /** The registered timer future, used to cancel when batch flushes before timeout. */
     ...
     * @param asyncBatchFunction The async batch function to invoke
     * @param maxBatchSize Maximum batch size before triggering async invocation
     * @param capacity Maximum number of concurrent async batch operations
     * @param mailboxExecutor Mailbox executor for processing async results
     */
    public AsyncBatchWaitOperator(
            @Nonnull StreamOperatorParameters<OUT> parameters,
            ...
            int capacity,
            @Nonnull MailboxExecutor mailboxExecutor) {
        this(parameters, asyncBatchFunction, maxBatchSize, NO_TIMEOUT, capacity, mailboxExecutor);
    }
    ...
    private void flushBuffer() throws Exception {
        ...
        // Wait for capacity if needed (like AsyncWaitOperator)
        while (inFlightCount >= capacity) {
    ...
    private void registerBatchTimer() {
        if (!timerRegistered && isTimeoutEnabled()) {
            ...
            timerFuture = getProcessingTimeService().registerTimer(fireTime, this);
            timerRegistered = true;
        }
    }

    /** Clear timer state. Cancels the pending timer if one is registered. */
