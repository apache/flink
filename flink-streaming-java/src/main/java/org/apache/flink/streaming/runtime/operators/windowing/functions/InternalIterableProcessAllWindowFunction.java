...
public final class InternalIterableProcessAllWindowFunction<IN, OUT, W extends Window>
	...
	@Override
	public void process(Byte aByte, final W window, final InternalWindowContext context, Iterable<IN> input, Collector<OUT> out) throws Exception {
		ProcessAllWindowFunction<IN, OUT, W> wrappedFunction = this.wrappedFunction;
		ProcessAllWindowFunction<IN, OUT, W>.Context ctx = wrappedFunction.new Context() {
			@Override
			public W window() {
				return window;
			}
