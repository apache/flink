...
public class OperatorChain<OUT, OP extends StreamOperator<OUT>> {
	...
	@VisibleForTesting
	public OperatorChain (StreamTask<OUT, OP> containingTask, RecordWriterOutput<?>[] recordWriterOutputs) {
		this(containingTask, recordWriterOutputs, new StreamStatusMaintainerImpl(recordWriterOutputs));
	}

	public OperatorChain(
			StreamTask<OUT, OP> containingTask,
			RecordWriterOutput<?>[] recordWriterOutputs,
		...
		try {
			for (int i = 0; i < outEdgesInOrder.size(); i++) {
				...
				streamOutputMap.put(outEdge, recordWriterOutputs[i]);
			}
		...
		if (selectors == null || selectors.isEmpty()) {
			...
			if (allOutputs.size() == 1) {
				...
				if (containingTask.getExecutionConfig().isObjectReuseEnabled()) {
					return new CopyingBroadcastingOutputCollector<>(asArray, streamStatusMaintainer);
				} else  {
					return new BroadcastingOutputCollector<>(asArray, streamStatusMaintainer);
				}
			}
		}
		else {
	...
	private <IN, OUT> WatermarkGaugeExposingOutput<StreamRecord<IN>> createChainedOperator(
		...
		if (containingTask.getExecutionConfig().isObjectReuseEnabled()) {
			currentOperatorOutput = new ChainingOutput<>(chainedOperator, streamStatusMaintainer, outputTag);
		}
		else {
			TypeSerializer<IN> inSerializer = operatorConfig.getTypeSerializerIn1(userCodeClassloader);
			currentOperatorOutput = new CopyingChainingOutput<>(
				chainedOperator, inSerializer, outputTag, streamStatusMaintainer);
		}
