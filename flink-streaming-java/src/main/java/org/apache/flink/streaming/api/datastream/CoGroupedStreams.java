...
public class CoGroupedStreams<T1, T2> {
    ...
    public static class WithWindow<T1, T2, KEY, W extends Window> {
        ...
        public <T> DataStream<T> apply(
            ...
            optionallyAddUid(input1, taggedInput1);
            ...
            optionallyAddUid(input2, taggedInput2);
        ...
        /**
         * Adds a uid to the dataStream operator based on uid from `input`, happens when
         * autoGeneratedUids is disabled.
         *
         * @param input
         * @param dataStream
         * @return
         */
        private void optionallyAddUid(
                DataStream<?> input, SingleOutputStreamOperator<TaggedUnion<T1, T2>> dataStream) {
            if (input.getExecutionConfig().hasAutoGeneratedUIDsEnabled()) {
                return;
            }
            Transformation<?> transform = input.getTransformation();
            final String hash =
                    StringUtils.defaultIfBlank(
                            transform.getUid(), transform.getUserProvidedNodeHash());
            final String uid =
                    StringUtils.defaultIfBlank(hash, String.valueOf(transform.hashCode()));
            dataStream.uid(uid + "-taggerMap");
        }
