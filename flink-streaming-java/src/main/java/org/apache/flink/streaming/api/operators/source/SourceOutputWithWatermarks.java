 ...
 *
 * <p>Currently, we have one implementation of these methods in the batch case (see class {@link
 * BatchTimestampsAndWatermarks}) and one for the streaming case (this class). When the JVM is
 * dedicated to a single job (or type of job) only one of these classes will be loaded. In mixed job
 * setups, we still have a bimorphic method (rather than a poly/-/mega-morphic method).
 *
