...
public class CheckpointedInputGateTest {
    ...
    public void testPersisting(boolean drainGate) throws Exception {
        ...
        try {
            ...
            emit(gate, 0, buildSomeBuffer());
            emit(gate, 0, barrier(checkpointId));
            emit(gate, 0, buildSomeBuffer());
            emit(gate, 1, buildSomeBuffer());
            emit(gate, 1, barrier(obsoleteCheckpointId));
            emit(gate, 1, buildSomeBuffer());
            emit(gate, 2, buildSomeBuffer());
            ...
            emit(gate, 1, barrier(checkpointId));
            emit(gate, 1, buildSomeBuffer());
            // Another obsoleted barrier that should be ignored
            emit(gate, 2, barrier(obsoleteCheckpointId));
            emit(gate, 2, buildSomeBuffer());
            ...
            emit(gate, 2, barrier(checkpointId));
            emit(gate, 2, buildSomeBuffer());
    ...
    private void emitEndOfState(CheckpointedInputGate checkpointedInputGate, int channelIndex)
            throws IOException {
        emit(checkpointedInputGate, channelIndex, EndOfChannelStateEvent.INSTANCE);
    }
    ...
    private void emit(
        ...
        emit(checkpointedInputGate, channelIndex, EventSerializer.toBuffer(event, hasPriority));
    }

    private void emit(CheckpointedInputGate checkpointedInputGate, int channelIndex, Buffer buffer)
            throws IOException {
