...
public class StreamingJobGraphGeneratorTest extends TestLogger {
	...
	public void testDisabledAutoUIDGenerator() {
		...
		env.fromElements(1, 2, 3).map(new NoOpIntMap()).addSink(new DiscardingSink<>());
		StreamingJobGraphGenerator.createJobGraph(env.getStreamGraph());

		Assert.fail("Accepted job graph without manually set UIDs");
	}
	...
	public void testDisabledAutoUIDWithManuallySetId() {
		...
		env.fromElements(1, 2, 3).uid("uid").map(new NoOpIntMap()).addSink(new DiscardingSink<>());
		StreamingJobGraphGenerator.createJobGraph(env.getStreamGraph());
	}

	@Test
	public void testDisabledAutoUIDWithMultipleChains() {
		StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
		env.getConfig().disableAutoGeneratedUIDs();
	...
	@Test
	public void testDisabledAutoUIDWithManuallySetHash() {
		StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
		env.getConfig().disableAutoGeneratedUIDs();
		env.setParallelism(1);

		String hash = "01234567890123456789012345678901";

		env.fromElements(1, 2, 3).setUidHash(hash).map(new NoOpIntMap()).addSink(new DiscardingSink<>());
		StreamingJobGraphGenerator.createJobGraph(env.getStreamGraph());
	}
}
