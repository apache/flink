 ...
 * {@link WindowAssigner WindowAssigners}. For examples on how to use this class, the user
 * can read the {@link org.apache.flink.streaming.runtime.operators.windowing.WindowingTestHarnessTest}.
 * The input elements of type {@code IN} should implement the {@code equals()} method, for the
 * tests to be valid.
 */
public class WindowingTestHarness<K, IN, W extends Window> {
	...
	public WindowingTestHarness(ExecutionConfig executionConfig,
		...
		ListStateDescriptor<IN> windowStateDesc = new ListStateDescriptor<>("window-contents",
	 ...
	 * <p>
	 * The expected output should contain the elements and watermarks that we expect the output of the operator to
	 ...
	 *<p>
	 *     <b>NOTE:</b> This methods uses an {@code assert()} internally, thus failing the test if the {@code expected} output
	 *     does not match the {@code actual} one.
	 */
	public void compareActualToExpectedOutput(String errorMessage) {
		TestHarnessUtil.assertOutputEqualsSorted(errorMessage, expectedOutputs, testHarness.getOutput(), new Tuple2ResultSortComparator());
	}
	...
	private class Tuple2ResultSortComparator implements Comparator<Object> {
		@Override
