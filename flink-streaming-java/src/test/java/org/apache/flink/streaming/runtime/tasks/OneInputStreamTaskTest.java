...
public class OneInputStreamTaskTest extends TestLogger {
	...
	private static class TestMutableObjectReuseOperator
		...
		public void processElement(StreamRecord<Tuple2<String, Integer>> element) throws Exception {
			if (isHeadOperator) {
				if (prevRecord != null) {
					assertNotEquals("Reuse StreamRecord object in the head operator.", prevRecord, element);
					assertEquals("No reuse value object in the head operator.", prevValue, element.getValue());
				}
