...
public class DataStreamTest extends StreamingMultipleProgramsTestBase {
	...
	public void sinkKeyTest() {
		...
		assertTrue(env.getStreamGraph().getStreamNode(sink2.getTransformation().getId()).getInEdges().get(0).getPartitioner() instanceof KeyGroupPartitioner);
		...
		assertTrue(env.getStreamGraph().getStreamNode(sink3.getTransformation().getId()).getInEdges().get(0).getPartitioner() instanceof KeyGroupPartitioner);
	}
	...
	private static boolean isPartitioned(List<StreamEdge> edges) {
		...
		for (StreamEdge edge: edges) {
			if (!(edge.getPartitioner() instanceof KeyGroupPartitioner)) {
				result = false;
			}
		}
		return result;
