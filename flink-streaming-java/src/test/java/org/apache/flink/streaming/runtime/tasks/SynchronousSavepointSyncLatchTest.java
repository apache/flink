...
public class SynchronousSavepointSyncLatchTest {
	...
	@After
	public void cleanErrorRef() throws InterruptedException {
		while (!executors.isTerminated()) {
			executors.shutdownNow();
			executors.awaitTermination(10, TimeUnit.MILLISECONDS);
		}
	}
	...
	@Test
	public void checkpointIdIsSetOnlyOnce() throws Exception {
		final SynchronousSavepointLatch latchUnderTest = new SynchronousSavepointLatch();
		...
		assertTrue(latchUnderTest.isWaiting());
		latchUnderTest.acknowledgeCheckpointAndTrigger(1L);
		boolean firstResult = firstFuture.get();

		assertTrue(firstResult);
		assertTrue(latchUnderTest.isCompleted());

		boolean secondResult = secondFuture.get();

		// it will unblock but it is not the correct checkpointId.
		assertFalse(secondResult);
	}

	private final class WaitingOnLatchCallable implements Callable<Boolean> {
