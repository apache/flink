...
public class RocksDbMultiClassLoaderTest {
	...
	public void testTwoSeparateClassLoaders() throws Exception {
		final String className = "org.apache.flink.contrib.streaming.state.RocksDBStateBackend";

		// collect the libraries / class foders with RocksDB related code: the state backend and RocksDB itself
		final URL codePath1 = Class.forName(className, false, getClass().getClassLoader())
				.getProtectionDomain().getCodeSource().getLocation();
		final URL codePath2 = Class.forName("org.rocksdb.RocksDB", false, getClass().getClassLoader())
				.getProtectionDomain().getCodeSource().getLocation();
		...
		final Class<?> clazz1 = Class.forName("org.apache.flink.contrib.streaming.state.RocksDBStateBackend", false, loader1);
		final Class<?> clazz2 = Class.forName("org.apache.flink.contrib.streaming.state.RocksDBStateBackend", false, loader2);
