...
NOTE: Use the prebound Execution Environments:
 "benv" for the Batch API and "senv" for Streaming API, e.g.:
  * benv.execute("Batch Program")
  * senv.execute("Stream Program")
      """
    // scalastyle:on
    )
