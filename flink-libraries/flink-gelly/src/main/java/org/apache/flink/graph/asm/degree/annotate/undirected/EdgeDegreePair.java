...
/**
 * Annotates edges of an undirected graph with the source and target degree count.
 *
...
extends CachingGraphAlgorithm<K, VV, EV, DataSet<Edge<K,Tuple2<LongValue,LongValue>>>> {
	...
	protected boolean reduceOnTargetLabel = false;
	 ...
	 * @return this
	 */
	public EdgeDegreePair<K,VV,EV> setReduceOnTargetLabel(boolean reduceOnTargetLabel) {
	...
	public DataSet<Edge<K,Tuple2<LongValue,LongValue>>> runInternal(Graph<K,VV,EV> input)
		...
		DataSet<Edge<K,LongValue>> edgeSourceDegrees = input
			.run(new EdgeSourceDegree<K,VV,EV>()
				.setReduceOnTargetLabel(reduceOnTargetLabel)
