...
import org.apache.flink.table.runtime.join.batch.RandomSortMergeInnerJoinTest.TestSortMergeJoinOperator;
...
import static org.apache.flink.table.runtime.join.batch.RandomSortMergeInnerJoinTest.match;
import static org.apache.flink.table.runtime.join.batch.RandomSortMergeInnerJoinTest.transformToBinary;
...
public class RandomSortMergeOuterJoinTest {
	...
	public void testLeftOuterJoinWithHighNumberOfCommonKeys() {
		testOuterJoinWithHighNumberOfCommonKeys(FlinkJoinRelType.LEFT, 200, 10, 4096, 0.02f, 100, 4000, 2048, 0.02f);
	}

	@Test
	public void testRightOuterJoinWithHighNumberOfCommonKeys() {
		testOuterJoinWithHighNumberOfCommonKeys(FlinkJoinRelType.RIGHT, 100, 10, 2048, 0.02f, 200, 4000, 4096, 0.02f);
	}

	@SuppressWarnings("unchecked, rawtypes")
	protected void testOuterJoinWithHighNumberOfCommonKeys(
			FlinkJoinRelType outerJoinType, int input1Size, int input1Duplicates, int input1ValueLength,
	...
	private Map<Integer, Collection<Match>> joinValues(
			...
			FlinkJoinRelType outerJoinType) {
		...
		for (Integer key : leftMap.keySet()) {
			...
			if (outerJoinType == FlinkJoinRelType.RIGHT && rightValues == null) {
				continue;
			}
		...
		if (outerJoinType == FlinkJoinRelType.RIGHT || outerJoinType == FlinkJoinRelType.FULL) {
