...
class ValueIntervalTest {
  ...
  def testUnion(): Unit = {
    ...
    assertEquals(union(empty, empty), empty)
    // infinity union infinity = infinity
    assertEquals(union(infinite, infinite), infinite)
    // empty union [1, 3] = [1, 3]
    val interval1 = ValueInterval(1, 3)
    assertEquals(union(empty, interval1), interval1)
    assertEquals(union(interval1, empty), interval1)
    // infinity union [1,3] = infinity
    assertEquals(union(infinite, interval1), infinite)
    assertEquals(union(interval1, infinite), infinite)
    // [1, 3] union (-1, 2) = (-1, 3]
    assertEquals(
      union(interval1, ValueInterval(-1, 2, false, true)),
      ValueInterval(-1, 3, false, true))
    // [1, 3] union [-1, 4] = [-1, 4]
    assertEquals(union(interval1, ValueInterval(-1, 4)), ValueInterval(-1, 4))
    // [1, 3] union [-3, -2) = [-3, 3]
    assertEquals(union(interval1, ValueInterval(-3, -2, true, false)), ValueInterval(-3, 3))
    // [1, 3] union (0, 3) = (0, 3]
    assertEquals(
      union(interval1, ValueInterval(0, 3, false, false)),
      ValueInterval(0, 3, false, true))
    // [1, 3] union (0, 4] = (0, 4]
    assertEquals(
      union(interval1, ValueInterval(0, 4, false, true)),
      ValueInterval(0, 4, false, true))
    // [1, 3] union [4, 7] = [1,7]
    assertEquals(union(interval1, ValueInterval(4, 7)), ValueInterval(1, 7))
    ...
    assertEquals(union(interval1, ValueInterval(null, 4)), ValueInterval(null, 4))
    assertEquals(union(ValueInterval(null, 4), interval1), ValueInterval(null, 4))
    // [1, 3] union [-1, Inf) = [-1, Inf)
    assertEquals(union(interval1, ValueInterval(-1, null)), ValueInterval(-1, null))
    assertEquals(union(ValueInterval(-1, null), interval1), ValueInterval(-1, null))
    // [1, 3] union [0, Inf) = [0, Inf)
    assertEquals(union(interval1, ValueInterval(0, null)), ValueInterval(0, null))
    assertEquals(union(ValueInterval(0, null), interval1), ValueInterval(0, null))
    // [1, 3] union [4, Inf) = [1, Inf)
    assertEquals(union(interval1, ValueInterval(4, null)), ValueInterval(1, null))
    assertEquals(union(ValueInterval(4, null), interval1), ValueInterval(1, null))
    val interval2 = ValueInterval(null, 2)
    // (-Inf, 2] union [3, Inf) = infinity
    assertEquals(union(interval2, ValueInterval(3, null)), infinite)
    assertEquals(union(ValueInterval(3, null), interval2), infinite)
    // (-Inf, 2] union [-1, Inf) = infinity
    assertEquals(union(interval2, ValueInterval(-1, null)), infinite)
    assertEquals(union(ValueInterval(-1, null), interval2), infinite)
    // (-Inf, 2] union (-Inf, 1) = (-Inf, 2]
    assertEquals(union(interval2, ValueInterval(null, 1, includeUpper = false)), interval2)
    ...
    assertEquals(union(interval3, ValueInterval(4, null, includeLower = false)), interval3)
  ...
  def testIsIntersected(): Unit = {
    ...
    // [1, 3] intersect (-1, 2)
    assertTrue(isIntersected(interval1, ValueInterval(-1, 2, false, false)))
    // [1, 3] intersect [-1, 4]
    assertTrue(isIntersected(interval1, ValueInterval(-1, 4)))
    // [1, 3] not intersect [-3, -2)
    assertFalse(isIntersected(interval1, ValueInterval(-3, -2, true, false)))
    // [1, 3] intersect (0, 3)
    assertTrue(isIntersected(interval1, ValueInterval(0, 3, false, false)))
    // [1, 3] intersect (0, 4]
    assertTrue(isIntersected(interval1, ValueInterval(0, 4, false, true)))
    // [1, 3] not intersect [4, 7]
    assertFalse(isIntersected(interval1, ValueInterval(4, 7)))
    // [1, 3) not intersect [3, 3]
    assertFalse(isIntersected(ValueInterval(1, 3, true, false), ValueInterval(3, 3)))
  ...
  def testIntersect(): Unit = {
    ...
    assertEquals(intersect(infinite, infinite), infinite)
    val interval1 = ValueInterval(1, 3)
    // empty intersect [1, 3] = empty
    assertEquals(intersect(empty, interval1), empty)
    assertEquals(intersect(interval1, empty), empty)
    // infinity intersect [1,3] = [1,3]
    assertEquals(intersect(infinite, interval1), interval1)
    assertEquals(intersect(interval1, infinite), interval1)
    // [1, 3] intersect (-1, 2) = [1, 2)
    assertEquals(
      intersect(interval1, ValueInterval(-1, 2, false, false)),
      ValueInterval(1, 2, true, false))
    // [1, 3] intersect [-1, 4] = [1, 3]
    assertEquals(intersect(interval1, ValueInterval(-1, 4)), interval1)
    // [1, 3] intersect [-3, -2) = empty
    assertEquals(intersect(interval1, ValueInterval(-3, -2, true, false)), empty)
    // [1, 3] intersect (0, 3) = [1, 3)
    assertEquals(
      intersect(interval1, ValueInterval(0, 3, false, false)),
      ValueInterval(1, 3, true, false))
    // [1, 3] intersect (0, 4] = [1, 3]
    assertEquals(intersect(interval1, ValueInterval(0, 4, false, true)), interval1)
    // [1, 3] intersect [4, 7] = empty
    assertEquals(intersect(interval1, ValueInterval(4, 7)), empty)
    // [1, 3) intersect [3, 3] = empty
    assertEquals(intersect(ValueInterval(1, 3, true, false), ValueInterval(3, 3)), empty)
    // [1, 3] intersect (-Inf, -2] = empty
    assertEquals(intersect(interval1, ValueInterval(null, -2)), empty)
    assertEquals(intersect(ValueInterval(null, -2), interval1), empty)
    ...
    assertEquals(intersect(interval1, ValueInterval(null, 4)), interval1)
    assertEquals(intersect(ValueInterval(null, 4), interval1), interval1)
    // [1, 3] intersect [-1, Inf) = [1, 3]
    assertEquals(intersect(interval1, ValueInterval(-1, null)), interval1)
    assertEquals(intersect(ValueInterval(-1, null), interval1), interval1)
    // [1, 3] intersect [0, Inf) = [1, 3]
    assertEquals(intersect(interval1, ValueInterval(0, null)), interval1)
    assertEquals(intersect(ValueInterval(0, null), interval1), interval1)
    // [1, 3] intersect [4, Inf) = empty
    assertEquals(intersect(interval1, ValueInterval(4, null)), empty)
    assertEquals(intersect(ValueInterval(4, null), interval1), empty)
    // [1, 3] intersect (3, Inf) = empty
    assertEquals(intersect(interval1, ValueInterval(3, null, includeLower = false)), empty)
    assertEquals(intersect(ValueInterval(3, null, includeLower = false), interval1), empty)
    val interval2 = ValueInterval(null, 2)
    // (-Inf, 2] intersect [3, Inf) = empty
    assertEquals(intersect(interval2, ValueInterval(3, null)), empty)
    assertEquals(intersect(ValueInterval(3, null), interval2), empty)
    // (-Inf, 2] intersect [-1, Inf) = [-1, 2]
    assertEquals(intersect(interval2, ValueInterval(-1, null)), ValueInterval(-1, 2))
    assertEquals(intersect(ValueInterval(-1, null), interval2), ValueInterval(-1, 2))
    // (-Inf, 2] intersect (2, Inf) = empty
    assertEquals(intersect(interval2, ValueInterval(2, null, includeLower = false)), empty)
    assertEquals(intersect(ValueInterval(2, null, includeLower = false), interval2), empty)
    // (-Inf, 2] intersect [2, Inf) = [2, 2]
    assertEquals(intersect(interval2, ValueInterval(2, null)), ValueInterval(2, 2))
    assertEquals(intersect(ValueInterval(2, null), interval2), ValueInterval(2, 2))
    ...
    assertEquals(intersect(interval2, ValueInterval(null, 3, includeUpper = false)), interval2)
