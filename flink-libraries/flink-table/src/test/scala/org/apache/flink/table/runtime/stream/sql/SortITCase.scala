...
class SortITCase extends StreamingWithStateTestBase {
  ...
  /***
   * time descending for sort with retraction
   */
  
   @Test
  def testEventTimeOrderByDescOffset(): Unit = {
    val data = Seq(
    ...
    val  sqlQuery = "SELECT b FROM T1 " +
      "ORDER BY rowtime DESC, b ASC OFFSET 2";
  ...
  def testEventTimeOrderByOffsetFetch(): Unit = {
    ...
    val  sqlQuery = "SELECT b FROM T1 " +
      "ORDER BY rowtime DESC, b ASC OFFSET 2 FETCH FIRST 2 ROWS ONLY";
  ...
  @Test
  def testEventTimeOrderByAscOffset(): Unit = {
    val data = Seq(
      Left((1500L, (1L, 15, "Hello"))),
      Left((1600L, (1L, 16, "Hello"))),
      Left((1000L, (1L, 1, "Hello"))),
    ...
    val expected = mutable.MutableList("16", "1", "2", "2", "3" , "3", "4")
      
    assertEquals(expected, SortITCase.testResults)
  }
   
  @Test
  def testEventTimeOrderByAscOffsetFetch(): Unit = {
    val data = Seq(
      Left((1500L, (1L, 15, "Hello"))),
      Left((1600L, (1L, 16, "Hello"))),
      Left((1000L, (1L, 1, "Hello"))),
    ...
    val  sqlQuery = "SELECT b FROM T1 " +
      "ORDER BY rowtime, b ASC OFFSET 5 FETCH FIRST 2 ROWS ONLY";
