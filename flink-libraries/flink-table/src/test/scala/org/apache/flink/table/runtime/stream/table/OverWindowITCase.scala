...
class OverWindowITCase(mode: StateBackendMode) extends StreamingWithStateTestBase(mode) {
  ...
  def testProcTimeBoundedPartitionedRowsOver(): Unit = {
    ...
    val stream = failingDataSource(data)
    val table = stream.toTable(tEnv, 'a, 'b, 'c, 'd, 'e)

    val windowedTable = table.select('a, 'b, 'c, 'd, 'e, proctime() as 'proctime)
  ...
  def testProcTimeBoundedPartitionedRowsOverWithJavaAPI(): Unit = {
    ...
    val windowedTable = table.select("a, b, c, d, e, proctime() as proctime")
                        .window(org.apache.flink.table.api.java.Over
                                  .partitionBy("a")
                                  .orderBy("proctime")
                                  .preceding("4.rows")
  ...
  @Test
  def testSortedMapViewWithProcTimeUnBoundedPartitionedRowOver(): Unit = {
    ...
    val countFun = new CountAggFunction
    val weightAvgFun = new WeightedAvg
    val sortedMapTestAgg = new SortedMapTestAgg
    ...
    val windowedTable = table.select('a, 'b, 'c, proctime() as 'proctime)
      ...
      .select('c,
        countFun('b) over 'w as 'mycount,
        weightAvgFun('a, 'b) over 'w as 'wAvg,
        sortedMapTestAgg('a) over 'w as 'sortedMapTestAgg)
      .select('c, 'mycount, 'wAvg, 'sortedMapTestAgg)
