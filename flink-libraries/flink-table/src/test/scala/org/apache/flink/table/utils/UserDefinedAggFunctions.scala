...
case class NonMergedAccumulator(var count: Long)

class NonMergedAgg extends AggregateFunction[Long, NonMergedAccumulator] {

  def accumulate(acc: NonMergedAccumulator, value: Any): Unit = {
    if (null != value) {
      acc.count = acc.count + 1
    }
  }

  def resetAccumulator(acc: NonMergedAccumulator): Unit = {
  }

  override def createAccumulator(): NonMergedAccumulator = NonMergedAccumulator(0)

  override def getValue(acc: NonMergedAccumulator): Long = acc.count
}
