...
import org.apache.flink.table.api.scala.batch.utils.{TableProgramsClusterTestBase, UDFTestUtils}
...
class DataSetCorrelateITCase(
  ...
  @Test
  def testUDTFWithParameter(): Unit = {
    val env = ExecutionEnvironment.getExecutionEnvironment
  ...
  @Test
  def testRichUDTFWithRichUDF(): Unit = {
    val env = ExecutionEnvironment.getExecutionEnvironment
    val tEnv = TableEnvironment.getTableEnvironment(env)
    val richTableFunc1 = new RichTableFunc1
    tEnv.registerFunction("RichTableFunc1", richTableFunc1)
