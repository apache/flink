...
class FlinkRewriteSubQueryRuleTest extends SubQueryTestBase(true) {
  ...
  def testUnsupportedConversionWithUnexpectedComparisonNumber(): Unit = {
    // without correlation
    util.verifySqlNotExpected(
      "SELECT * FROM x WHERE (SELECT COUNT(*) FROM y WHERE d > 10) > 1", "SemiJoin")
    util.verifySqlNotExpected(
      "SELECT * FROM x WHERE (SELECT COUNT(*) FROM y WHERE d > 10) >= 0", "SemiJoin")
    util.verifySqlNotExpected(
      "SELECT * FROM x WHERE (SELECT COUNT(*) FROM y WHERE d > 10) > -1", "SemiJoin")
    util.verifySqlNotExpected(
      "SELECT * FROM x WHERE 0 <= (SELECT COUNT(*) FROM y WHERE d > 10)", "SemiJoin")
    util.verifySqlNotExpected(
      "SELECT * FROM x WHERE -1 < (SELECT COUNT(*) FROM y WHERE d > 10)", "SemiJoin")

    // with correlation
    util.verifySqlNotExpected(
      "SELECT * FROM x WHERE (SELECT COUNT(*) FROM y WHERE a = d) > 1", "SemiJoin")
    util.verifySqlNotExpected(
      "SELECT * FROM x WHERE (SELECT COUNT(*) FROM y WHERE a = d) >= 0", "SemiJoin")
    util.verifySqlNotExpected(
      "SELECT * FROM x WHERE 1 < (SELECT COUNT(*) FROM y WHERE a = d)", "SemiJoin")
    util.verifySqlNotExpected(
      "SELECT * FROM x WHERE 0 <= (SELECT COUNT(*) FROM y WHERE a = d)", "SemiJoin")
  }
  ...
  def testSqlFromTpcDsQ41(): Unit = {
    util.addTable[(Int, String, String, String, String, String, String)]("item",
