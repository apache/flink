...
import org.apache.flink.table.runtime.join.{JoinTimeIndicator, ProcTimeWindowInnerJoin, TimeBoundedStreamInnerJoin}
...
class JoinHarnessTest extends HarnessTestBase{
  ...
  def testNewNormalProcTimeJoin() {
    ...
    val joinProcessFunc = new TimeBoundedStreamInnerJoin(
      -10, 20, 0, rT, rT, "TestJoinFunction", funcCode, -1, -1, JoinTimeIndicator.PROCTIME)
  ...
  def testNewProcTimeJoinSingleNeedStore() {
    ...
    val joinProcessFunc = new TimeBoundedStreamInnerJoin(
      -10, -5, 0, rT, rT, "TestJoinFunction", funcCode, -1, -1, JoinTimeIndicator.PROCTIME)
