...
class GroupingSetsITCase extends BatchTestBase {
  ...
  val TABLE_NAME_EMPS = "emps"
  val empsTypes = new RowTypeInfo(Types.LONG, Types.STRING, Types.INT, Types.STRING, Types.STRING,
    Types.LONG, Types.INT, Types.BOOLEAN, Types.BOOLEAN, Types.SQL_DATE)
  val empsNames = "empno, name, deptno, gender, city, empid, age, slacker, manager, joinedat"
  val nullableOfEmps = Seq(false, false, false, true, true, false, true, true, false, false)
  lazy val empsData = Seq(
  ...
  val TABLE_NAME_EMP = "emp"
  val empTypes = new RowTypeInfo(Types.STRING, Types.INT, Types.STRING)
  val empNames = "ename, deptno, gender"
  val nullableOfEmp = Seq(false, true, false)
  lazy val empData = Seq(
    row("Adam", 50, "M"),
    row("Alice", 30, "F"),
    row("Bob", 10, "M"),
    row("Eric", 20, "M"),
  ...
  val TABLE_NAME_DEPT = "dept"
  val deptTypes = new RowTypeInfo(Types.INT, Types.STRING)
  val deptNames = "deptno, dname"
  val nullableOfDept = Seq(false, false)
  lazy val deptData = Seq(
    row(10, "Sales"),
    row(20, "Marketing"),
    row(30, "Engineering"),
    row(40, "Empty")
  ...
  val TABLE_NAME_SCOTT_EMP = "scott_emp"
  val scottEmpTypes = new RowTypeInfo(Types.INT, Types.STRING, Types.STRING, Types.INT,
    Types.SQL_DATE, Types.DOUBLE, Types.DOUBLE, Types.INT)
  val scottEmpNames = "empno, ename, job, mgr, hiredate, sal, comm, deptno"
  val nullableOfScottEmp = Seq(false, false, false, true, false, false, true, false)
  lazy val scottEmpData = Seq(
  ...
  def before(): Unit = {
    ...
    val nullablesOfNullsData3 = Seq(false, false, true)
