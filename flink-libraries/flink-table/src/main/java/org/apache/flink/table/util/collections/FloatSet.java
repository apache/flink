...
public class FloatSet extends OptimizableHashSet {
	...
	public FloatSet(final int expected, final float f) {
		super(expected, f);
		this.key = new float[this.n + 1];
	}

	public FloatSet(final int expected) {
	...
	public boolean add(final float k) {
		...
		if (intKey == 0) {
			...
			int pos;
			int curr;
			if ((curr = Float.floatToIntBits(key[pos = Murmur32.fmix(intKey) & this.mask])) != 0) {
				if (curr == intKey) {
					return false;
				}
	...
	private void rehash(final int newN) {
		...
		for (int j = this.realSize(); j-- != 0; newKey[pos] = key[i]) {
			...
			if (Float.floatToIntBits(newKey[pos =
					Murmur32.fmix(Float.floatToIntBits(key[i])) & mask]) != 0) {
