 ...
 * Double set.
 */
public class DoubleSet extends OptimizableHashSet {

	private double[] key;

	public DoubleSet(final int expected, final float f) {
		super(expected, f);
		this.key = new double[this.n + 1];
	}

	public DoubleSet(final int expected) {
	...
	public boolean add(final double k) {
		...
		if (longKey == 0L) {
			...
			int pos;
			long curr;
			if ((curr = Double.doubleToLongBits(key[pos = (int) Murmur32.fmix(longKey) & this.mask])) != 0L) {
				if (curr == longKey) {
					return false;
				}
	...
	private void rehash(final int newN) {
		...
		for (int j = this.realSize(); j-- != 0; newKey[pos] = key[i]) {
			...
			if (Double.doubleToLongBits(newKey[pos =
					(int) Murmur32.fmix(Double.doubleToLongBits(key[i])) & mask]) != 0L) {
