...
/**
  ...
  * @param resultType The result type. If it is null, it will be inferred from the
  *                   [[tableFunction]]
...
class FlinkTableFunctionImpl[T](
    ...
    var resultType: TypeInformation[_],
  ...
  override def getRowType(typeFactory: RelDataTypeFactory,
    ...
    // Get the result type from table function. If it is not null, the resultType may
    // already be generated by Table API's apply() method.
    if (tableFunction.getResultType(arguments) != null) {
      resultType = tableFunction.getResultType(arguments)
    }
  ...
  private def checkFields(): Unit = {
    ...
    if (null == resultType) {
      throw new TableException(
        "The result type must be initialized.")
    }
