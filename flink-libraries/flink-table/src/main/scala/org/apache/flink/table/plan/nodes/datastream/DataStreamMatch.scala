...
class DataStreamMatch(
  ...
  private def translateTimeBound(interval: RexNode): Time = {
    interval match {
      ...
      case x: RexLiteral if x.getTypeName.getFamily == SqlTypeFamily.INTERVAL_YEAR_MONTH =>
        throw new ValidationException("Time restrictions can only be used with DAY_TIME " +
          "resolution.")
