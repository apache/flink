...
class GroupingsMapFunction[IN, OUT](private val mappings: Array[(Int, Int)],
  ...
  override def open(config: Configuration) {
    ...
    output = new Row(returnType.getTotalFields)
  }

  override def map(value: IN): OUT = {

    val input = value.asInstanceOf[Row]
    for (i <- 0 until returnType.getTotalFields) {
      output.setField(i, input.getField(i))
    }
    mappings.foreach {
      case (inputIndex, outputIndex) =>
        output.setField(outputIndex, input.getField(inputIndex) == null)
    }
