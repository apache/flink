...
import org.apache.calcite.util.{Util => CUtil}
...
/**
  ...
  *
  * <p>There are three transformation contexts:
  * <ul>
  ...
  * </ul>
  */
abstract class FlinkRewriteCoalesceRule[T <: RelNode](
    inputClass: Class[T],
    description: String) extends RelOptRule(operand(inputClass, any), description) {

  private class CoalesceToCaseShuttle(rexBuilder: RexBuilder) extends RexShuttle {
    override def visitCall(call: RexCall): RexNode = call.getKind match {
      case SqlKind.COALESCE =>
        val operands = call.getOperands
        if (operands.size == 1) {
          operands.head
        } else {
          val operandsExceptLast = CUtil.skipLast(call.getOperands)
  ...
  protected def existsCoalesce(rexNode: RexNode): Boolean = {
    class CoalesceFinder extends RexVisitorImpl[Unit](true) {
      var found = false
      override def visitCall(call: RexCall): Unit = call.getKind match {
        case SqlKind.COALESCE =>
          found = true
        case _ => super.visitCall(call)
      }
...
class FilterRewriteCoalesceRule extends
    FlinkRewriteCoalesceRule(classOf[LogicalFilter],
...
class ProjectRewriteCoalesceRule extends
    FlinkRewriteCoalesceRule(classOf[LogicalProject],
...
class JoinRewriteCoalesceRule extends
    FlinkRewriteCoalesceRule(classOf[LogicalJoin],
...
class CalcRewriteCoalesceRule extends
    FlinkRewriteCoalesceRule(classOf[LogicalCalc],
  ...
  override def onMatch(call: RelOptRuleCall): Unit = {
    ...
    val builder: RexProgramBuilder = new RexProgramBuilder(calc.getInput.getRowType,
