...
/**
  * A wrapper for generated class, defines a `newInstance` method so that get the instance by
  * reference objects.
  */
trait GeneratedClass[T] {
  ...
  @throws(classOf[CompileException])
  def newInstance(cl: ClassLoader): T = {
    compile(cl).getConstructor(classOf[Array[AnyRef]]).newInstance(references)
  }
}
