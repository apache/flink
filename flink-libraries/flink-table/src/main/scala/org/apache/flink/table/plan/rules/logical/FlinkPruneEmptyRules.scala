...
import org.apache.calcite.rel.core.{Join, SemiJoin, Values}
...
object FlinkPruneEmptyRules {
  /**
    ...
    * </ul>
    *
    * <p>The difference between this rule and [[org.apache.calcite.rel.rules.PruneEmptyRules]]
    * is that this rule handles anti-join specially.
    */
  val JOIN_RIGHT_INSTANCE: RelOptRule = new RelOptRule(
    ...
    override def onMatch(call: RelOptRuleCall): Unit = {
      ...
      join match {
        case sj: SemiJoin if sj.isAnti =>
          // "select * from emp where deptno not in (select deptno where dept where 1=0)"
          // return emp
