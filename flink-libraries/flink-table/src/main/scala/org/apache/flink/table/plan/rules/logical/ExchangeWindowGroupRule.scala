...
/**
 * make the window groups contains the same shuffle keys and order keys together.
 */
class ExchangeWindowGroupRule extends RelOptRule(
      operand(classOf[LogicalWindow],
        operand(classOf[RelNode], any)), "ExchangeWindowGroupRule") {

  override def matches(call: RelOptRuleCall): Boolean = {
    val window = call.rel(0).asInstanceOf[LogicalWindow]
  ...
  override def onMatch(call: RelOptRuleCall): Unit = {
    ...
    val oldGroups = new util.ArrayList(window.groups)
    val sequenceGroups = new util.ArrayList(window.groups)
    ...
    if (!sequenceGroups.equals(oldGroups) && !sequenceGroups.reverse.equals(oldGroups)) {
      ...
      val newLogicalWindow = LogicalWindow.create(
        window.getCluster.getPlanner.emptyTraitSet(), input, window.constants, intermediateRowType,
        sequenceGroups)
...
object ExchangeWindowGroupRule {
  val INSTANCE = new ExchangeWindowGroupRule
}
