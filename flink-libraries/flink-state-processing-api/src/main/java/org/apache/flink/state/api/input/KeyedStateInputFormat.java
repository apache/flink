...
public class KeyedStateInputFormat<K, N, OUT>
    ...
    public void open(KeyGroupRangeInputSplit split) throws IOException {
        ...
        // Deserialize any POJO state whose class is missing from the classpath into RowData instead
        // of failing the restore. PojoSerializerSnapshot only consults the fallback when its POJO
        // class could not be resolved, so registering it unconditionally is safe.
        //
        // The fallback is never cleared: it is registered on the task thread that also drives all
        // subsequent state access, and a given POJO type is either on the classpath for the whole
        // read or not at all. Clearing it after build() would break the RocksDB backend, which
        // defers schema compatibility checks to the first state access in nextRecord().
