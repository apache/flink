...
class StateValueConverter implements java.io.Serializable {
    ...
    // Guava caches are not serializable; the reader function containing this converter is shipped
    // to task managers via Java serialization, so these are rebuilt lazily on first use.
    private transient Cache<Tuple2<Class, String>, Field> classFieldCache;
    private transient Cache<Tuple2<Class, String>, Method> classMethodCache;

    private Cache<Tuple2<Class, String>, Field> classFieldCache() {
        if (classFieldCache == null) {
            classFieldCache = CacheBuilder.newBuilder().maximumSize(CACHE_MAX_SIZE).build();
        }
        return classFieldCache;
    }

    private Cache<Tuple2<Class, String>, Method> classMethodCache() {
        if (classMethodCache == null) {
            classMethodCache = CacheBuilder.newBuilder().maximumSize(CACHE_MAX_SIZE).build();
        }
    ...
    private Object getObjectField(Object object, RowType.RowField rowField) {
        ...
        // Avro GenericRecord: use the typed API directly.
        if (object instanceof GenericRecord) {
            return ((GenericRecord) object).get(rowFieldName);
        }
        ...
        } catch (ExecutionException noPublicField) {
            try {
                return getGetter(objectClass, rowFieldName).invoke(object);
    ...
    private Method getGetter(Class objectClass, String rowFieldName) {
        ...
        try {
            return lookupMethod(objectClass, "get" + capitalized);
        } catch (ExecutionException noGetter) {
            try {
                return lookupMethod(objectClass, "is" + capitalized);
    ...
    private Method lookupMethod(Class objectClass, String methodName) throws ExecutionException {
        return classMethodCache()
                .get(Tuple2.of(objectClass, methodName), () -> objectClass.getMethod(methodName));
    }
}
