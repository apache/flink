...
object IsotonicRegression {
  ...
  /**
    * Map function that can be applied to a data partition and that runs the PAV algorithm on that
    * data.
    */
  class AdjacentPoolViolatersMapper extends MapFunction[Array[(Double, Double, Double)], Array[
    ...
    Array[(Double, Double, Double)] = {
      ...
      // Indicates whether any data points were pooled in the current PAV iteration.
      var pooled = true
      while (pooled) {
        ...
        while (i < n) {
          // Check if the data points following the current one form a monotonically decreasing
          // sequence. After the while loop, k will contain the index of the last data point of that
          // monotonically decreasing sequence. If the points following the current one don't form
          // such a sequence, k will still point to the current data point.
          var k = i
          while (k < n && input(k)._1 >= input(k + 1)._1) {
            k += 1
          }

          // If the Y values of the current data point and the data point at index k differ, we know
          // that all data points between the current one and the one at index k form a
          // monotonically decreasing sequence, in which case we want to combine them into a single
          // pool.
          if (input(i)._1 != input(k)._1) {
            ...
            // If the sum of weights is 0, we can't use that for calculating the weighted average.
            // But since we know that all data points we want to pool have a weight of 0, we can
            // just pretend they all have a weight of 1 by using the number of data points as the
            // sum of weights.
            if (sumOfWeights == 0) {
              sumOfWeights = dataPointCount
            }

            // Calculate the weighted average and pool all data points between i and k by setting
            // their Y values to that weighted average.
          ...
          // Continue with the next data point after the current pool or simply with the next data
          // point if the current one is not part of a pool (in which case k still equals i).
          i = k + 1
        }
      }
      ...
      while (i < input.length) {
        ...
        if (label == curLabel) {
          // If the next Y value is equal to the current Y value, add its weight to the current
          // weight and move the current right boundary to the right, effectively removing the next
          // data point by merging its information into the current one.
          curWeight += weight
          rightBound = feature
        } else {
          // If the next data point's Y value differs from the current Y value, add the current data
          // point to the result. If the current point actually compresses multiple data points
          // (if it contains information of more than one data point), we have to add another tuple
          // so we don't lose the information on where the right boundary is.
  ...
  implicit val fitIR = new FitOperation[IsotonicRegression, (Double, Double, Double)] {
    ...
    override def fit(instance: IsotonicRegression,
      ...
      // First, run the PAV algorithm in parallel on separate partitions. Then, collect the combined
      // results of that parallel step on a single node (parallelism = 1) and run the algorithm
      // again on that combined data. Finally, create a new IR model from the resulting boundaries
      // and predictions.
  ...
  implicit val predictIR =
    new PredictOperation[IsotonicRegression, IsotonicRegressionModel, Double, Double] {
      ...
      /**
        * Returns an instance's IR model, if available, or throw a [[RuntimeException]] if this
        * instance wasn't given any training data yet that it could have created a model on.
      ...
      /**
        * Linear interpolation that will be used for predicting the label of a feature that lies
        * between two features that actually exist in the model.
        */
      ...
      override def predict(value: Double, model: IsotonicRegressionModel): Double = {
        ...
        if (insertIndex == 0) {
          // If provided feature is lower than the model's first data point, simply return the label
          // of that first modeldata point.
          predictions.head
        } else if (insertIndex == boundaries.length) {
          // If provided feature is larger than the model's last data point, simply return the label
          // of that last model data point.
          predictions.last
        } else if (foundIndex < 0) {
          // If provided feature lies between two features that exist in the model, return a linear
          // interpolation of these data points as the prediction.
          linearInterpolation(
          ...
          // If provided feature matches a feature that exists in the model exactly, return that
          // feature's label as the prediction.
