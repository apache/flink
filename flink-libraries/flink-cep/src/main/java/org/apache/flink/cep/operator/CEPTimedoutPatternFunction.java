 ...
 */
public class CEPTimedoutPatternFunction<IN> extends CEPAbstractBasePatternFunction<IN, Either<Tuple2<Map<String, IN>, Long>, Map<String, IN>>> {
	private static final long serialVersionUID = 3570542177814518158L;

	public CEPTimedoutPatternFunction(
	...
	@Override
	protected void processEvent(NFA<IN> nfa, IN event, long timestamp, Collector<Either<Tuple2<Map<String, IN>, Long>, Map<String, IN>>> out) {
		...
		emitTimedOutSequences(patterns.f1, out);
	}

	@Override
	protected void advanceTime(NFA<IN> nfa, long timestamp, Collector<Either<Tuple2<Map<String, IN>, Long>, Map<String, IN>>> out) {
		Tuple2<Collection<Map<String, IN>>, Collection<Tuple2<Map<String, IN>, Long>>> patterns =
	...
	private void emitTimedOutSequences(Iterable<Tuple2<Map<String, IN>, Long>> timedOutSequences,
									Collector<Either<Tuple2<Map<String, IN>, Long>, Map<String, IN>>> out) {
		for (Tuple2<Map<String, IN>, Long> partialPattern: timedOutSequences) {
			Either<Tuple2<Map<String, IN>, Long>, Map<String, IN>> rec = Either.Left(partialPattern);
			out.collect(rec);
		}
	}
	...
	protected void emitMatchedSequences(Iterable<Map<String, IN>> matchedSequences,
		...
		for (Map<String, IN> matchedPattern : matchedSequences) {
			Either<Tuple2<Map<String, IN>, Long>, Map<String, IN>> rec = Either.Right(matchedPattern);
			out.collect(rec);
		}
	}
}
