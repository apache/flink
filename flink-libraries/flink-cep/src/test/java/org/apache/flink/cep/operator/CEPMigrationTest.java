...
public class CEPMigrationTest {
	...
	public void writeAndOrSubtypConditionsPatternAfterMigrationSnapshot() throws Exception {
		...
		OneInputStreamOperatorTestHarness<Event, Map<String, List<Event>>> harness =
			new KeyedOneInputStreamOperatorTestHarness<>(
				getKeyedCepOpearator(false, new NFAComplexConfitionsFactory()),
				keySelector,
				BasicTypeInfo.INT_TYPE_INFO);
	...
	private static class NFAComplexConfitionsFactory implements NFACompiler.NFAFactory<Event> {
		...
		private NFAComplexConfitionsFactory() {
			this(false);
		}

		private NFAComplexConfitionsFactory(boolean handleTimeout) {
			this.handleTimeout = handleTimeout;
		}
