...
public class CEPRescalingTest {
	...
	public void testCEPFunctionScalingUp() throws Exception {
		...
		OneInputStreamOperatorTestHarness<Event, Map<String, Event>> harness =
			getTestHarness(getPatternFunction(), maxParallelism, 1, 0);
		harness.open();
		...
		OneInputStreamOperatorTestHarness<Event, Map<String, Event>> harness1 =
			getTestHarness(getPatternFunction(), maxParallelism, 2, 0);
		...
		OneInputStreamOperatorTestHarness<Event, Map<String, Event>> harness2 =
			getTestHarness(getPatternFunction(), maxParallelism, 2, 1);
	...
	public void testCEPFunctionScalingDown() throws Exception {
		...
		OneInputStreamOperatorTestHarness<Event, Map<String, Event>> harness1 =
			getTestHarness(getPatternFunction(), maxParallelism, 3, 0);
		harness1.open();

		OneInputStreamOperatorTestHarness<Event, Map<String, Event>> harness2 =
			getTestHarness(getPatternFunction(), maxParallelism, 3, 1);
		harness2.open();

		OneInputStreamOperatorTestHarness<Event, Map<String, Event>> harness3 =
			getTestHarness(getPatternFunction(), maxParallelism, 3, 2);
		harness3.open();
	...
	private CEPPatternFunction<Event> getPatternFunction() {
		return new CEPPatternFunction<>(
	...
	private KeyedOneInputStreamOperatorTestHarness<Integer, Event, Map<String, Event>> getTestHarness(
		CEPPatternFunction<Event> cepPatternFunction,
		int maxParallelism,
		int taskParallelism,
		int subtaskIdx) throws Exception {

		return new KeyedOneInputStreamOperatorTestHarness<>(
			new ProcessOperator<>(cepPatternFunction),
			new TestKeySelector(),
