...
public class TpcdsResultComparator {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String queryId : VALIDATE_QUERIES) {
            File expectedResultFile = new File(expectedDir, queryId + RESULT_SUFFIX);

            // the file generated by filesystem connector and csv format is a dir
            File actualResultDir = new File(actualDir, queryId + RESULT_SUFFIX);

            if (compareResult(queryId, expectedResultFile, actualResultDir)) {
                passCnt++;
                System.out.println(
                        String.format(
                                "[INFO] Validation succeeded for file: %s (%d/%d)",
                                expectedResultFile.getName(), passCnt, VALIDATE_QUERIES.size()));
            } else {
                System.out.println(
    ...
    private static boolean compareResult(
            String queryId, File expectedResultFile, File actualResultDir) throws Exception {
        final String[] expectedLines = readContent(expectedResultFile);
        final String[] actualLines = readContent(actualResultDir);
    ...
    private static String[] readContent(File fileOrDir) throws Exception {
        // if it is a file
        if (fileOrDir.isFile()) {
            return Files.readAllLines(fileOrDir.toPath(), StandardCharsets.UTF_8)
        ...
        if (files != null && files.length == 1) {
            ...
            return Files.readAllLines(onlyFile.toPath(), StandardCharsets.UTF_8)
                    .toArray(new String[0]);
        }
        if (files == null || files.length == 0) {
            throw new IllegalStateException("The dir is empty: " + fileOrDir.getAbsolutePath());
        } else {
            throw new IllegalStateException(
                    String.format(
                            "The dir contains more than one file: %s\n\nThey are:\n%s",
                            fileOrDir.getAbsolutePath(),
                            Stream.of(files).map(File::getName).collect(Collectors.joining("\n"))));
        }
    }
