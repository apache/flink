...
echo "Running streaming kinesis with AWS Glue Schema Registry using AVRO data format"
...
function start_kinesalite {
    #docker run -d --rm --name flink-glue-schema-registry-avro-test -p ${KINESALITE_PORT}:${KINESALITE_PORT} instructure/kinesalite
    # override entrypoint to enable SSL
    docker run -d --rm --entrypoint "/tini" \
        --name flink-glue-schema-registry-avro-test \
...
docker logs flink-glue-schema-registry-avro-test

function test_cleanup {
  # job needs to stop before kinesalite
  stop_cluster
  echo "terminating kinesalite"
  docker kill flink-glue-schema-registry-avro-test
}
...
TEST_JAR="${END_TO_END_DIR}/flink-glue-schema-registry-avro-test/target/GlueSchemaRegistryExample.jar"
...
$FLINK_DIR/bin/flink run -p 1 -c org.apache.flink.glue.schema.registry.test.GlueSchemaRegistryExampleTest $TEST_JAR \
  --input-stream gsr-avro-input-stream --output-stream gsr-avro-output-stream \
