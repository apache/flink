...
public enum TestUtils {
	 ...
	 * for the initialization of static {@link Path} fields for jars that reside in the modules {@code target}
	 * directory.
	 *
	...
	public static Path getResourceJar(final String jarNameRegex) {
		...
		try (Stream<Path> dependencyJars = Files.walk(Paths.get(moduleDirProp))) {
			final List<Path> matchingJars = dependencyJars
				                                .filter(jar -> Pattern.compile(jarNameRegex)
					                                               .matcher(jar.toAbsolutePath().toString()).find())
			...
			switch (matchingJars.size()) {
				...
				default:
					throw new RuntimeException(
						new IOException(
							String.format("Multiple jars were found matching the pattern %s. Matches=%s", jarNameRegex,
								matchingJars)
						)
					);
			}
