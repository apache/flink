...
public final class DistributedStandaloneFlinkResourceFactory implements FlinkResourceFactory {
	...
	private static final ParameterProperty<List<String>> JM_HOSTS = new ParameterProperty<>("jm_hosts",
		value -> Arrays.asList(value.split(";")));
	private static final ParameterProperty<List<String>> TM_HOSTS = new ParameterProperty<>("tm_hosts",
		value -> Arrays.asList(value.split(";")));
	...
	public Optional<FlinkResource> create(FlinkResourceSetup setup) {
		...
		if (jmHosts.toString().isEmpty()) {
			LOG.debug("Not loading {} because {} was not set.", DistributedStandaloneFlinkResource.class,
				JM_HOSTS.getPropertyName());
			return Optional.empty();
		}
		Optional<List<String>> tmHosts = TM_HOSTS.get();
		if (tmHosts.toString().isEmpty()) {
			LOG.debug("Not loading {} because {} was not set.", DistributedStandaloneFlinkResource.class,
				TM_HOSTS.getPropertyName());
			return Optional.empty();
		}
