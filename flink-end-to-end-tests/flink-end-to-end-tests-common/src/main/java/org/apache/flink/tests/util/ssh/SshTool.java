...
public class SshTool {
	...
	// TODO: check whether username/password are actually necessary; start-cluster.sh requires passwordless SSH to be
	//  setup
	...
	public SshTool(String username, int port) {
	...
	public static SshTool fromSystemProperties() {
		String username = SSH_USERNAME.get().orElseThrow(
			() -> new IllegalStateException(SSH_USERNAME.getPropertyName() + " was not configured."));
		String password = SSH_PASSWORD.get().orElseThrow(
			() -> new IllegalStateException(SSH_PASSWORD.getPropertyName() + " was not configured."));
		int port = SSH_PORT.get(2020);

		return new SshTool(username, port);
	}
}
