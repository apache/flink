...
import org.apache.flink.api.scala.{ClosureCleaner, ObjectChecker}
...
class StreamExecutionEnvironment(javaEnv: JavaEnv) {
  ...
  def addSource[T: TypeInformation](function: SourceFunction[T]): DataStream[T] = {
    ...
    val cleanFun = scalaClean(scalaCheck(function))
    val typeInfo = implicitly[TypeInformation[T]]
 ...
 *
  ...
  /**
    ...
    * Currently, check if the function was implemented by a Scala Singleton Object.
    */
  private[flink] def scalaCheck[F <: AnyRef](f: F): F = {
    if (getConfig.isScalaObjectFunctionForbidden) {
      ObjectChecker.assertScalaSingleton(f)
    }
    f
  }
}
