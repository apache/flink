...
import org.apache.flink.streaming.api.scala.async.{ResultFuture, AsyncFunction, JavaResultFutureWrapper}
...
object AsyncDataStream {
  ...
  def unorderedWait[IN, OUT: TypeInformation](
    ...
    val javaAsyncFunction = new JavaAsyncFunction[IN, OUT] {
      override def asyncInvoke(input: IN, collector: JavaResultFuture[OUT]): Unit = {
        asyncFunction.asyncInvoke(input, new JavaResultFutureWrapper(collector))
      }
    }
  ...
  def unorderedWait[IN, OUT: TypeInformation](
    ...
    val func = new JavaAsyncFunction[IN, OUT] {
      override def asyncInvoke(input: IN, collector: JavaResultFuture[OUT]): Unit = {
        ...
        cleanAsyncFunction(input, new JavaResultFutureWrapper[OUT](collector))
      }
    }
  ...
  def orderedWait[IN, OUT: TypeInformation](
    ...
    val javaAsyncFunction = new JavaAsyncFunction[IN, OUT] {
      override def asyncInvoke(input: IN, collector: JavaResultFuture[OUT]): Unit = {
        asyncFunction.asyncInvoke(input, new JavaResultFutureWrapper[OUT](collector))
      }
    }
