...
import { JobLocalService, JobWithVertex } from './job-local.service';
...
describe('JobLocalService', () => {
  ...
  it('suppresses the paired stream while no vertex is selected', () => {
    service.setJobDetail(job);
    const emissions: JobWithVertex[] = [];
    const sub = service.jobWithVertexChanges().subscribe(val => emissions.push(val));
    ...
    service.setSelectedVertex(vertex);
    expect(emissions).toEqual([{ job, vertex }]);
