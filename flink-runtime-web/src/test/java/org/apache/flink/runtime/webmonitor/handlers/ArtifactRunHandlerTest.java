...
/**
 * Tests for the {@link ArtifactRunHandler}.
 */
public class ArtifactRunHandlerTest extends TestLogger {
	...
	public void testRunJar() throws Exception {
		...
		try {
			...
			try {
				ArtifactRunHeaders headers = ArtifactRunHeaders.getInstance();
				ArtifactRunMessageParameters parameters = headers.getUnresolvedMessageParameters();
				parameters.artifactIdPathParameter.resolve(emptyJar.getFileName().toString());

				String host = clientConfig.getString(RestOptions.ADDRESS);
				int port = clientConfig.getInteger(RestOptions.PORT);

				try {
					client.sendRequest(host, port, headers, parameters, new ArtifactRunRequestBody())
						.get();
				} catch (Exception e) {
