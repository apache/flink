				...
				new Within(deadline) {
					...
					protected void run() {
						try {
							...
							// Update stats (make sure to clear -- otherwise
							// old sample will be returned)
							statsTracker.invalidateOperatorStatsCache();

							stats = triggerStatsSample(statsTracker, vertex);

							assertEquals(1, stats.getSampleId());
							assertEquals(parallelism, stats.getNumberOfSubTasks());

							// Verify that all tasks are back pressured
							for (int i = 0; i < parallelism; i++) {
								assertTrue(stats.getBackPressureRatio(i) == 0.0);
							}
