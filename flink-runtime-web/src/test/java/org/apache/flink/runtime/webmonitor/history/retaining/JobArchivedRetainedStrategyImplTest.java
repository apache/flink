...
/** Testing for {@link JobArchivedRetainedStrategyImpl}. */
class JobArchivedRetainedStrategyImplTest {

    @Test
    void testTimeToLiveBasedJobRetainedStrategy() {
        Configuration conf = new Configuration();
        JobArchivesRetainedStrategy strategy = JobArchivedRetainedStrategyImpl.createFrom(conf);
        ...
        strategy = JobArchivedRetainedStrategyImpl.createFrom(conf);
        assertThat(strategy.shouldRetain(new TestingFileStatus(), 1)).isTrue();
        assertThat(
                        strategy.shouldRetain(
                                new TestingFileStatus(
