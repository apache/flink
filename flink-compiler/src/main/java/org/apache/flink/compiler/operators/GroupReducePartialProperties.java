...
public final class GroupReducePartialProperties extends OperatorDescriptorSingle {

	private final Ordering ordering;        // ordering that we need to use if an additional ordering is requested 

	public GroupReducePartialProperties(FieldSet groupKeys, Ordering additionalOrderKeys) {
		super(groupKeys);
	...
	public DriverStrategy getStrategy() {
		return DriverStrategy.GROUP_REDUCE_PARTIAL;
	}
	...
	public SingleInputPlanNode instantiate(Channel in, SingleInputNode node) {
		...
		SingleInputPlanNode singleInputPlanNode = new SingleInputPlanNode(
				node, 
				"GroupReducePartial (" + node.getPactContract().getName() + ")", 
				in,
				DriverStrategy.GROUP_REDUCE_PARTIAL, this.keyList);
