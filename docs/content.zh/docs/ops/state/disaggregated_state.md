...
* **资源使用尖刺**：本地状态模型会在定期在checkpoint时触发 CPU 和网络 I/O 的尖刺或者 SST 文件的 compaction。
...
这在状态非常大时很有用。存算分离以一种更有效的方式存储状态，轻量地持久化或恢复状态。存算分离的好处是：
...
存算分离包括三个部分：
* **ForSt State Backend**: 一个以外部存储系统作为主存的状态后端。它还可以利用本地磁盘进行缓存和缓冲。
异步I/O模型用于读取和写入状态。有关详细信息，请参阅[ForSt State Backend]({{< ref "docs/ops/state/state_backends#the-forststatebackend" >}}).
* **New State APIs**: 引入了新的状态 APIs (State V2) 用于异步状态读取和写入，异步状态访问是存算分离克服高网络延迟的必要组件。
详细信息请参阅[New State APIs]({{< ref "docs/dev/datastream/fault-tolerance/state_v2" >}}).
* **SQL Support**: 许多算子都被重写以支持存算分离和异步状态访问。用户可以轻松地通过配置项启用这些功能。
...
{{< hint warning >}}
存算分离(disaggregated state management)仍然是实验性功能。我们正在改进此功能的性能和稳定性。APIs和配置可能会在将来的版本中更改。
{{< /hint >}}
...
```
如此一来，您可以在 SQL 作业中使用存算分离和异步状态访问。目前还未全部实现所有 SQL 算子的异步状态访问。如果使用的算子不支持，则算子会自动回退到同步状态实现。
这种情况下性能可能不是最佳的。目前支持异步状态访问的有状态算子有：
...
execution.checkpointing.dir: s3://your-bucket/flink-checkpoints
```
然后，您应该使用新的 state APIs 来编写您的 datastream 作业。有关详细信息，请参阅[状态 V2]({{< ref "docs/dev/datastream/fault-tolerance/state_v2" >}}).

## 高级调优选项

### ForSt State Backend 调优
...
然后，用户可能想将状态存在一个不同的位置，例如，一个不同的 S3 桶。您可以通过以下配置来指定主要存储位置：
```yaml
...
- Size-based: 在缓存大小超过限制时，将驱逐最旧的文件。
- Reserved-based: 在磁盘上（缓存目录所在的磁盘）剩余空间不足时，将驱逐最旧的文件。
相关配置项如下：
```yaml
...
如果有特殊需要，您可以设置以下配置来指定异步线程的数量：
 ...
 - `state.backend.forst.executor.inline-coordinator`: 是否让任务线程（主线程）作为分发线程。默认值为 true。设置为 false 会提高 CPU 使用率。
