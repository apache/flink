---
title: "存算分离(Disaggregated State Management)"
weight: 20
type: docs
aliases:
...
- 基于固定大小的 (Size-based): 在已使用缓存大小超过限制时淘汰最旧的文件。
- 基于预留空间的 (Reserved-based): 在磁盘上（缓存目录所在的磁盘）剩余空间不足时淘汰最旧的文件。
...
```
以上两个配置项可以同时生效。如果同时开启两个选项，在缓存使用量触发以上两个限制之一时，缓存会淘汰最老的文件。
