...
通过 Flink 的配置文件 `config.yaml` 来设置默认的重启策略。配置参数 *restart-strategy.type* 定义了采取何种策略。
...
通过在 `config.yaml` 中设置如下配置参数，默认启用此策略。
...
Flink 支持多种不同的故障恢复策略，该策略需要通过 Flink 配置文件 `config.yaml` 中的 *jobmanager.execution.failover-strategy*
配置项进行配置。
