...
- 从 Flink 1.15版本开始，如果需要在 Flink SQL Client 或者 [SQL Gateway]({{< ref "docs/dev/table/sql-gateway/overview" >}}) 使用 Hive 方言的话，请首先将 `FLINK_HOME/opt` 下面的 `flink-table-planner_2.12` jar 包放到 `FLINK_HOME/lib` 下，并将 `FLINK_HOME/lib`
