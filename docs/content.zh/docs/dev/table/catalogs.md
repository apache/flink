...
For sql-gateway, you can add the option `table.catalog-modification.listeners` in the [Flink configuration file]({{< ref "docs/deployment/config#flink-configuration-file" >}}) and start
...
在 SQL Gateway 中，推荐在 [Flink 配置文件]({{< ref "docs/deployment/config#flink-configuration-file" >}})中进行配置，所有的 session 可以自动使用已经创建好的 Catalog 。
