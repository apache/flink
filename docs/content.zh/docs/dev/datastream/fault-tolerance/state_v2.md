...
* `StateFuture<Void> thenAccept(Consumer<T>)`: 接受一个在状态访问完成时会被调用的 `Consumer` 作为参数。 
   它返回一个 `StateFuture<Void>`，在 `Consumer` 执行完时完成。
* `StateFuture<R> thenApply(Function<T, R>)`: 接受一个在状态访问完成时会被调用的 `Function` 作为参数。 
   返回值 `StateFuture<R>` 会使用 `Function` 的返回值会作为内部结果，在 `Function` 执行完时完成。
...
以上这些方法类似于 `CompletableFuture` 的相关方法。在此之外，注意，`StateFuture` 不提供 `get()` 方法来阻塞当前线程，
直到状态访问完成。 这是因为阻塞当前线程可能会导致递归阻塞。 `StateFuture<T>` 还提供了条件版本的 `thenAccept`, 
`thenApply`, `thenCompose` 和 `thenCombine`，用于后续逻辑依赖 状态访问返回值来选择不同分支的情景。这些方法的条件版本是：
...
* `StateFuture<Void> onNext(Consumer<T>)` : 该方法接受一个 `Consumer`作为参数，`Consumer` 会在状态访问完成时与下一个
   ...
   结果，返回值在 `Function` 执行完时完成。

我们也提供了 `StateFutureUtils` 类，其中包含一些用于处理 `StateFuture` 的实用方法。这些方法是：
...
* `StateFuture<Iterable<T>> toIterable(StateFuture<StateIterator<T>>)` : 该方法接受一个结果为 `StateIterator` 
   ...
   但没有充分的理由这样做，因为这可能会禁用延迟加载的功能。只有当进一步的计算取决于迭代器的整个数据时才有用。
...
尤其是对于不同输入元素的状态访问方法，状态访问方法可能会乱序执行。在上面的例子中，如果 `flatMap` 函数接收搭配两个不同的
输入元素 A 和 B， A 和 B 的状态访问方法会被执行。首先，元素 A 的 `asyncGet`方法被执行，然后元素 B 的 `asyncGet` 方法接着被执行，
...
方法中的用户代码都会在单个线程（任务线程）中执行。因此不会出现用户代码的并发问题。
...
* **不要混合同步和异步状态访问**
* 使用 `thenXXxx` 方法的链式调用来处理状态访问的结果，然后给出另一个状态访问或结果。将逻辑分割成多个步骤，通过 `thenXXxx` 
  方法分割。 
* 避免在用户函数（例如 `RichFlatMapFunction`） 中使用可变（mutable）成员。因为状态访问方法是乱序执行的，
  可变成员可能会以无法预测的顺序执行。相反，使用 state access 的结果来在不同步骤之间传递数据。`StateFutureUtils.completedFuture` 
  和 `thenApply` 方法可以被用来传递数据。或者一个捕获变量的容器 （`AtomicReference`），该容器在每次调用 `flatMap`时
  都会初始化，以便在 lambda 之间共享。
...
从老的 state API 迁移到新的 state API 非常方便，请采用以下步骤：
1. 在  `KeyedStream` 上调用 `enableAsyncState()` 来启动新的 state API.
2. 替换 `StateDescriptor` 为 `v2` 包下的 `StateDescriptor`。同时， 替换旧的 state handle 为新的 state handle。
3. 用新的异步 API 重写旧的 state 访问方法。
4. 使用新的 state API时，推荐使用 ForSt State Backend，它支持异步 state 访问。其他 state backend 只支持同步的 state 访问，尽管它们可以和新的 state API 一起使用。
