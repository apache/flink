...
In the figure above, users add a `PrePartition` and `PostPartition` operator in the `SupportsPreWriteTopology` topology, and redistributes the input data to the `SinkWriter`.
...
In the figure above, users add a `CollectCommit` operator in the `SupportsPreCommitTopology` topology, and collects all the commit messages from the `SinkWriter` to one subtask, then sends to the `Committer` to process them centrally, this can reduce the number of interactions with external system. 
