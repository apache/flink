...
Flink 自带的[默认 config.yaml](#default-configuration-in-flink-confyaml) 文件指定了 [`taskmanager.memory.process.size`]({{< ref "docs/deployment/config" >}}#taskmanager-memory-process-size)（*>= 1.10*）和 [`jobmanager.memory.process.size`]({{< ref "docs/deployment/config" >}}#jobmanager-memory-process-size) (*>= 1.11*)，以便与此前的行为保持一致。
...
本节描述 Flink 自带的默认 `config.yaml` 文件中的变化。
...
{{< hint warning >}}
**注意：** 使用新的默认 `config.yaml` 可能会造成各内存部分的大小发生变化，从而产生性能变化。
{{< /hint >}}
