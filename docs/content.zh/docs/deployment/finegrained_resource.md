...
When a slot request with 0.25 Core and 1GB memory arrives, Flink will select a TaskManager that has enough free resources and create
a new slot with the requested resources from it. If a slot has been freed, its resources will be returned to the available resources of the TaskManager.
...
according to [user’s configuration]({{< ref "docs/deployment/memory/mem_setup_tm" >}}#set-up-taskmanager-memory). As the resource spec of TaskManagers is pre-defined:
  ...
  - You need to make sure the resource components configured for the slot sharing group are no larger than the total resources of the TaskManager. Otherwise, your job will fail with exception.
