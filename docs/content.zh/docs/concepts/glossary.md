...
Logical Graph 是一种有向图，其中顶点是 [算子](#operator)，边定义算子的输入/输出关系，并对应于数据流或数据集。通过从 [Flink Application](#flink-application) 提交作业来创建 logical Graph 。
Logical Graph 通常也称为*数据流图*。
