...
Savepoint 已保存在 `state.savepoints.dir` 指定的路径中，该配置在 [*Flink 配置文件*]({{< ref "docs/deployment/config#flink-configuration-file" >}})
中定义，[*Flink 配置文件*]({{< ref "docs/deployment/config#flink-configuration-file" >}}) 挂载在本机的 */tmp/flink-savepoints-directory/* 目录下。
