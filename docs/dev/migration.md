...
namely **keyed** and **non-keyed** state (also sometimes called **operator** state). Both types are available to 
both operators and user-defined functions. This document will guide you through the process of migrating your Flink-1.1 
function code to Flink-1.2 (see [Function Code Migration](#function-code-migration)) and will present some important 
internal changes introduced in Flink-1.2 that concern the deprecation of the aligned window operators from Flink-1.1 
