...
它也可以是持久化的元数据，例如 Hive Metastore 中的元数据。目录提供了一个统一的API，用于管理元数据，并使其可以从 Table API 和 SQL 查询语句中来访问。
...
Flink 的 [Hive 文档]({{ site.baseurl }}/zh/dev/table/hive/index.html) 提供了有关 Hive 设置目录以及现有接口的完整详细信息。
...
更多详细信息，请参考[Flink SQL 创建 DDL]({{ site.baseurl }}/zh/dev/table/sql/create.html)。
