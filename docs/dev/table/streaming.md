...
If a record with a key, whose state has been removed before, needs to be processed, the record will be treated as if it was the first record with the respective key. By removing the state, the continuous query completely forgets that it has seen this key before. For the example above this means that the count of a `sessionId` would start again at `0`.
