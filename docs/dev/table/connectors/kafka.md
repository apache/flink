...
partitions in the topics are idle, the watermark assigner will not advance. You can alleviate this problem by setting appropriate option [table.exec.source.idle-timeout]({{ site.baseurl }}/dev/table/config.html#table-exec-source-idle-timeout).
