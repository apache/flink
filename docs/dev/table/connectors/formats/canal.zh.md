...
[Canal](https://github.com/alibaba/canal/wiki) 是一个 CDC（ ChangeLog Data Capture，变更日志数据捕获）工具，可以实时地将 MySQL 变更传输到其他系统。Canal 为变更日志提供了统一的元数据格式，并支持使用 JSON 和 [protobuf](https://developers.google.com/protocol-buffers) 序列化消息（ Canal 默认使用 protobuf ）。

FLink 支持将 Canal 的 JSON 消息解析为 Flink SQL 系统中的 INSERT/UPDATE/DELETE 消息。在许多情况下此功能都非常有用，例如：
 - 将增量数据从数据库同步到其他系统
 - 审核日志
 - 数据库的实时物化视图
 - 临时关联数据库表的变更历史等等。

*注意：未来会支持 Canal protobuf 类型消息的解析及 Canal 消息的发出。*
...
<table class="table table-bordered">
    ...
    <tr>
      ...
      <td>Boolean</td>
      <td>跳过带有分析错误的字段和行，而非失败。
      如果发生错误，则字段设置为null。</td>
    </tr>
    <tr>
...
目前，Canal 格式使用 JSON 格式进行反序列化。 有关数据类型映射的更多详细信息，请参阅 [JSON 格式文档]({% link dev/table/connectors/formats/json.zh.md %}#data-type-mapping)。
