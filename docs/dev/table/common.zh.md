...
在 `TableEnvironment` 中可以将`批数据集或`流数据集`注册成视图。结果视图的 schema 取决于注册的`批数据集或`流数据集`的数据类型。请参阅文档 [数据类型到 table schema 的映射](#mapping-of-data-types-to-table-schema)获取详细信息。

**注释：** 通过`批数据集`或`流数据集`创建的视图只能注册成零时视图。
