...
If the language tag is JAVA/SCALA, the identifier is the full classpath of the UDF. For implementation of Java/Scala UDF, see [User-defined Functions]({{ site.baseurl }}/dev/table/functions/udfs.html).

If the language tag is PYTHON, the identifier is the fully qualified name of the UDF, e.g. `pyflink.table.tests.test_udf.add`. For implementation of Python UDF, see [Python UDFs]({{ site.baseurl }}/dev/table/python/python_udfs.html).
