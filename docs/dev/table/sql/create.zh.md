...
<column_definition>:
  column_name column_type [ <in_line_constraint> ] [COMMENT column_comment]
...
Upsert table sink 需要再更新 key 上加上主键限制。
例如，对于下面的表定义：

{% highlight sql %}
CREATE TABLE Orders (
    user BIGINT PRIMARY KEY NOT ENFORCED,
    product STRING,
    order_time TIMESTAMP(3)
) WITH ( . . . );
{% endhighlight %}

您可以从一个聚合查询发送按照 `user` 列聚合的消息。
