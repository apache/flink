...
CREATE 语句用于向当前或指定的 [Catalog]({% link dev/table/catalogs.zh.md %}) 中注册表、视图或函数。注册后的表、视图和函数可以在 SQL 查询中使用。
...
可以在 [SQL CLI]({% link dev/table/sqlClient.zh.md %}) 中执行 CREATE 语句。
...
Watermark 根据 [`pipeline.auto-watermark-interval`]({% link deployment/config.zh.md %}#pipeline-auto-watermark-interval) 中所配置的间隔发出。
...
表达式 `key1=val1` 的键和值必须为字符串文本常量。请参考 [连接外部系统]({% link dev/table/connect.zh.md %}) 了解不同连接器所支持的属性。
...
Check out more details at [Catalogs]({% link dev/table/catalogs.zh.md %}).
...
如果 language tag 是 PYTHON，而当前程序是 Java／Scala 程序或者纯 SQL 程序，则需要[配置 Python 相关的依赖]({% link dev/python/table-api-users-guide/dependency_management.zh.md %}#python-dependency-in-javascala-program)。
