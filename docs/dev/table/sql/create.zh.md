...
如果 language tag 是 JAVA 或者 SCALA ，则 identifier 是 UDF 实现类的全限定名。实现 Java/Scala UDF 请参考 [自定义函数]({{ site.baseurl }}/zh/dev/table/functions/udfs.html)。

如果 language tag 是 PYTHON ，则 identifier 是 UDF 对象的全限定名，例如 `pyflink.table.tests.test_udf.add`。实现 Python UDF 请参考 [Python UDFs]({{ site.baseurl }}/zh/dev/table/python/python_udfs.html)。
