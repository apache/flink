...
有关 Kerberos 安全性 Flink 配置的更多信息，请参见[这里]({% link ops/config.zh.md %})。你也可以在[这里]({% link deployment/security/security-kerberos.zh.md %})进一步了解 Flink 如何在内部设置基于 kerberos 的安全性。
