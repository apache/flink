...
# Window Join
A window join joins the elements of two streams that share a common key and lie in the same window. These windows can be defined by using a [window assigner]({% link dev/stream/operators/windows.zh.md %}#window-assigners) and are evaluated on elements from both of the streams.
