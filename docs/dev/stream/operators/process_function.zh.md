...
For fault-tolerant state, the `ProcessFunction` gives access to Flink's [keyed state]({% link dev/stream/state/state.zh.md %}), accessible via the
`RuntimeContext`, similar to the way other stateful functions can access keyed state.

The timers allow applications to react to changes in processing time and in [event time]({% link dev/event_time.zh.md %}).
