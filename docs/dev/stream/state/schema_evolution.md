...
The current restrictions varies across different type's and state structures (et.c `ValueState`, `ListState`, etc.).

Note that the information on this page is relevant only if you are using state serializers that is
...
or `TypeInformation`, and therefore allowing Flink to infer information about the state type:
...
To evolve the schema of a given state type, you would take the following steps:
 ...
 2. Update state types in your application (e.g., modifying your Avro / POJO type schema).
...
Moreover, it is possible on restore to switch from using Avro-generated `SpecificRecord`s to `GenericRecord`s,
