...
// 激活在 job 中止后仍然保留的 externalized checkpoints
...
## Iterative Jobs 中的状态和 checkpoint

Flink 现在只提供没有 iterations 的 job 的处理保证。在 iterative job 上激活 checkpoint 会导致异常。为了在迭代程序中强制进行 checkpoint，用户需要在激活 checkpoint 时设置一个特殊的标志： `env.enableCheckpointing(interval, CheckpointingMode.EXACTLY_ONCE, force = true)`。
