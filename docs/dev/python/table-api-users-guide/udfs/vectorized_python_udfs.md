...
Also Pandas UDAF is only supported in Blink Planner.
...
my_table.groupby(my_table.a).select(my_table.a, mean_udaf(add(my_table.b)))
...
There are many ways to define a Vectorized Python aggregate functions besides defining a Python Function.
The following examples show the different ways to define a Vectorized Python aggregate function
which takes two columns of bigint as the input parameters and returns the sum of the max of them as the result.
