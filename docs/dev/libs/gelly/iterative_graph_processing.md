...
* <strong>Broadcast Variables</strong>: DataSets can be added as [Broadcast Variables]({% link dev/batch/index.md %}#broadcast-variables) to the `ComputeFunction`, using the `addBroadcastSet()` method.
...
* <strong>Broadcast Variables</strong>: DataSets can be added as [Broadcast Variables]({% link dev/batch/index.md %}#broadcast-variables) to the `ScatterFunction` and `GatherFunction`, using the `addBroadcastSetForUpdateFunction()` and `addBroadcastSetForMessagingFunction()` methods, respectively.
