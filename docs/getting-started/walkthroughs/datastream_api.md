...
It provides fine-grained control over time and state, which allows for the implementation of complex event-driven systems.
...
You can edit the `groupId`, `artifactId` and `package` if you like.
With the above parameters, Maven will create a project with all the dependencies to complete this tutorial.
After importing the project in your editor, you will see the following code. 
...
This walkthrough uses a source that produces an infinite stream of credit card transactions for you to process. 
Each transaction contains an account ID (`id`), timestamp (`timestamp`) of when the transaction occurred, and US$ amount (`amount`). 
...
The stream contains transactions from a large number of users; however, fraud occurs on a per-account basis. To detect fraud, you must ensure that the same instance of the FraudDetector processes every event for a given account.
...
You can call `StreamExecutionEnvironment#execute` to begin the execution of our Job. 
...
It also includes several constants that will helpful as you work through your implementation.
...
For the initial implementation, a fraud alert should be output for any account that makes a small transaction followed by a large transaction; where a small is less than $0.10 and large is more than $500.
To do this, the `FraudDetector` must _remember_ information across events; a large transaction is only fraudulent if the previous one was small.
Remembering information across events requires `state` and so you will implement your `FraudDetector` using a `KeyedProcessFunction`, which provides fine-grained control over state and time.

The most basic implementation would be to set a flag whenever a small transaction is processed. 
This way, when a large transaction comes through, you can check if the flag is set for that account. 
If it is, then this is fraud, otherwise, reset the flag.
This flag is what you want to store in Flink state.

The most basic type of state in Flink is  `ValueState`, a data type that provides _fault tolerant_, _managed_, _per key_ state.
...
For example, suppose you wanted to set a 24-hour timeout to your fraud detector.
...
To cancel a timer, we have to remember what time it is set for, and remembering implies state, so you will begin by creating a timer state along with your flag state.
...
class FraudDetector extends KeyedProcessFunction[Long, Transaction, Alert] {
    ...
    override def open(parameters: Configuration): Unit = {
        ...
        val timerDescriptor = new ValueStateDescriptor(
            "timer-state",
            Types.Long)
...
The timer service users to query the current time, register timers, and delete timers.
Using this, you can set a timer for 24 hours in the future every time the flag is set and store the timestamp in `timerState`.
