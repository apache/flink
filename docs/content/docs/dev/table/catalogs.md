...
The `JdbcCatalog` enables users to connect Flink to relational databases over JDBC protocol. `PostgresCatalog` and `MySQLCatalog` are the only two implementations of JDBC Catalog at the moment.
