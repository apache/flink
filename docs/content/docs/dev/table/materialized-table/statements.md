...
The FRESHNESS parameter range is INTERVAL `'<num>'` { SECOND | MINUTE | HOUR | DAY }. `'<num>'` must be a positive integer, and in FULL mode, `'<num>'` should be a common divisor of the respective time interval.
...
This clause used to fill the materialized table with data from the select query. The upstream table can be a materialized table, table, or view. The select statement supports all Flink SQL [Queries]({{< ref "docs/dev/table/sql/queries/overview" >}}).
...
Create a materialized table with a data freshness of 10 seconds and a refresh mode of CONTINUOUS:
...
Create a materialized table with a data freshness of 10 minutes and a refresh mode of FULL:
...
RESUME is used to resume the refresh pipeline of the materialized table.
...
<span class="label label-danger">Note</span>
- The REFRESH operation will start a Flink batch job to refresh the materialized table data. In CONTINUOUS mode, the materialized table needs to be paused first to avoid data inconsistency.


## Limitations

- Currently, modifying the refresh mode, data freshness, and partition fields of the materialized table is not supported, nor can the query definition of the materialized table be changed.
...
When deleting a materialized table, the background refresh pipeline will be stopped first, and then the metadata corresponding to the materialized table will be removed from the Catalog.
