...
Please see the documentation about [Table Sources & Sinks]({{< ref "docs/dev/table/sourcesSinks" >}}) for details about available sources and sinks and instructions for how to implement a custom `DynamicTableSink`.

The `Table.insertInto(String tableName)` method defines an `INSERT INTO` pipeline emitting the `Table` to a registered `TableSink`.
The method looks up the `TableSink` from the catalog by the name and validates that the schema of the `Table` is identical to the schema of the `TableSink`.
The pipeline can be explained with `TablePipeline.explain()` and executed invoking `TablePipeline.execute()`.
