...
When using the [SQL Gateway configured with HiveServer2 Endpoint]({{<ref "docs/sql/interfaces/sql-gateway/hiveserver2">}}), the dialect will be Hive dialect by default, so you don't need to do anything if you want to use Hive dialect. But you can still
