...
The default restart strategy is set via Flink's configuration file `config.yaml`.
...
This strategy is enabled as default by setting the following configuration parameter in `config.yaml`.
...
*jobmanager.execution.failover-strategy* in Flink's configuration file `config.yaml`.
