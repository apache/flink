...
Then before starting the playground, create the checkpoint and savepoint directories on the Docker host machine and need confirm that these directories has the write access (these volumes are mounted by the jobmanager and taskmanager, as specified in docker-compose.yaml):
