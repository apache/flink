...
With Reactive Mode enabled, if the operator specifies the degree of parallelism, Flink supports a fixed degree of parallelism for the operator. In other words, the maximum parallelism of an operator is equal to its specified parallelism, and the minimum parallelism is equal to the number of slots available to the cluster. This is especially useful in certain scenarios where you want to fix the degree of operator parallelism.
#### Recommendations
