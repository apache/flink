...
The archiving of completed jobs happens on the JobManager, which uploads the archived job information to a file system directory. You can configure the directory to archive completed jobs in `config.yaml` by setting a directory via `jobmanager.archive.fs.dir`.
