...
To allow for easy adoption, you can use the same configuration keys in `config.yaml` as in Hadoop's `core-site.xml`
...
There are some required configurations that must be added to `config.yaml` (**Other configurations defined in Hadoop OSS documentation are advanced configurations which used by performance tuning**):
...
An alternative `CredentialsProvider` can also be configured in the `config.yaml`, e.g. 
```yaml
