...
The underlying Hadoop file system can be configured using the [Hadoop configuration keys](https://github.com/GoogleCloudDataproc/hadoop-connectors/blob/v2.2.18/gcs/CONFIGURATION.md) for `gcs-connector` by adding the configurations to your `config.yaml`.

For example, `gcs-connector` has a `fs.gs.http.connect-timeout` configuration key. If you want to change it, you need to set `gs.http.connect-timeout: xyz` in `config.yaml`. Flink will internally translate this back to `fs.gs.http.connect-timeout`. 
...
`flink-gs-fs-hadoop` can also be configured by setting the following options in `config.yaml`:
