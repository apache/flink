...
Flink's network connections can be secured via SSL. Please refer to the [SSL Setup Docs]({{< ref "docs/security/ssl" >}}) for detailed setup guide and background.
...
Please refer to the [Flink and Kerberos Docs]({{< ref "docs/security/kerberos" >}}) for a setup guide and a list of external system to which Flink can authenticate itself via Kerberos.
