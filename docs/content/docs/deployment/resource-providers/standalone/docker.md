...
Options set via dynamic properties overwrite the options from `config.yaml`.
...
the same way as in the `config.yaml`. `FLINK_PROPERTIES` takes precedence over configurations in `config.yaml`.

### Via config.yaml

The configuration files (`config.yaml`, logging, hosts etc) are located in the `/opt/flink/conf` directory in the Flink image.
...
The `config.yaml` file must have write permission so that the Docker entry point script can modify it in certain cases.
{{< /hint >}}
