...
Besides passing [configuration]({{< ref "docs/deployment/config" >}}) via the `conf/config.yaml` file, you can also pass any configuration at submission time to the `./bin/yarn-session.sh` client using `-Dkey=value` arguments.
