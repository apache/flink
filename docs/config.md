...
(DEFAULT: 256). You may have to increase the heap size for the JobManager if you are running
very large applications (with may operators), or if you are keeping a long history of them.
...
large as possible (DEFAULT: 512). If the cluster is exclusively running Flink,
