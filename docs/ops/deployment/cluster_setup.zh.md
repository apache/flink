...
Set the `jobmanager.rpc.address` key to point to your master node. You should also define the maximum amount of main memory Flink is allowed to allocate on each node by setting the `master.memory.process.size` and `taskmanager.memory.process.size` keys.
 ...
 * the amount of available memory per Flink Master (`master.memory.process.size`),
