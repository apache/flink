...
Mesos setups following [this documentation]({% link deployment/resource-providers/mesos.md %}) currently behave very much like the a
...
[`classloader.resolve-order`]({% link deployment/config.md %}#classloader-resolve-order) in the Flink config to `parent-first`
...
[`classloader.parent-first-patterns-default`]({% link deployment/config.md %}#classloader-parent-first-patterns-default) and
[`classloader.parent-first-patterns-additional`]({% link deployment/config.md %}#classloader-parent-first-patterns-additional).
...
to verify this (set [`classloader.resolve-order: parent-first`]({% link deployment/config.md %}#classloader-resolve-order) in the Flink config) or exclude
the library from inverted classloading (set [`classloader.parent-first-patterns-additional`]({% link deployment/config.md %}#classloader-parent-first-patterns-additional)
in the Flink config).
