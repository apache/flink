...
<table class="table table-bordered">
  ...
  <tbody>
    <tr>
      ...
      <td>The latency distributions from a given source (subtask) to an operator subtask (in milliseconds), depending on the <a href="{% link deployment/config.md %}#metrics-latency-granularity">latency granularity</a>.</td>
      <td>Histogram</td>
    </tr>
    <tr>
...
[Flink configuration]({% link deployment/config.md %}#metrics-latency-interval) or `ExecutionConfig`.
...
be controlled in the [Flink configuration]({% link deployment/config.md %}#metrics-latency-interval). For the highest 
...
Metrics can be queried through the [Monitoring REST API]({% link monitoring/monitoring/rest_api.md %}).
