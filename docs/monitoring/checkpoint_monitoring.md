...
- **Latest Failed Checkpoint**: The latest failed checkpoint.
- **Latest Savepoint**: The latest triggered savepoint with its external path.
...
- **Status**: The current status of the checkpoint, which is either *In Progress*, *Completed*, or *Failed*. If the triggered checkpoint is a savepoint, you will see a `[savePoint]` symbol.
...
When you click on a *expand* icon for a checkpoint, you get a Minimum/Average/Maximum summary over all its operators and also the detailed numbers per single subtask.
