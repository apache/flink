...
关于Python执行环境的要求，请参考Python Table API[环境安装]({% link dev/python/getting-started/installation.md %})。

## 创建一个Python Table API项目

首先，使用您最熟悉的IDE创建一个Python项目，然后安装PyFlink包，请参考[PyFlink安装指南]({% link dev/python/getting-started/installation.zh.md %}#installation-of-pyflink)了解详细信息。
