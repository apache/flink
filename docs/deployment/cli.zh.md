...
-   Run example program using a [per-job YARN cluster]({% link deployment/resource-providers/yarn.zh.md %}#run-a-single-flink-job-on-hadoop-yarn) with 2 TaskManagers:
...
-   提交一个运行在有两个TaskManager的[per-job YARN cluster]({% link deployment/resource-providers/yarn.zh.md %}#run-a-single-flink-job-on-hadoop-yarn)的Python Table的作业:
