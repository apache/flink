...
Application 模式允许用户创建单个镜像，其中包含他们的作业和 Flink 运行时，该镜像将按需自动创建和销毁集群组件。Flink 社区提供了可以构建[多用途自定义镜像]({% link deployment/resource-providers/docker.zh.md %}#customize-flink-image)的基础镜像。
