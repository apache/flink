...
The [query service port]({{site.baseurl}}/deployment/config.html#metrics-internal-query-service-port) can be configured in `flink-conf.yaml`.

### Granularity of latency metrics

The default granularity for latency metrics has been modified.
To restore the previous behavior users have to explicitly set the [granularity]({{site.baseurl}}/deployment/config.html#metrics-latency-granularity) to `subtask`.
...
To restore the previous default behavior users have to configure the [latency interval]({{site.baseurl}}/deployment/config.html#metrics-latency-interval) in `flink-conf.yaml`.
...
Consequently, we encourage our users to enable [local recovery]({{site.baseurl}}/deployment/config.html#state-backend-local-recovery) in `flink-conf.yaml`.
...
Thus, in order to run a cluster in HA mode, one needs to set a different [cluster id]({{site.baseurl}}/deployment/config.html#high-availability-cluster-id) for each job/cluster. 
