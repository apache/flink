...
to be loaded through [plugins]({% link deployment/filesystems/index.md %}#pluggable-file-systems)
...
Failover Strategy]({% link dev/task_failure_recovery.md %}#restart-pipelined-region-failover-strategy)
...
documentation]({% link ops/state/large_state_tuning.md %}#tuning-rocksdb-memory).
...
Connectors documentation]({% link dev/table/connect.md %}#table-connectors)
...
Users are advised to use the [new Catalog API]({% link dev/table/catalogs.md %}#catalog-api).
...
[_"Task Failure Recovery"_]({% link dev/task_failure_recovery.md %})
for details.
...
[Background cleanup of expired state with TTL]({% link dev/stream/state/state.md %}#cleanup-of-expired-state)
