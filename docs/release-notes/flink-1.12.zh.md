...
The `CheckpointConfig#setPreferCheckpointForRecovery` method has been deprecated, because using checkpoints for recovery can lead to data loss.
...
#### RocksDB optimizeForPointLookup results in missing time windows [FLINK-17800](https://issues.apache.org/jira/browse/FLINK-17800)

By default we will set `setTotalOrderSeek` to true for RocksDB's `ReadOptions`, to prevent user from miss using `optimizeForPointLookup`. In the meantime we support customizing `ReadOptions` through `RocksDBOptionsFactory`. Please set `setTotalOrderSeek` back to false if any performance regressions are observed (this is not expected based on our testing). 

#### Custom OptionsFactory settings seem to have no effect on RocksDB [FLINK-18242](https://issues.apache.org/jira/browse/FLINK-18242)

The deprecated `OptionsFactory` and `ConfigurableOptionsFactory` classes are removed, please use `RocksDBOptionsFactory` and `ConfigurableRocksDBOptionsFactory` instead. Please also recompile your application codes if any class extending `DefaultConfigurableOptionsFactory`. 