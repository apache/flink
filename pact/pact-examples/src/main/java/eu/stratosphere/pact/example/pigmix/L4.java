...
public class L4 implements PlanAssembler{
	...
	public static class ProjectPageViews extends MapStub
	{
	 ...
	 public static class Group extends ReduceStub{
			...
			public void reduce(Iterator<PactRecord> records, Collector out)
					throws Exception {
				PactRecord rec = null;
				 HashSet<PactString> hash = new HashSet<PactString>();
		         while (records.hasNext()) {
		        	 rec = records.next();
		             hash.add(rec.getField(1, PactString.class));
		         }
		         out.collect(new PactRecord(rec.getField(0, PactString.class), new PactInteger(hash.size())));
	 ...
	 /* (non-Javadoc)
		...
		public Plan getPlan(String... args)
		{
			...
			Plan plan = new Plan(sink, "L4 foreach + nested distinct");
			return plan;
		}
}
