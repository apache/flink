...
public abstract class AbstractMinimalTask extends AbstractTask {
  ...
  protected TypeSerializer<? extends Value>[] accessors;
  protected TypeSerializer<? extends Value>[] outputAccessors;
  ...
  public final void registerInputOutput() {
    ...
    try {
      classLoader = LibraryCacheManager.getClassLoader(getEnvironment().getJobID());
    } catch (IOException ioe) {
      throw new RuntimeException("The ClassLoader for the user code could not be instantiated from the library cache.",
          ioe);
    }
  ...
  protected void initInputs() {
    ...
    @SuppressWarnings("unchecked")
    final TypeSerializer<? extends Value>[] accessors = new TypeSerializer[numInputs];
    ...
    for (int i = 0; i < numInputs; i++) {
      ...
      switch (shipStrategy) {
        ...
        case PARTITION_LOCAL_RANGE:
          dp = DistributionPattern.BIPARTITE;
          break;
        case PARTITION_HASH:
      ...
      @SuppressWarnings("unchecked")
      Class<? extends TypeSerializer<? extends Value>> clsAccessor =
        (Class<? extends TypeSerializer<? extends Value>>)
            getEnvironment().getTaskConfiguration().getClass(TYPE + i, PactRecordAccessorsV2.class);
  ...
  protected void initOutputs()
  {
    ...
    outputAccessors = new TypeSerializer[numOutputs];
    ...
    for (int i = 0; i < numOutputs; i++) {
      ...
      final OutputEmitterV2 oe = new OutputEmitterV2(
          ...
          (TypeSerializer<Value>) outputAccessors[i]);
  ...
  protected void setOutputAccessors() {
    ...
    // create a writer for each output
    for (int i = 0; i < numOutputs; i++) {
      ChannelSelector selector = output.getWriters().get(i).getOutputGate().getChannelSelector();
      ((OutputEmitterV2) selector).setTypeAccessor((TypeAccessorsV2<Value>) outputAccessors[i]);
    }
  }
