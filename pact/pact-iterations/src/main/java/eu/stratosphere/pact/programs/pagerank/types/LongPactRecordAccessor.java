...
public class LongPactRecordAccessor implements TypeAccessorsV2<PactRecord> {
   ...
   * @param keyFields
   */
  public LongPactRecordAccessor(int[] keyFields, Class<? extends Key>[] keyTypes)
  {
   ...
   */
  private LongPactRecordAccessor(int[] keyFields, Key[] keys, int[] normalKeyLengths,
  ...
  @Override
  public int hash(PactRecord record)
  {
    final long vid = record.getField(1, number).getValue();
   ...
   */
  @Override
  public TypeAccessorsV2<PactRecord> duplicate() {
    return new LongPactRecordAccessor(this.keyFields, this.keyHolders, this.normalizedKeyLengths,
