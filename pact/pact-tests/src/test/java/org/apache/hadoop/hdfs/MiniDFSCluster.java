...
public class MiniDFSCluster {
	 ...
	 * <p>
	 * The caller must manage the creation of NameNode and DataNode directories and have already set dfs.name.dir and
	 * dfs.data.dir in the given conf.
	 * 
	 * @param conf
	 ...
	 */
	public MiniDFSCluster(Configuration conf, int numDataNodes, StartupOption nameNodeOperation)
																								throws IOException {
	 ...
	 */
	public MiniDFSCluster(Configuration conf, int numDataNodes, boolean format, String[] racks)
																								throws IOException {
	...
	public MiniDFSCluster(int nameNodePort, Configuration conf, int numDataNodes, boolean format,
			...
			String hosts[], long[] simulatedCapacities)
														throws IOException {
		this.conf = conf;
		try {
	...
	boolean corruptBlockOnDataNode(int i, String blockName) throws Exception {
		...
		for (int dn = i * 2; dn < i * 2 + 2; dn++) {
			...
			if (blockFile.exists()) {
				// Corrupt replica by writing random memory into replica
