...
//		nationInput.getCompilerHints().setAvgNumValuesPerKey(1);
		...
		MapContract filterPart = new MapContract.Builder(PartFilter.class)
			.name("filterParts")
			.build();
		...
		MapContract mapPartsupp = new MapContract.Builder(PartsuppMap.class)
			.name("mapPartsupp")
			.build();
		mapPartsupp.setDegreeOfParallelism(this.degreeOfParallelism);

		/* Map to extract the year from order: */
		MapContract mapOrder = new MapContract.Builder(OrderMap.class)
			.name("mapOrder")
			.build();
		...
		MapContract mapLineItem = new MapContract.Builder(LineItemMap.class)
			.name("proj.Partsupp")
			.build();
		...
		MapContract mapSupplier = new MapContract.Builder(SupplierMap.class)
			.name("proj.Partsupp")
			.build();
		mapSupplier.setDegreeOfParallelism(this.degreeOfParallelism);

		/* Equijoin on partkey of part and partsupp: */
		MatchContract partsJoin = new MatchContract.Builder(PartJoin.class, PactInteger.class, 0, 0)
			.name("partsJoin")
			.build();
		...
		MatchContract orderedPartsJoin =
			new MatchContract.Builder(OrderedPartsJoin.class, PactInteger.class, 0, 0)
		...
		MatchContract suppliersJoin =
			new MatchContract.Builder(SuppliersJoin.class, PactInteger.class, 0, 0)
			.name("suppliersJoin")
			.build();
		...
		MatchContract filteredPartsJoin =
			new MatchContract.Builder(FilteredPartsJoin.class, IntPair.class, 0, 0)
		...
		MatchContract partListJoin =
			new MatchContract.Builder(PartListJoin.class, PactInteger.class , 0, 0)
			.name("partlistJoin")
			.build();
		filteredPartsJoin.setDegreeOfParallelism(this.degreeOfParallelism);
		...
		ReduceContract sumAmountAggregate =
			new ReduceContract.Builder(AmountAggregate.class, StringIntPair.class, 0)
			.name("groupyBy")
			.build();
