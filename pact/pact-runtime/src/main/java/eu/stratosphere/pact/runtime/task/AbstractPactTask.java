...
public abstract class AbstractPactTask<T extends Stub> extends AbstractTask {
	 ...
	 * 
	 * @throws Exception
	 *         Exceptions may be forwarded and signal task failure.
	 */
	...
	protected void initInputs() {
		...
		for (int i = 0; i < numInputs; i++) {
			...
			case SFR:
				...
				break;
			default:
				throw new RuntimeException("Invalid input ship strategy provided for input " + i + ": "
					+ shipStrategy.name());
			}
	 ...
	 */
	public static OutputCollector getOutputCollector(AbstractInvokable task, TaskConfig config, ClassLoader cl,
			int numOutputs) {
		...
		for (int i = 0; i < numOutputs; i++) {
			...
			final OutputEmitter oe = (keyPositions == null || keyClasses == null) ? new OutputEmitter(strategy)
			...
			if (strategy == ShipStrategy.BROADCAST) {
				if (task instanceof AbstractTask) {
					...
					output.addWriter(new BroadcastRecordWriter<PactRecord>((AbstractInputTask<?>) task,
						PactRecord.class));
				}
			} else {
	 ...
	 */
	public static Collector initOutputs(AbstractInvokable nepheleTask, ClassLoader cl, TaskConfig config,
			List<ChainedTask> chainedTasksTarget) {
