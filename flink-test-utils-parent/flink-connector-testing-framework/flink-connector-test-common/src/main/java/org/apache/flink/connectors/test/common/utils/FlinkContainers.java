...
public class FlinkContainers extends ExternalResource {
	...
	protected void before() throws Throwable {
		...
		// Launch JM
		jobManager.start();
		LOG.debug("Flink Job Manager is running on {}:{}", getJobManagerHost(), getJobManagerRESTPort());
	...
	protected void after() {
		...
		if (!keepTestScene) {
			...
			LOG.debug("Workspace directory is kept at {}", workspaceDirOutside.getRoot().getAbsolutePath());
			LOG.debug("Checkpoint directory is kept at {}", checkpointDirOutside.getRoot().getAbsolutePath());
		}
		jobManager.stop();
	...
	public static final class Builder {
		...
		private static final String FLINK_IMAGE_NAME = "flink:1.12-SNAPSHOT";
