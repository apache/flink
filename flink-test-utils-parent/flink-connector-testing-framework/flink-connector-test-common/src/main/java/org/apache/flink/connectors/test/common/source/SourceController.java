...
/**
 * A wrapped controller for controlling {@link ControllableSource}, which handles Java RMI
 * connecting and wraps all RPC methods for pre-condition checking and logging.
 */
public class SourceController implements SourceControlRpc {
    ...
    public void connect() {
        ...
        for (Integer port : potentialPorts) {
            try {
                stub =
                        (SourceControlRpc)
                                LocateRegistry.getRegistry(host, port).lookup("SourceControl");
                actualRMIPort = port;
                break;
                ...
                throw new IllegalStateException(
                        "Error when getting registry on " + host + ":" + port, e);
            }
        }

        if (stub == null || actualRMIPort == -1) {
            throw new IllegalStateException(
                    "Cannot find any controllable source among task managers");
        }

        LOG.info("Connected to controllable source at {}:{}", host, actualRMIPort);

        // Because of the mechanism of Java RMI, host and port registered in RMI registry would be
        // LOCAL inside docker,
        // which is not accessible on docker host / testing framework.
        // So we "hack" into the dynamic proxy object created by Java RMI to correct the port number
        // using reflection.
        ...
        try {
            FieldUtils.writeField(
                    FieldUtils.readField(
                            FieldUtils.readField(
                                    FieldUtils.readField(
                                            FieldUtils.readField(stub, "h", true), "ref", true),
                                    "ref",
                                    true),
                            "ep",
                            true),
    ...
    public void connect(Duration timeout) {
        ...
        if (!connected) {
            throw new IllegalStateException(
                    "Cannot connect to controllable source within " + timeout);
        }
    }
