...
import com.github.fommil.netlib.ARPACK.{getInstance => arpack}
...
object EigenSolver {
  ...
  /**
    ...
    * @param k
    * @param tol
    * @param maxIter The maximum number of Arnoldi update iterations. Suggest value is 300.
    ...
    *       for more details).
    */
  def solve(A: DenseMatrix, k: Int, tol: Double, maxIter: Int): (DenseVector, DenseMatrix) = {
    val n = A.numCols()
    require(k <= n)

    /**
      * NOTE: The following codes are borrowed from SPARK MLLib
      */
    ...
    val G = MatVecOp.toBreezeMatrix(A)
    val ido = new intW(0)
    val info = new intW(0)
    ...
    (MatVecOp.fromBreezeVector(BDV[Double](sortedEigenPairs.map(_._1))), MatVecOp.fromBreezeMatrix(sortedU))
  }
}
