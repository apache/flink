...
public class GroupingTest extends SopremoTest<Grouping> {
	@Override
	protected Grouping createDefaultInstance(int index) {
		Grouping aggregation = new Grouping(new ConstantExpression(index), null, null);
		return aggregation;
	}
	...
	public void shouldGroupTwoSources() {
		...
		Grouping aggregation = new Grouping(transformation, sopremoPlan.getInputOperators(0, 2));
	...
	public void shouldGroupThreeSources() {
		...
		Grouping aggregation = new Grouping(transformation, sopremoPlan.getInputOperators(0, 3));
		aggregation.setKeyProjection(0, createPath("dept"));
		aggregation.setKeyProjection(1, createPath("did"));
	...
	public void shouldGroupWithSingleSource() {
		...
		Grouping aggregation = new Grouping(transformation, sopremoPlan.getInputOperator(0));
		aggregation.setKeyProjection(0, createPath("dept"));
	...
	public void shouldPerformSimpleGroupBy() {
		...
		Grouping aggregation = new Grouping(BuiltinFunctions.COUNT.asExpression(), sopremoPlan.getInputOperator(0));
		sopremoPlan.getOutputOperator(0).setInputs(aggregation);
