...
public class TwoSourceJoin extends TwoSourceJoinBase<TwoSourceJoin> {
	...
	private EvaluationExpression condition;
	...
	public TwoSourceJoin withCondition(EvaluationExpression condition) {
		...
		if (this.condition instanceof ComparativeExpression) {
			...
			case EQUAL:
				...
				return new OuterJoin().
					withKeyExpression(0, expressions[0]).
					withKeyExpression(1, expressions[1]).
