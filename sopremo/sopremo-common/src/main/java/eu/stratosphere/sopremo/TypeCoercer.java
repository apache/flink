...
public class TypeCoercer {
	...
	private Map<Class<? extends JsonNode>, Map<Class<? extends JsonNode>, Coercer>> coercers = new IdentityHashMap<Class<? extends JsonNode>, Map<Class<? extends JsonNode>, Coercer>>();
	...
	private void addSelfCoercers() {
		Coercer selfCoercer = new Coercer() {
			@Override
			public JsonNode coerce(JsonNode node) {
				return node;
			}
		};
		for (Entry<Class<? extends JsonNode>, Map<Class<? extends JsonNode>, Coercer>> toCoercers : this.coercers
			.entrySet())
			toCoercers.getValue().put(toCoercers.getKey(), selfCoercer);
	}

	private void addNumericCoercers(Map<Class<? extends JsonNode>, Map<Class<? extends JsonNode>, Coercer>> coercers) {
		coercers.put(NumericNode.class, new IdentityHashMap<Class<? extends JsonNode>, TypeCoercer.Coercer>());
		Class<? extends NumericNode>[] NUMERIC_TYPES = (Class<? extends NumericNode>[]) new Class<?>[] { IntNode.class,
		...
		for (final Class<? extends NumericNode> numericType : NUMERIC_TYPES) {
			IdentityHashMap<Class<? extends JsonNode>, TypeCoercer.Coercer> typeCoercers = new IdentityHashMap<Class<? extends JsonNode>, TypeCoercer.Coercer>();
	...
	private Map<Class<? extends JsonNode>, Coercer> getToArrayCoercers() {
		...
		for (Class<? extends JsonNode> arrayType : ARRAY_TYPES)
	...
	private Map<Class<? extends JsonNode>, Coercer> getToStringCoercers() {
		Map<Class<? extends JsonNode>, TypeCoercer.Coercer> toStringCoercers = new IdentityHashMap<Class<? extends JsonNode>, TypeCoercer.Coercer>();
	...
	public void setCoercer(Class<? extends JsonNode> from, Class<? extends JsonNode> to, Coercer coercer) {
	...
	@SuppressWarnings("unchecked")
	public <T extends JsonNode> T coerce(JsonNode node, Class<T> targetType, T defaultValue) {
		Map<Class<? extends JsonNode>, Coercer> toCoercer = this.coercers.get(targetType);
	...
	@SuppressWarnings("unchecked")
	public <T extends JsonNode> T coerce(JsonNode node, T defaultValue) {
		return this.coerce(node, (Class<T>) defaultValue.getClass(), defaultValue);
	}

	public <T extends JsonNode> T coerce(JsonNode node, Class<T> targetType) {
		T result = this.coerce(node, targetType, null);
		if (result == null)
