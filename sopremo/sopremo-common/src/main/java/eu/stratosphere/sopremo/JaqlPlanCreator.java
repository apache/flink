...
public class JaqlPlanCreator extends PlanCreator {
	...
	private TypeSpecificHandler<Expr, List<Operator>, TypeHandler<Expr, List<Operator>>> exprConverter = new TypeSpecificHandler<Expr, List<Operator>, TypeHandler<Expr, List<Operator>>>();
	...
	private void initTransformationConverter() {
		...
		transformationConverter.register(NameValueBinding.class, new TransformationConverter<NameValueBinding>() {
			...
			public ValueTransformation convert(NameValueBinding expr, List<Mapping> childTransformations) {
				return new ValueTransformation(((ConstExpr) expr.nameExpr()).value.toString(), parsePath(expr
					.valueExpr()));
			}
		});
	...
	private void initExprConverter() {
	...
	protected Transformation parseTransformation(TransformExpr expr) {
		return (Transformation) transformationConverter.handleRecursively(EXPR_NAVIGATOR, expr);
	}
	...
	private List<Operator> convertSubtree(Expr expr) {
		return this.exprConverter.handleRecursively(EXPR_NAVIGATOR, expr);
	}
