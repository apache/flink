...
public class ReflectUtil {
	 ...
	 */
	public static BoundType resolveParameterizedSuperclass(BoundType type, ParameterizedType superclass) {
		...
		for (int index = 0; index < actualTypeArguments.length; index++) {
			if (actualTypeArguments[index] instanceof TypeVariable<?>)
				...
				arguments[index] = resolveParameterizedSuperclass(type, (ParameterizedType) actualTypeArguments[index]);
			else
	...
	public static BoundType getBindingOfSuperclass(Class<?> type, Class<?> superClass) {
		...
		if (hierarchy.isEmpty())
			return BoundType.of(superClass, getStaticBoundTypes((ParameterizedType) type.getGenericSuperclass()));
	...
	private static BoundType resolvePartially(BoundType boundType, List<Type> hierarchy) {
		...
		return resolvePartially(resolveParameterizedSuperclass(boundType, (ParameterizedType) type),
			hierarchy.subList(1, hierarchy.size()));
	}

	private static BoundType getBinding(BoundType type, ParameterizedType superclass) {
		Type[] actualTypeArguments = superclass.getActualTypeArguments();
		BoundType[] arguments = new BoundType[actualTypeArguments.length];
		for (int index = 0; index < actualTypeArguments.length; index++) {
			if (actualTypeArguments[index] instanceof TypeVariable<?>)
