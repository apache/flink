...
/**
 * Tests for the entropy injection in the {@link FlinkS3FileSystem}.
 */
public class S3FileSystemEntropyTest {

	@Test
	public void testEmptyPath() throws Exception {
		Path path = new Path("hdfs://localhost:12345");
		FlinkS3FileSystem fs = createFs("test", 4);
	...
	public void testFullUriNonMatching() throws Exception {
		...
		FlinkS3FileSystem fs = createFs("_entropy_key_", 4);
	...
	public void testFullUriMatching() throws Exception {
		...
		FlinkS3FileSystem fs = createFs("s0mek3y", 8);
	...
	public void testEntropyNotFullSegment() throws Exception {
		...
		FlinkS3FileSystem fs = createFs("_entropy_key_", 3);
	...
	public void testWriteOptionWithEntropy() throws Exception {
		...
		Path path = new Path("s3://hugo@myawesomehost:55522/path/s0mek3y/the/file");
		FlinkS3FileSystem fs = new FlinkS3FileSystem(underlyingFs, "s0mek3y", 11);
	...
	private static FlinkS3FileSystem createFs(String entropyKey, int entropyLen) {
		return new FlinkS3FileSystem(mock(FileSystem.class), entropyKey, entropyLen);
	}
