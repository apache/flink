...
public class HdfsBehaviorTest extends FileSystemBehaviorTestSuite {
	...
	public static void createHDFS() throws Exception {
		...
		org.apache.hadoop.fs.FileSystem hdfs = HDFS_CLUSTER.getFileSystem();
		FS = new HadoopFileSystem(hdfs);

		BASE_PATH = new Path(hdfs.getUri().toString() + "/tests");
	}

	@AfterClass
	public static void destroyHDFS() throws Exception {
		if (HDFS_CLUSTER != null) {
			HDFS_CLUSTER.getFileSystem().delete(new org.apache.hadoop.fs.Path(BASE_PATH.toUri()), true);
