...
import org.apache.flink.fs.common.util.BackPressuringExecutor;
 ...
 *
 * <p>This implementation makes heavy use of MultiPart Uploads in OSS to persist
...
public class OSSRecoverableWriter implements RecoverableWriter {
	...
	private final FunctionWithException<File, RefCountedFile, IOException> cachedFileCreator;

	public OSSRecoverableWriter(OSSAccessor ossAccessor,
								long ossUploadPartSize,
								int streamConcurrentUploads,
								Executor executor,
								FunctionWithException<File, RefCountedFile, IOException> cachedFileCreator) {
	...
	@Override
	public OSSRecoverableFsDataOutputStream recover(ResumeRecoverable resumable) throws IOException {
		final OSSRecoverable recoverable = (OSSRecoverable) resumable;
		OSSRecoverableMultipartUpload upload = new OSSRecoverableMultipartUpload(
		...
		return new OSSRecoverableFsDataOutputStream(ossUploadPartSize, cachedFileCreator, upload, recoverable.getNumBytesInParts());
	}
	...
	private Optional<File> recoverInProgressPart(OSSRecoverable recoverable) throws IOException {
		...
		final RefCountedFile refCountedFile = cachedFileCreator.apply(null);
		final File file = refCountedFile.getFile();

		ossAccessor.getObject(objectKey, file.getAbsolutePath(), recoverable.getLastPartObjectLength());
