...
public class OSSRecoverableSerializer implements SimpleVersionedSerializer<OSSRecoverable> {
	...
	public byte[] serialize(OSSRecoverable ossRecoverable) throws IOException {
		...
		/**
		 * magic number
		 * object name length + object name bytes
		 * upload id length + upload id bytes
		 * etags length + (part number + etag length + etag bytes)?
		 * number bytes of parts
		 * last part bytes length + last part bytes
		 * last part object length
		 */
		final byte[] resultBytes = new byte[
				Integer.BYTES +
				Integer.BYTES + objectBytes.length +
