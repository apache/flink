...
public class OSSRecoverableFsDataOutputStream extends RecoverableFsDataOutputStream {
	...
	private FunctionWithException<File, RefCountedFile, IOException> cachedFileCreator;
	...
	public OSSRecoverableFsDataOutputStream(
		...
		long sizeBeforeCurrentPart) throws IOException {
		this.ossUploadPartSize = ossUploadPartSize;
		this.cachedFileCreator = cachedFileCreator;
		this.upload = upload;

		if (upload.getIncompletePart().isPresent()) {
			this.fileStream = RefCountedBufferingFileStream.restore(this.cachedFileCreator, upload.getIncompletePart().get());
		} else {
