...
import org.apache.flink.fs.common.util.RefCountedFSOutputStream;
...
public class OSSRecoverableMultipartUpload {
	...
	public void uploadPart(RefCountedFSOutputStream file) throws IOException {
		...
		file.retain();
		uploadThreadPool.execute(new UploadTask(ossAccessor, objectName, uploadId, numberOfRegisteredParts, file, future));
	}
	...
	public OSSRecoverable getRecoverable(RefCountedFSOutputStream file) throws IOException {
		...
		if (file == null) {
			return new OSSRecoverable(uploadId, objectName, completeParts, null, expectedSizeInBytes, 0);
		} else {
			return new OSSRecoverable(uploadId, objectName, completeParts, incompletePartObjectName, expectedSizeInBytes, file.getPos());
		}
	}
	...
	private static class UploadTask implements Runnable {
		...
		public void run() {
			try {
				PartETag partETag = ossAccessor.uploadPart(file.getInputFile(), objectName, uploadId, partNumber);
