...
public class OSSCommitter implements RecoverableFsDataOutputStream.Committer {
	...
	public void commitAfterRecovery() throws IOException {
		if (totalLength > 0L) {
			try {
				LOG.info("Trying to commit after recovery {} with multi-part upload ID {}", objectName, uploadId);
				ossAccessor.completeMultipartUpload(objectName, uploadId, partETags);
			} catch (Exception e) {
				LOG.info("Failed to commit after recovery {} with multi-part upload ID {}. " +
					"exception {}", objectName, uploadId, e);
			}
		} else {
			LOG.debug("No data to commit for file: {}", objectName);
		}
