...
public class OSSRecoverableSerializerTest {
		...
		return new TypeSafeMatcher<OSSRecoverable>() {
			...
			protected boolean matchesSafely(OSSRecoverable actualRecoverable) {
				...
				return Objects.equals(expectedRecoverable.getObjectName(), actualRecoverable.getObjectName())
					&& Objects.equals(expectedRecoverable.getUploadId(), actualRecoverable.getUploadId())
					&& expectedRecoverable.getNumBytesInParts() == actualRecoverable.getNumBytesInParts()
					&& Objects.equals(expectedRecoverable.getLastPartObject(), actualRecoverable.getLastPartObject())
					&& expectedRecoverable.getLastPartObjectLength() == actualRecoverable.getLastPartObjectLength()
					&& compareLists(expectedRecoverable.getPartETags(), actualRecoverable.getPartETags());
			}
	...
	private static OSSRecoverable createOSSRecoverable(boolean withIncompletePart, int... partNumbers) {
		...
		if (withIncompletePart) {
			return new OSSRecoverable(TEST_UPLOAD_ID, TEST_OBJECT_NAME, etags, INCOMPLETE_OBJECT_NAME, 12345L, 54321L);
		} else {
