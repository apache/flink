...
public class OSSRecoverableMultipartUploadTest {
	...
	public void before() throws IOException {
		...
		ossAccessor = new OSSAccessor((AliyunOSSFileSystem) ((HadoopFileSystem) fs).getHadoopFileSystem());

		uploadId = ossAccessor.startMultipartUpload(ossAccessor.pathToObject(objectPath));

		completeParts = new ArrayList<>();

		uploader = new OSSRecoverableMultipartUpload(
	...
	public void testUploadSinglePart() throws IOException {
		...
		ossAccessor.completeMultipartUpload(ossAccessor.pathToObject(objectPath), uploadId, completeParts);
	...
	public void testUploadIncompletePart() throws IOException {
		...
		OSSTestUtils.objectContentEquals(fs, ossAccessor.objectToPath(ossRecoverable.getLastPartObject()), part);
	}
	...
	public void testMultipartAndIncompletePart() throws IOException {
		...
		RefCountedBufferingFileStream partFile = OSSTestUtils.writeData(temporaryFolder, thirdIncompletePart);
		...
		OSSTestUtils.objectContentEquals(fs, ossAccessor.objectToPath(ossRecoverable.getLastPartObject()), thirdIncompletePart);
	}
	...
	public void testRecoverableReflectsTheLatestPartialObject() throws IOException {
		...
		RefCountedBufferingFileStream partFile = OSSTestUtils.writeData(temporaryFolder, incompletePartOne);
		partFile.close();
	...
	public void testUploadingNonClosedFileAsCompleteShouldThroughException() throws IOException {
		...
		RefCountedBufferingFileStream partFile = OSSTestUtils.writeData(temporaryFolder, incompletePart);
