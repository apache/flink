...
public class NativeS3FileSystemFactory implements FileSystemFactory {
    ...
    public FileSystem create(URI fsUri) throws IOException {
        ...
        if (s3minPartSize < S3_MULTIPART_MIN_PART_SIZE) {
            throw new IllegalConfigurationException(
                    PART_UPLOAD_MIN_SIZE.key() + " must be at least 5MB");
        }
        if (s3minPartSize > 5L * 1024 * 1024 * 1024) {
            throw new IllegalConfigurationException(
                    PART_UPLOAD_MIN_SIZE.key() + " must not exceed 5GB");
        }
        if (maxConcurrentUploads <= 0) {
            throw new IllegalConfigurationException(MAX_CONCURRENT_UPLOADS.key() + " must be > 0");
        }

        boolean useAsyncOperations = config.get(USE_ASYNC_OPERATIONS);

        int readBufferSize = config.get(READ_BUFFER_SIZE);
        readBufferSize = Math.max(64 * 1024, Math.min(readBufferSize, 4 * 1024 * 1024));
