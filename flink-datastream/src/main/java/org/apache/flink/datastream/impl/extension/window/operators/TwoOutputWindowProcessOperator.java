...
/** Operator for {@link TwoOutputWindowStreamProcessFunction} in {@link KeyedPartitionStream}. */
public class TwoOutputWindowProcessOperator<K, IN, OUT_MAIN, OUT_SIDE, W extends Window>
    ...
    private final TwoOutputWindowStreamProcessFunction<IN, OUT_MAIN, OUT_SIDE>
    ...
    public TwoOutputWindowProcessOperator(
            InternalTwoOutputWindowStreamProcessFunction<IN, OUT_MAIN, OUT_SIDE, W>
                    windowProcessFunction,
            OutputTag<OUT_SIDE> outputTag,
            @Nullable KeySelector<OUT_MAIN, K> mainOutKeySelector,
            @Nullable KeySelector<OUT_SIDE, K> sideOutKeySelector,
    ...
    private void clearAllState(
        ...
        triggerContext.clear();
        windowFunctionContext.setWindow(window);
        windowProcessFunction.onClear(
                mainCollector, sideCollector, partitionedContext, windowFunctionContext);
        if (mergingWindows != null) {
    ...
    /**
     * Emits the contents of the given window using the user-defined {@link WindowProcessFunction}.
     */
    private void emitWindowContents(W window) throws Exception {
        ...
        sideCollector.setTimestamp(window.maxTimestamp());
        windowFunctionContext.setWindow(window);
        windowProcessFunction.onTrigger(
                mainCollector, sideCollector, partitionedContext, windowFunctionContext);
    }
