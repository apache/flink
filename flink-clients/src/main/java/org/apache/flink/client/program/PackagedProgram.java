 ...
 * supplies functionality to extract nested libraries, search for the program
 * entry point, and extract a program plan.
 */
public class PackagedProgram {
	...
	/**
	 * Property name of the entry in JAR manifest file that describes the class
	...
	/**
	 * Creates an instance that wraps the plan defined in the jar file using the
	 * given argument.
	 *
	 * @param jarFile
	 *            The jar file which contains the plan and a Manifest which
	 *            defines the program-class
	 * @param args
	 *            Optional. The arguments used to create the pact plan, depend
	 *            on implementation of the pact plan. See getDescription().
	 * @throws ProgramInvocationException
	 *             This invocation is thrown if the Program can't be properly
	 *             loaded. Causes may be a missing / wrong class or manifest
	 *             files.
	 */
	...
	/**
	 * Creates an instance that wraps the plan defined in the jar file using the
	 * given arguments. For generating the plan the class defined in the
	 * className parameter is used.
	 * 
	 * @param jarFile
	 *            The jar file which contains the plan.
	 * @param entryPointClassName
	 *            Name of the class which generates the plan. Overrides the
	 ...
	 *             files.
	 */
	public PackagedProgram(File jarFile, List<URL> classpaths, String entryPointClassName, String... args)
			throws ProgramInvocationException {
		if (jarFile == null) {
		...
		// if no entryPointClassName name was given, we try and look one up
		// through the manifest
		if (entryPointClassName == null) {
			entryPointClassName = getEntryPointClassNameFromJar(jarFileUrl);
		}

		// now that we have an entry point, we can extract the nested jar files
		// (if any)
		this.extractedTempLibraries = extractContainedLibaries(jarFileUrl);
		this.classpaths = classpaths;
		try {
			this.userCodeClassLoader = JobWithJars.buildUserCodeClassLoader(getAllLibraries(), classpaths,
		...
		// if the entry point is a program, instantiate the class and get the
		// plan
		if (Program.class.isAssignableFrom(this.mainClass)) {
			...
			throw new ProgramInvocationException(
					"The given program class neither has a main(String[]) method, nor does it implement the "
	 ...
	 * @throws ProgramInvocationException
	 *             Thrown if an error occurred in the user-provided pact
	 *             assembler. This may indicate missing parameters for
	 *             generation.
	 */
	...
	/**
	 * Returns the description provided by the Program class. This may contain a
	 * description of the plan itself and its arguments.
	 * 
	...
	public String getDescription() throws ProgramInvocationException {
		if (ProgramDescription.class.isAssignableFrom(this.mainClass)) {
			...
			if (this.program != null) {
				...
				try {
					descr = InstantiationUtil.instantiate(this.mainClass.asSubclass(ProgramDescription.class),
	 ...
	 * @throws ProgramInvocationException
	 *             Thrown, if an error occurred in the program while creating
	 *             the program's {@link Plan}.
	 */
	...
	private static void callMainMethod(Class<?> entryClass, String[] args) throws ProgramInvocationException {
		...
		try {
			...
			throw new ProgramInvocationException(
					"An error occurred while invoking the program's main method: " + t.getMessage(), t);
		}
	}
	...
	private static String getEntryPointClassNameFromJar(URL jarFile) throws ProgramInvocationException {
		...
		try {
			...
			throw new ProgramInvocationException(
					"Error while opening jar file '" + jarFile.getPath() + "'. " + ioex.getMessage(), ioex);
		}
		...
		try {
			...
			if (manifest == null) {
				throw new ProgramInvocationException("No manifest found in jar file '" + jarFile.getPath()
	...
	private static Class<?> loadMainClass(String className, ClassLoader cl) throws ProgramInvocationException {
		...
		try {
			...
			throw new ProgramInvocationException(
					"The program's entry point class '" + className + "' was not found in the jar file.", e);
		} catch (ExceptionInInitializerError e) {
			throw new ProgramInvocationException(
					"The program's entry point class '" + className + "' threw an error during initialization.", e);
	...
	/**
	 * Takes the jar described by the given file and invokes its pact assembler
	 * class to assemble a plan. The assembler class name is either passed
	 * through a parameter, or it is read from the manifest of the jar. The
	 * assembler is handed the given options for its assembly.
	 * 
	 * @param program
	 *            The program to create the plan for.
	 * @param options
	...
	/**
	 * Takes all JAR files that are contained in this program's JAR file and
	 * extracts them to the system's temp directory.
	 * 
				...
				try {
					for (int i = 0; i < containedJarFileEntries.size(); i++) {
						...
						try {
							...
							throw new ProgramInvocationException(
									"An I/O error occurred while extracting nested library '" + entry.getName()
	...
	private static void checkJarFile(URL jarfile) throws ProgramInvocationException {
		try {
			...
			throw new ProgramInvocationException(
					"Cannot access jar file" + (t.getMessage() == null ? "." : ": " + t.getMessage()), t);
		}
	}
