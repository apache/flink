 ...
 * that contain the classes of the functions and libraries necessary for the
 * execution.
 */
public class JobWithJars {
	 ...
	 */
	public ClassLoader getUserCodeClassLoader() throws Exception {
	...
	public static ClassLoader buildUserCodeClassLoader(List<URL> jars, List<URL> classpaths, ClassLoader parent) throws ProgramInvocationException {
		List<URL> urls = new ArrayList<>();
		for (int i = 0; i < jars.size(); i++) {
			urls.add(jars.get(i));
		}
		for (int i = 0; i < classpaths.size(); i++) {
			// so this test will be for the form of
			// file:///my/path/to/dir/<*>
			if (classpaths.get(i).getProtocol().equals("file") &&
					(classpaths.get(i).getPath().endsWith("/") ||
					classpaths.get(i).getPath().endsWith("/*"))) {
				urls.addAll(expandDir(classpaths.get(i), classpaths.get(i).getProtocol()));
			} else {
				urls.add(classpaths.get(i));
			}
		}
		return new URLClassLoader(urls.toArray(new URL[0]), parent);
	}

	private static List<URL> expandDir(URL dir, String proto) throws ProgramInvocationException {
		List<URL> jarFiles = new ArrayList<>();
		File[] directoryList;

		// File.listFiles does not work with the '*' at the end, so remove it first
		File dirSpec = dir.getPath().endsWith("/*") ?
				new File(dir.getPath().substring(0, dir.getPath().length()-1)) :
				new File(dir.getPath());

		if ((directoryList = dirSpec.listFiles()) != null) {
			try {
				for (File file : directoryList) {
					if (file.isFile() && file.getPath().endsWith(".jar")) {
						jarFiles.add(new URL(proto + "://" + file.getPath()));
					}
				}
			} catch (Exception ex) {
				throw new ProgramInvocationException("Cannot extract jars from directory path");
			}
		}
