...
public class Client {
	...
	/**
	 * If != -1, this field specifies the total number of available slots on the
	 * cluster connected to the client.
	 */
	 ...
	 * ContextEnvironment has been run inside the user JAR. We pass the Client
	 * to every instance of the ContextEnvironment which lets us access the last
	 * JobID here.
	 */
	...
	/**
	 * Creates a instance that submits the programs to the JobManager defined in
	 * the configuration. This method will try to resolve the JobManager
	 * hostname and throw an exception if that is not possible.
	 *
	 * @param config
	 *            The config used to obtain the job-manager's address, and used
	 *            to configure the optimizer.
	 *
	 * @throws java.io.IOException
	 *             Thrown, if the client's actor system could not be started.
	 * @throws java.net.UnknownHostException
	 *             Thrown, if the JobManager's hostname could not be resolved.
	 */
	 ...
	 * job-manager. at the given address using the default port.
	 *
	 * @param config
	 *            The configuration for the client-side processes, like the
	 *            optimizer.
	 * @param maxSlots
	 *            maxSlots The number of maxSlots on the cluster if != -1.
	 *
	 * @throws java.io.IOException
	 *             Thrown, if the client's actor system could not be started.
	 * @throws java.net.UnknownHostException
	...
	/**
	 * Configures whether the client should print progress updates during the
	 * execution to {@code System.out}. All updates are logged via the SLF4J
	 * loggers regardless of this setting.
	 *
	 * @param print
	 *            True to print updates to standard out during execution, false
	 *            to not print them.
	 */
	...
	/**
	 * @return -1 if unknown. The maximum number of available processing slots
	 *         at the Flink cluster connected to this client.
	 */
	...
	public JobSubmissionResult runBlocking(PackagedProgram prog, int parallelism) throws ProgramInvocationException {
		...
		if (prog.isUsingProgramEntryPoint()) {
			...
			ContextEnvironment.setAsContext(new ContextEnvironmentFactory(this, prog.getAllLibraries(),
					prog.getClasspaths(), prog.getUserCodeClassLoader(), parallelism, true, prog.getSavepointPath()));
	...
	public JobSubmissionResult runDetached(PackagedProgram prog, int parallelism) throws ProgramInvocationException {
		...
		if (prog.isUsingProgramEntryPoint()) {
			...
			ContextEnvironmentFactory factory = new ContextEnvironmentFactory(this, prog.getAllLibraries(),
					prog.getClasspaths(), prog.getUserCodeClassLoader(), parallelism, false, prog.getSavepointPath());
	 ...
	 * The call blocks until the execution is complete, and returns afterwards.
	 *
	 * @param program
	 *            The program to be executed.
	 * @param parallelism
	 *            The default parallelism to use when running the program. The
	 *            default parallelism is used when the program does not set a
	 *            parallelism by itself.
	 *
	 * @throws CompilerException
	 *             Thrown, if the compiler encounters an illegal situation.
	 * @throws ProgramInvocationException
	 *             Thrown, if the program could not be instantiated from its jar
	 *             file, or if the submission failed. That might be either due
	 *             to an I/O problem, i.e. the job-manager is unreachable, or
	 *             due to the fact that the parallel execution failed.
	 */
	 ...
	 * The call returns after the program was submitted and does not wait for
	 ...
	 *             Thrown, if the program could not be instantiated from its jar
	 *             file, or if the submission failed. That might be either due
	 *             to an I/O problem, i.e. the job-manager is unreachable.
	 */
	...
	public JobExecutionResult runBlocking(JobGraph jobGraph, ClassLoader classLoader)
			throws ProgramInvocationException {
		...
		try {
			...
			return JobClient.submitJobAndWait(actorSystem, leaderRetrievalService, jobGraph, timeout,
	...
	public JobSubmissionResult runDetached(JobGraph jobGraph, ClassLoader classLoader)
			throws ProgramInvocationException {
	 ...
	 * client's configuration. Stopping works only for streaming programs. Be
	 * aware, that the program might continue to run for a while after sending
	 * the stop command, because after sources stopped to emit data all
	 * operators need to finish processing.
	 * 
	 * @param jobId
	 *            the job ID of the streaming program to stop
	 * @throws Exception
	 *             If the job ID is invalid (ie, is unknown or refers to a batch
	 *             job) or if sending the stop signal failed. That might be due
	 *             to an I/O problem, ie, the job-manager is unreachable.
	 */
	 ...
	 * Accumulators can be requested while a is running or after it has
	 * finished. The default class loader is used to deserialize the incoming
	 * accumulator results.
	 * 
	 * @param jobID
	 ...
	 *            The job identifier of a job.
	 * @param loader
	 *            The class loader for deserializing the accumulator results.
	 * @return A Map containing the accumulator's name and its value.
	 */
	...
	/**
	 * Tells the JobManager to finish the sessions (jobs) defined by the given
	 * IDs.
	 *
	 * @param jobIds
	 *            The IDs that identify the sessions.
	 */
	...
	private ActorGateway getJobManagerGateway() throws Exception {
		...
		return LeaderRetrievalUtils.retrieveLeaderGateway(leaderRetrievalService, actorSystem, lookupTimeout);
	}
