...
/**
 * This class was originally a wrapper around {@link RocksIterator} to check the iterator status
 * for all the methods mentioned to require this check in the wiki documentation: seek, next,
 * seekToFirst, seekToLast, seekForPrev, and prev. At that time, this was required because the
 * iterator may pass the blocks or files it had difficulties in reading (because of IO errors,
 * data corruptions or other issues) and continue with the next available keys. The status flag
 * may not be OK, even if the iterator is valid.
 *
 * However, after <a href="https://github.com/facebook/rocksdb/pull/3810">3810</a> was merged, the
