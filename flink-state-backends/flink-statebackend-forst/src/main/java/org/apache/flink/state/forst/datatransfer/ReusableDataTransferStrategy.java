...
public class ReusableDataTransferStrategy extends CopyDataTransferStrategy {
    ...
    private @Nullable HandleAndLocalPath reuseFileToCheckpoint(
        ...
        // Create a StateHandle with the real path
        FileStatus fileStatus = forStFs.getFileStatus(dbFilePath);
        StreamStateHandle realSourceHandle = new FileStateHandle(sourcePath, fileStatus.getLen());

        // Give file ownership to JM, i.e. DB will not delete it from now on
        forStFs.giveUpOwnership(dbFilePath, realSourceHandle);

        return HandleAndLocalPath.of(realSourceHandle, dbFilePath.getName());
    }
