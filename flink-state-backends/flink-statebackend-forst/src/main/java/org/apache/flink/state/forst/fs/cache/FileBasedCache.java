...
public class FileBasedCache extends LruCache<String, FileCacheEntry> {
    ...
    public FileBasedCache(
            int capacity, CacheLimiter cacheLimiter, FileSystem cacheFs, Path basePath) {
        super(capacity, cacheLimiter);
        this.closed = false;
    ...
    public CachedDataInputStream openForRead(Path path) {
        if (closed) {
            return null;
        }
    ...
    public CachedDataOutputStream openForWrite(FSDataOutputStream originalOutputStream, Path path)
            throws IOException {
        if (closed) {
            return null;
        }
