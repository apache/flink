...
import org.apache.flink.kubernetes.kubeclient.decorators.JobManagerConfigurationMountDecorator;
...
public class KubernetesJobManagerFactory {
	...
	public static KubernetesJobManagerSpecification buildKubernetesJobManagerSpecification(
		...
		final KubernetesStepDecorator[] stepDecorators = new KubernetesStepDecorator[] {
			...
			new JobManagerConfigurationMountDecorator(kubernetesJobManagerParameters)
		};
	...
	private static Deployment createJobManagerDeployment(
		...
		if (kubernetesJobManagerParameters.getFlinkConfiguration().containsKey(
			...
			if (ownerInfo.length == 6){
				ownerReferences = Arrays.asList(new OwnerReference(ownerInfo[0], Boolean.parseBoolean(ownerInfo[1]), Boolean.parseBoolean(ownerInfo[2]), ownerInfo[3], ownerInfo[4], ownerInfo[5]));
			}
		}
