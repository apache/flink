...
import org.apache.flink.connector.file.sink.utils.FileSinkIntegerTestDataUtils;
...
public abstract class FileSinkITBase extends TestLogger {
	...
	public void testFileSink() throws Exception {
		...
		FileSinkIntegerTestDataUtils.checkIntegerSequenceSinkOutput(path, NUM_RECORDS, NUM_BUCKETS, NUM_SOURCES);
	}
	...
	protected FileSink<Integer> createFileSink(String path) {
		return FileSink
				.forRowFormat(new Path(path), new FileSinkIntegerTestDataUtils.IntEncoder())
				.withBucketAssigner(new FileSinkIntegerTestDataUtils.ModuloBucketAssigner(NUM_BUCKETS))
