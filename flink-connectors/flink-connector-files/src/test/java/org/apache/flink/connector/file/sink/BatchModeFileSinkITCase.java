...
public class BatchModeFileSinkITCase extends AbstractFileSinkITCase {

	/**
	 * Creating the testing job graph in blocking mode. The graph created is
	...
	protected JobGraph createJobGraph(String path) {
		...
		StreamSource<Integer, ?> sourceOperator = new StreamSource<>(new BatchModeTestSource(NUM_RECORDS));
		...
		source.setParallelism(NUM_SOURCES)
				.rebalance()
				.map(new BatchModeOnceFailingMap(NUM_RECORDS, triggerFailover))
	 ...
	 * data.
	 */
	private static class BatchModeTestSource extends RichParallelSourceFunction<Integer> {

		private final int numberOfRecords;

		private volatile boolean isCanceled;

		public BatchModeTestSource(int numberOfRecords) {
	 ...
	 */
	private static class BatchModeOnceFailingMap extends RichMapFunction<Integer, Integer> {

		private final int maxNumber;

		private final boolean triggerFailover;

		public BatchModeOnceFailingMap(int maxNumber, boolean triggerFailover) {
