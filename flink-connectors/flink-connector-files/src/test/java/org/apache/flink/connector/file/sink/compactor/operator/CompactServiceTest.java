...
class CompactServiceTest extends AbstractCompactTestBase {
    ...
    @Test
    void testCompressedStreamWriterType() {
        final FileCompactor compactor = new ConcatFileCompactor(null, true);
        final Type writerType = CompactService.getWriterType(compactor);

        assertThat(writerType).isEqualTo(Type.COMPRESSED_STREAM);
    }
    ...
    @Test
    void testInitDifferentBucketWriterForCompressedStream(@TempDir File tmpDir) throws Exception {
        ...
        final BucketWriter<?, String> bucketWriter =
                compactService.initCompactingBucketWriter(Path.fromLocalFile(tmpDir));

        assertThat(bucketWriter).isNotInstanceOf(TestBucketWriter.class);
    }
