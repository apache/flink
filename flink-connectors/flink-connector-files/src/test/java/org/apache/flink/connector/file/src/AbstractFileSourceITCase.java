...
public abstract class AbstractFileSourceITCase extends TestLogger {
	...
	public void testBoundedTextFileSource() throws Exception {
		...
		final DataStream<String> stream = createSourceStream(env, Path.fromLocalFile(testDir), null);
	...
	public void testContinuousTextFileSource() throws Exception {
		...
		final DataStream<String> stream = createSourceStream(
				env, Path.fromLocalFile(testDir), Duration.ofMillis(5));
	...
	private void writeFile(File testDir, int num) throws IOException {
		final File file = new File(testDir, FILE_PATHS[num]);
		writeFileAtomically(file, LINES_PER_FILE[num]);
	}

	private void writeAllFiles(File testDir) throws IOException {
		for (int i = 0; i < FILE_PATHS.length; i++) {
			writeFile(testDir, i);
		}
	}

	private void writeHiddenJunkFiles(File testDir) throws IOException {
	...
	private void writeFileAtomically(File file, String[] lines) throws IOException {
		final File parent = file.getParentFile();
		final File stagingFile = new File(parent, ".tmp-" + file.getName());
		assertTrue(parent.mkdirs() || parent.exists());

		writeFormatFile(stagingFile, lines);

		assertTrue(stagingFile.renameTo(file));
	}

	protected abstract DataStream<String> createSourceStream(
			StreamExecutionEnvironment env, Path path, Duration discoveryInterval);

	protected abstract void writeFormatFile(File file, String[] lines) throws IOException;
