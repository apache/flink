...
public class FileSourceSplitStateTest {
	...
	public void testStateStartsWithSplitValues() {
		...
		assertEquals(split.offset(), state.getCurrentOffset());
		assertEquals(split.skippedRecordCount(), state.getCurrentRecordCount());
	}
	...
	public void testNewSplitTakesModifiedOffsetAndCount() {
		...
		state.setCurrentOffset(1234);
		state.setCurrentRecordCount(7566);
		final FileSourceSplit stateSplit = state.toFileSourceSplit();

		assertEquals(1234, stateSplit.offset());
		assertEquals(7566, stateSplit.skippedRecordCount());
	}
