...
import org.apache.flink.connector.file.src.reader.FileRecordFormatAdapter;
import org.apache.flink.connector.file.src.reader.StreamFormat;
import org.apache.flink.connector.file.src.reader.StreamFormatAdapter;
...
public final class FileSource<T> implements Source<T, FileSourceSplit, PendingSplitsCheckpoint>, ResultTypeQueryable<T> {
	...
	/** The default file enumerator, which recursively enumerates files, split files that consist of
	* multiple distributed storage blocks into multiple splits, and filters hidden files (files
	* starting with '.' or '_'). */
	public static final FileEnumerator.Provider DEFAULT_FILE_ENUMERATOR = BlockSplittingRecursiveEnumerator::new;
	...
	public SourceReader<T, FileSourceSplit> createReader(SourceReaderContext readerContext) {
		return FileSourceReader.create(readerContext, readerFormat, readerContext.getConfiguration());
	}
	 ...
	 */
	public static <T, SELF extends FileSourceBuilder<T, SELF>> FileSourceBuilder<T, SELF> forBulkFileFormat(
			BulkFormat<T> reader,
			Path... paths) {
	 ...
	 */
	public static <T, SELF extends FileSourceBuilder<T, SELF>> FileSourceBuilder<T, SELF> forRecordStreamFormat(
			StreamFormat<T> reader,
			Path... paths) {
	 ...
	 */
	public static <T, SELF extends FileSourceBuilder<T, SELF>> FileSourceBuilder<T, SELF> forRecordFileFormat(
			FileRecordFormat<T> reader,
			Path... paths) {
	 ...
	 */
	public static class FileSourceBuilder<T, SELF extends FileSourceBuilder<T, SELF>> {
		...
		private FileEnumerator.Provider fileEnumerator = FileSource.DEFAULT_FILE_ENUMERATOR;
		private FileSplitAssigner.Provider splitAssigner = FileSource.DEFAULT_SPLIT_ASSIGNER;
		@Nullable
		private ContinuousEnumerationSettings continuousSourceSettings;

		protected FileSourceBuilder(Path[] inputPaths, BulkFormat<T> readerFormat) {
