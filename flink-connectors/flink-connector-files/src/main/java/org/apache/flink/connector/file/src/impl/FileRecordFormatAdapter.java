...
public final class FileRecordFormatAdapter<T> implements BulkFormat<T> {
	...
	public BulkFormat.Reader<T> createReader(
		...
		return doWithCleanupOnException(reader, () -> {
			...
			return wrapReader(reader, config);
		});
	}
	...
	private static <T> Reader<T> wrapReader(final FileRecordFormat.Reader<T> reader, final Configuration config) {
	...
	public static final class Reader<T> implements BulkFormat.Reader<T> {
		...
		Reader(FileRecordFormat.Reader<T> reader, int numRecordsPerBatch) {
			...
			this.lastOffset = CheckpointedPosition.NO_OFFSET;
		}
