 ...
 *
 * <p>The actual reading is done by the {@link StreamFormat.Reader}, which is created based on an
 * input stream in the {@link #createReader(FSDataInputStream, Configuration)} method.
 ...
 * {@link StreamFormat.Reader#getCheckpointablePosition()}. This can improve recovery speed from
 * a checkpoint.
 *
 ...
 * See {@link StreamFormatAdapter#FETCH_IO_SIZE} to configure that fetch size.
 */
public interface StreamFormat<T> extends Serializable {
	 ...
	 */
	StreamFormat.Reader<T> createReader(FSDataInputStream stream, Configuration config) throws IOException;
