...
public class KinesisFirehoseSinkITCase {
    ...
    private SdkAsyncHttpClient s3HttpClient;
    private SdkAsyncHttpClient firehoseHttpClient;
    private SdkAsyncHttpClient iamHttpClient;
    ...
    public void setup() throws Exception {
        ...
        firehoseHttpClient = createHttpClient(mockFirehoseContainer.getEndpoint());
        iamHttpClient = createHttpClient(mockFirehoseContainer.getEndpoint());
        s3AsyncClient = createS3Client(mockFirehoseContainer.getEndpoint(), s3HttpClient);
        firehoseAsyncClient =
                getFirehoseClient(mockFirehoseContainer.getEndpoint(), firehoseHttpClient);
    ...
    public void firehoseSinkWritesCorrectDataToMockAWSServices() throws Exception {
        ...
        SdkAsyncHttpClient s3RetrievalHttpClient =
                createHttpClient(mockFirehoseContainer.getEndpoint());
        S3AsyncClient s3RetrievalAsyncClient =
                createS3Client(mockFirehoseContainer.getEndpoint(), s3RetrievalHttpClient);
        List<S3Object> objects = listBucketObjects(s3RetrievalAsyncClient, BUCKET_NAME);
        assertThat(objects.size()).isEqualTo(NUMBER_OF_ELEMENTS);
        assertThat(
                        readObjectsFromS3Bucket(
                                s3RetrievalAsyncClient,
                                objects,
                                BUCKET_NAME,
                                response -> new String(response.asByteArrayUnsafe())))
                .containsAll(KinesisFirehoseTestUtils.getSampleDataGenerated(NUMBER_OF_ELEMENTS));
    }
}
