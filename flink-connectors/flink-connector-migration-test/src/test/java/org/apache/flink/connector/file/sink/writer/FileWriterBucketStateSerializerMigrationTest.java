...
/**
 * Tests for the {@link FileWriterBucketStateSerializerMigrationTest} that verify we can still read
 * snapshots taken from old {@link StreamingFileSink}. We keep snapshots for all previous
 * versions in version control (including the current version). The tests verify that the current
 * version of the serializer can still read data from all previous versions.
 */
...
public class FileWriterBucketStateSerializerMigrationTest {
	...
	private static FileWriterBucket<String> restoreBucket(final FileWriterBucketState bucketState) throws IOException {
