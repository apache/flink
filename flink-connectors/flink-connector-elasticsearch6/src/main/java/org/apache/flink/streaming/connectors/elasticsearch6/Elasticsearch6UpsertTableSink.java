...
public class Elasticsearch6UpsertTableSink extends ElasticsearchUpsertTableSinkBase {
	...
	protected SinkFunction<Tuple2<Boolean, Row>> createSinkFunction(
		...
		Integer maxRetryTimeout = Optional.ofNullable(sinkOptions.get(REST_MAX_RETRY_TIMEOUT))
			.map(Integer::valueOf)
			.orElse(null);
		...
		if (Optional.ofNullable(username).isPresent() && Optional.ofNullable(password).isPresent()) {
			builder.setRestClientFactory(new AuthRestClientFactory(username, password, maxRetryTimeout, pathPrefix));
		} else {
			builder.setRestClientFactory(new DefaultRestClientFactory(maxRetryTimeout, pathPrefix));
		}
	...
	static class AuthRestClientFactory implements RestClientFactory {
		...
		public AuthRestClientFactory(@Nullable String userName, @Nullable String password,
