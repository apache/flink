...
public class DataGenerators {
     ...
     * SourceReaderWithCheckpointsLatch} for details.
     *
    ...
    public static <OUT> DataGeneratorSource<OUT> fromDataWithSnapshotsLatch(
        ...
        return new DataGeneratorSource<>(
                (SourceReaderFactory<OUT, NumberSequenceSplit>)
                        (readerContext) ->
                                new SourceReaderWithCheckpointsLatch<>(
     ...
     * @param data The collection of elements to create the stream from.
     * @param <OUT> The type of the returned data stream.
     * @param couldExit The boolean supplier that makes it possible to delay termination based on
     *     external conditions.
     */
    public static <OUT> DataGeneratorSource<OUT> fromDataWithSnapshotsLatch(
            Collection<OUT> data, TypeInformation<OUT> typeInfo, BooleanSupplier couldExit) {
        IndexLookupGeneratorFunction<OUT> generatorFunction =
                new IndexLookupGeneratorFunction<>(typeInfo, data);
        ...
        return new DataGeneratorSource<>(
                (SourceReaderFactory<OUT, NumberSequenceSplit>)
                        (readerContext) ->
                                new SourceReaderWithCheckpointsLatch<>(
                                        readerContext, generatorFunction, couldExit),
                data.size(),
                typeInfo);
    }
}
