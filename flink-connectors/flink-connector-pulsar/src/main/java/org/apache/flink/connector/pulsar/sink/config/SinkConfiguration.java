...
public class SinkConfiguration extends PulsarConfiguration {
     ...
     * for making sure the message could be write only once. Since the checkpoint interval couldn't
     * be acquired from {@link InitContext}, we have to expose this option. Make sure this value is
    ...
    /** The message hash logic for routing the message into one Pulsar partition. */
    public MessageKeyHash getMessageKeyHash() {
        return messageKeyHash;
    }

    /** If we should serialize and send message by using Pulsar {@link Schema}. */
    public boolean isEnableSchemaEvolution() {
        return enableSchemaEvolution;
    }

    /** The limit of triggering a flush action on Pulsar sink. */
    public int getMaxPendingMessages() {
        return maxPendingMessages;
    }

    /** The maximum allowed recommit time for Pulsar transaction. */
