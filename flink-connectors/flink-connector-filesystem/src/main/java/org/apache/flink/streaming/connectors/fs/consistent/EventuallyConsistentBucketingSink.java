 ...
 *
 * <p>
 * When creating the sink a {@code basePath} must be specified. The base directory contains one directory for every
 ...
 *
 * <p>
 * The sink uses a {@link EventuallyConsistentBucketer} to determine which directory each file should be written to
 ...
 *
 * <p>
 * An eventually consistent filesystem typically provides weak guarantee's about what operations can be performed in
 ...
 *
 * <p>
 * Because {@code DELETE} operations are not permitted, if a checkpoint fails
...
public class EventuallyConsistentBucketingSink<T>
	...
	private static Logger LOG = LoggerFactory.getLogger(EventuallyConsistentBucketingSink.class);
	 ...
	 */
	private final String DEFAULT_FLAG_FILE = "_DONE";

	/**
	 * The consistent file system directory where files are buffered
	 * before being copied to their final location. The default
	 * location is platform dependent tmp directory on local disk.
	 */
	...
	public void initializeState(FunctionInitializationContext context) throws Exception {
		...
		completedBucketState = context.getOperatorStateStore().getSerializableListState("completed-buckets");
	...
	static class BucketState<T> implements Serializable {
		/**
		 * Closed buffer files that **have not** been
		 * uploaded to remote
		 */
		...
		/**
		 * The remote path for this checkpoint
		 * as designated by the bucketer
		 */
