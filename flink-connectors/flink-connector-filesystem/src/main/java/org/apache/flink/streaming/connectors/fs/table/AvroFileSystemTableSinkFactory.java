...
public class AvroFileSystemTableSinkFactory extends BultFileSystemTableSinkFactory {
	...
	public StreamTableSink createStreamTableSink(Map<String, String> properties) {
		...
		if (descriptorProperties.containsKey(AvroValidator.FORMAT_RECORD_CLASS)) {
			Class specificClass = descriptorProperties.getClass(AvroValidator.FORMAT_RECORD_CLASS, SpecificRecord.class);
			tableSchema = TableSchema.fromTypeInfo(AvroSchemaConverter.convertToTypeInfo(specificClass));
			...
			tableSchema = TableSchema.fromTypeInfo(AvroSchemaConverter.convertToTypeInfo(schemaString));
			writerFactory = new AvroWriterFactory(schemaString);
		}


		StreamingFileSink.BulkFormatBuilder builder = StreamingFileSink.forBulkFormat(new Path(path), writerFactory);
