...
public class HiveTableSinkCompactITCase {
    ...
    public void testNoCompaction() throws Exception {
        ...
        assertThat(toSortedResult(result).toString())
                .isEqualTo("[+I[k1, v1], +I[k2, v2], +I[k3, v3], +I[k4, v4]]");
    }
    ...
    public void testCompactPartitionedTable() throws Exception {
        ...
        assertThat(files).hasSize(1);
        // verify the result
        result = CollectionUtil.iteratorToList(tableEnv.executeSql("select * from src").collect());
        assertThat(toSortedResult(result).toString())
                .isEqualTo(
    ...
    private List<String> toSortedResult(List<Row> result) {
        return result.stream().map(Row::toString).sorted().collect(Collectors.toList());
    }
}
