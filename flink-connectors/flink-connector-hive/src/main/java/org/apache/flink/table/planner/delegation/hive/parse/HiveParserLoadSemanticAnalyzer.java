...
public class HiveParserLoadSemanticAnalyzer {
    ...
    public HiveLoadOperation convertToOperation(HiveParserASTNode ast) throws SemanticException {
        ...
        TableSpec ts =
                new TableSpec(db, conf, (HiveParserASTNode) tableTree, frameworkConfig, cluster);
        ...
        if (ts.partSpec != null && ts.partSpec.size() > 0) {
            // it's to load partition
            return new HiveLoadOperation(null, null, false, false);
        } else {
            return new HiveLoadOperation(
                    new Path(fromURI),
