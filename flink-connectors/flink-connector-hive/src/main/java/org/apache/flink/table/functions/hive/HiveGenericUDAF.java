...
public class HiveGenericUDAF
    ...
    private static final Logger LOG = LoggerFactory.getLogger(HiveGenericUDAF.class);
    ...
    public DataType inferReturnType() {
        try {
            LOG.info(
                    "Getting result type of HiveGenericUDAF with {}",
                    hiveFunctionWrapper.getClassName());
            if (!initialized) {
                init();
            }
    ...
    @Override
    public HiveFunctionWrapper getFunctionWrapper() {
