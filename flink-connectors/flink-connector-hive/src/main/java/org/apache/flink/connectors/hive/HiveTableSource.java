...
public class HiveTableSource
    ...
    // A constructor mainly used to create copies during optimizations like partition pruning and
        ...
        if (isStreamingSource()) {
            if (catalogTable.getPartitionKeys().isEmpty()) {
                return createStreamSourceForNonPartitionTable(
                        execEnv, typeInfo, inputFormat, allHivePartitions.get(0));
            } else {
    ...
    private DataStream<RowData> createStreamSourceForPartitionTable(
        ...
        HiveContinuousMonitoringFunction monitoringFunction =
    ...
    private DataStream<RowData> createStreamSourceForNonPartitionTable(
        ...
        HiveTableFileInputFormat fileInputFormat =
                new HiveTableFileInputFormat(inputFormat, hiveTable);
    ...
    @Override
    public TableSource<RowData> applyPartitionPruning(
            List<Map<String, String>> remainingPartitions) {
    ...
    private List<HiveTablePartition> initAllPartitions() {
        ...
        // Please note that the following directly accesses Hive metastore, which is only a
        // temporary workaround.
        // Ideally, we need to go thru Catalog API to get all info we need here, which requires some
        // major
    ...
    private static List<String> partitionSpecToValues(
    ...
    private static Object restorePartitionValueFromFromType(
    ...
    public String explainSource() {
        String explain =
                String.format(
                        " TablePath: %s, PartitionPruned: %s, PartitionNums: %d",
                        tablePath.getFullName(),
                        partitionPruned,
                        null == remainingPartitions ? null : remainingPartitions.size());
        if (projectedFields != null) {
