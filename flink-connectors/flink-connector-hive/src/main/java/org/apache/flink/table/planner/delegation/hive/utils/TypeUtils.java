                        ...
                        .map(
                                field ->
                                        TypeConversions.fromLogicalToDataType(
                                                toLogicType(field.getType())))
                        .toArray(DataType[]::new);

        return ResolvedSchema.physical(fieldNames, fieldTypes);
    }

    private static LogicalType toLogicType(RelDataType relDataType) {
            ...
            case ROW:
                {
                    ...
                    return RowType.of(
                            relDataType.getFieldList().stream()
                                    .map(fieldType -> toLogicType(fieldType.getType()))
                                    .toArray(LogicalType[]::new),
                            relDataType.getFieldNames().toArray(new String[0]));
                }
            case MULTISET:
                return new MultisetType(toLogicType(relDataType.getComponentType()));
            case MAP:
                return new MapType(
                        toLogicType(relDataType.getKeyType()),
                        toLogicType(relDataType.getValueType()));
            case ARRAY:
                return new ArrayType(toLogicType(relDataType.getComponentType()));
        }
