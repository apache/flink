...
public class FlinkKafkaProducerMigrationVersionTest extends KafkaTestBase {
	...
	public FlinkKafkaProducerMigrationVersionTest(MigrationVersion testMigrateVersion) {
		this.testMigrateVersion = testMigrateVersion;
	}

	@BeforeClass
	public static void selectKafkaTestImplementation() {
		// A specific kafka test implementation that doesn't erase the tmp file.
		setKafkaTestImplementation("org.apache.flink.streaming.connectors.kafka.KafkaMigrationTestEnvironmentImpl");
	}

	/**
	 * Manually run this to write binary snapshot data.
	 * The savepoint for migration test of a FlinkKafkaProducer-011 --> FlinkKafkaProducer on master branch should be
	 * created by using the writeSnapshot function of the FlinkKafkaProducer011MigrationTest.
	 */
	@Ignore
	@Test
	public void writeSnapshot() throws Exception {
		...
		OperatorSnapshotUtil.writeStateHandle(snapshot, "src/test/resources/kafka-producer-migration-test-flink-" + flinkGenerateSavepointVersion + "-snapshot");
	...
	private void initializeState(OneInputStreamOperatorTestHarness testHarness) throws Exception{
		...
		testHarness.initializeState(
			OperatorSnapshotUtil.getResourceFilename(
				"kafka-producer-migration-test-flink-" + testMigrateVersion + "-snapshot"
			)
		);
		testHarness.open();
	}
