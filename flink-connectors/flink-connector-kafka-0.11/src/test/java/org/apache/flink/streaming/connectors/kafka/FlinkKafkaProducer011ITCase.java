...
public class FlinkKafkaProducer011ITCase extends KafkaTestBaseWithFlink {
    ...
    public void testRestoreToCheckpointAfterExceedingProducersPool() throws Exception {
        ...
        try (OneInputStreamOperatorTestHarness<Integer, Object> testHarness1 =
            ...
            if (!findSerializedThrowable(
                            ex, ProducerFencedException.class, ClassLoader.getSystemClassLoader())
                    .isPresent()) {
                throw ex;
            }
        }
    ...
    /**
     * This tests checks whether FlinkKafkaProducer011 correctly aborts lingering transactions after
     * a failure. If such transactions were left alone lingering it consumers would be unable to
    ...
    /**
     * This tests checks whether FlinkKafkaProducer011 correctly aborts lingering transactions after
     * a failure, which happened before first checkpoint and was followed up by reducing the
    ...
    public void testScaleDownBeforeFirstCheckpoint() throws Exception {
        ...
        OneInputStreamOperatorTestHarness<Integer, Object> postScaleDownOperator1 =
                createTestHarness(topic, 1, 1, 0, EXACTLY_ONCE);
    ...
    /**
     * Each instance of FlinkKafkaProducer011 uses it's own pool of transactional ids. After the
    ...
    private OperatorSubtaskState repartitionAndExecute(
        ...
        for (int subtaskIndex = 0; subtaskIndex < newParallelism; subtaskIndex++) {
            ...
            OneInputStreamOperatorTestHarness<Integer, Object> testHarness =
                    createTestHarness(
                            topic, maxParallelism, newParallelism, subtaskIndex, EXACTLY_ONCE);
            ...
            if (inputData.hasNext()) {
                ...
                for (int i = 1;
                        i < FlinkKafkaProducer011.DEFAULT_KAFKA_PRODUCERS_POOL_SIZE - 1;
                        i++) {
                    testHarness.processElement(-nextValue, 0);
    ...
    private void testRecoverWithChangeSemantics(
            String topic, Semantic fromSemantic, Semantic toSemantic) throws Exception {
    ...
    private OneInputStreamOperatorTestHarness<Integer, Object> createTestHarness(
            String topic, Semantic semantic) throws Exception {
        return createTestHarness(topic, 1, 1, 0, semantic);
    }

    private OneInputStreamOperatorTestHarness<Integer, Object> createTestHarness(
            String topic, int maxParallelism, int parallelism, int subtaskIndex, Semantic semantic)
            throws Exception {
    ...
    private boolean isCausedBy(FlinkKafka011ErrorCode expectedErrorCode, Throwable ex) {
        ...
        Optional<FlinkKafka011Exception> cause =
                findSerializedThrowable(
                        ex, FlinkKafka011Exception.class, ClassLoader.getSystemClassLoader());
        if (cause.isPresent()) {
