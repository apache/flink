...
public class FlinkKafkaProducer10ITCase extends KafkaTestBase {
	 ...
	 */
	public void resourceCleanUp(FlinkKafkaProducer10.Semantic semantic) throws Exception {
	...
	public void testRestoreToCheckpointAfterExceedingProducersPool() throws Exception {
		...
		try (OneInputStreamOperatorTestHarness<Integer, Object> testHarness1 = createTestHarness(topic)) {
			...
			try {
				for (int i = 0; i < FlinkKafkaProducer10.DEFAULT_KAFKA_PRODUCERS_POOL_SIZE; i++) {
	...
	public void testFlinkKafkaProducer10FailTransactionCoordinatorBeforeNotify() throws Exception {
		...
		FlinkKafkaProducer10<Integer> kafkaProducer = new FlinkKafkaProducer10<>(
			topic,
	...
	/**
	 * This tests checks whether FlinkKafkaProducer10 correctly aborts lingering transactions after a failure.
	...
	public void testScaleDownBeforeFirstCheckpoint() throws Exception {
		...
		int preScaleDownParallelism = Math.max(2, FlinkKafkaProducer10.SAFE_SCALE_DOWN_FACTOR);
		...
		OneInputStreamOperatorTestHarness<Integer, Object> postScaleDownOperator1 = createTestHarness(topic, 1, 1, 0, FlinkKafkaProducer10.Semantic.EXACTLY_ONCE);
	...
	/**
	 * Each instance of FlinkKafkaProducer10 uses it's own pool of transactional ids. After the restore from checkpoint
	...
	private List<OperatorStateHandle> repartitionAndExecute(
		...
		for (int subtaskIndex = 0; subtaskIndex < parallelism; subtaskIndex++) {
			OneInputStreamOperatorTestHarness<Integer, Object> testHarness =
				createTestHarness(topic, maxParallelism, parallelism, subtaskIndex, FlinkKafkaProducer10.Semantic.EXACTLY_ONCE);
			...
			if (inputData.hasNext()) {
				...
				for (int i = 1; i < FlinkKafkaProducer10.DEFAULT_KAFKA_PRODUCERS_POOL_SIZE - 1; i++) {
					testHarness.processElement(-nextValue, 0);
	...
	public void testRunOutOfProducersInThePool() throws Exception {
		...
		try (OneInputStreamOperatorTestHarness<Integer, Object> testHarness = createTestHarness(topic)) {
			...
			for (int i = 0; i < FlinkKafkaProducer10.DEFAULT_KAFKA_PRODUCERS_POOL_SIZE * 2; i++) {
	...
	private OneInputStreamOperatorTestHarness<Integer, Object> createTestHarness(String topic) throws Exception {
		return createTestHarness(topic, 1, 1, 0, FlinkKafkaProducer10.Semantic.EXACTLY_ONCE);
	}
	...
	private Properties createProperties() {
		...
		properties.put(FlinkKafkaProducer10.KEY_DISABLE_METRICS, "true");
		return properties;
	}
