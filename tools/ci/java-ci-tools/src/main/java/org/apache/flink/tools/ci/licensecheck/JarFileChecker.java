...
public class JarFileChecker {
    ...
    private static int findNonBinaryFilesContainingText(
        ...
        try (Stream<Path> files = Files.walk(jarRoot)) {
            return files.filter(path -> !path.equals(jarRoot))
                    ...
                    .filter(path -> !fileNameStartsWith(path, "license"))
                    // false-positives due to optional components; startsWith covers .txt/.md files
                    .filter(path -> !fileNameStartsWith(path, "notice"))
    ...
    private static boolean fileNameStartsWith(Path file, String prefix) {
        return getFileName(file).startsWith(prefix);
    }

    private static boolean fileNameEndWith(Path file, String suffix) {
        return getFileName(file).endsWith(suffix);
    }
    ...
    private static boolean isJarFile(Path file) {
        return getFileName(file).endsWith(".jar");
    }

    private static boolean isTestsJarFile(Path file) {
        return getFileName(file).endsWith("-tests.jar");
    }
