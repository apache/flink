...
FLINK_UBERJAR=/home/sebastian/Daten/workspace/incubator-flink/flink-dist/target/flink-dist-0.7-incubating-SNAPSHOT-yarn-uberjar.jar
FLINK_JM_CLASSPATH=`manglePathList "$(constructJobManagerClassPath)"`
FLINK_JM_CLASSPATH=$FLINK_JM_CLASSPATH:$FLINK_UBERJAR
...
case $STARTSTOP in
    ...
    (start)
        ...
        $JAVA_RUN $JVM_ARGS ${FLINK_ENV_JAVA_OPTS} "${log_setting[@]}" -classpath "$FLINK_JM_CLASSPATH" org.apache.flink.runtime.jobmanager.JobManager -executionMode $EXECUTIONMODE -configDir "$FLINK_CONF_DIR"
        echo $! > $pid
    ;;
