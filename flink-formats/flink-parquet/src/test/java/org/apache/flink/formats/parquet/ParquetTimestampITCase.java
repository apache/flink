...
import static org.apache.flink.connector.file.table.FileSystemConnectorOptions.*;
...
public class ParquetTimestampITCase extends FsStreamingSinkITCaseBase {
    ...
    public void before() {
        super.before();
        tEnv().dropTemporaryView("my_table");
    ...
    @Override
    public String getDDL(boolean partition, String policy) {
        ...
        ddl.append(") ");
        if (partition) {
            ddl.append("partitioned by (d, e) ");
        }
        ddl.append("with ( ");
        ddl.append(" 'connector' = 'filesystem', ");
        ddl.append(" 'path' = '" + resultPath() + "', ");
        ddl.append(" '" + PARTITION_TIME_EXTRACTOR_TIMESTAMP_PATTERN.key() + "' = '$d $e:00:00', ");
        ddl.append(" '" + SINK_PARTITION_COMMIT_DELAY.key() + "' = '1h', ");
        ddl.append(" '" + SINK_PARTITION_COMMIT_POLICY_KIND.key() + "' = '" + policy + "', ");
        ddl.append(" '" + SINK_PARTITION_COMMIT_SUCCESS_FILE_NAME.key() + "' = '_MY_SUCCESS', ");
