...
public class ParquetColumnarRowInputFormat<SplitT extends FileSourceSplit>
     ...
     *
     * @param producedType the projected row type for parquet format, excludes extra fields.
    ...
    public static <SplitT extends FileSourceSplit>
            ParquetColumnarRowInputFormat<SplitT> createPartitionedFormat(
                    ...
                    boolean isUtcTimestamp,
                    boolean isCaseSensitive) {
        int[][] newProjectedFields = new int[projectedFields.length][1];
        for (int i = 0; i < projectedFields.length; i++) {
            newProjectedFields[i][0] = projectedFields[i];
        }
        return createPartitionedFormat(
                hadoopConfig,
                tableType,
                producedTypeInfo,
                newProjectedFields,
    ...
    public static <SplitT extends FileSourceSplit>
        ...
        List<RowType.RowField> projectedRowFields = buildRowFields(tableType, projectedFields);
        ...
        for (int i = 0; i < projectedRowFields.size(); i++) {
            if (partitionKeys.contains(projectedRowFields.get(i).getName())) {
                continue;
            }
                        ...
                        vectors[i] =
                                partitionKeys.contains(field.getName())
                                        ? createVectorFromConstant(
                                                ...
                                                batchSize)
                                        : parquetVectors[fieldIndex.get(field)];
                    }
                    return new VectorizedColumnBatch(vectors);
                };
        DataType projectedType = Projection.of(newProjectedFields).project(DataTypes.of(tableType));
