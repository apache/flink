...
public abstract class ParquetVectorizedInputFormat<T, SplitT extends FileSourceSplit>
    ...
    private final List<FilterPredicate> pushDownFilters;
    ...
    public ParquetReader createReader(final Configuration config, final SplitT split)
        ...
        // set push down filter
        Set<String> fileFields =
                fileSchema.getFields().stream().map(Type::toString).collect(Collectors.toSet());
        pushDownFilters.stream()
                // only the filters whose fields are totally in parquet file's fields are considered
                .filter(f -> ParquetFilters.isFilterFieldsIn(f, fileFields))
