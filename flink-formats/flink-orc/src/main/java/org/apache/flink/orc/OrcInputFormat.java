...
public abstract class OrcInputFormat<T> extends FileInputFormat<T> {
    ...
    public OrcInputFormat(
            Path path, TypeDescription orcSchema, Configuration orcConfig, int batchSize) {
        super(path);
     ...
     *
     * @param selectedFields The indices of the fields of the ORC schema that are returned by the
     *     InputFormat.
     */
     ...
     *
     * <p>Note: Predicates can significantly reduce the amount of data that is read. However, the
     * OrcInputFormat does not guarantee that all returned rows qualify the predicates. Moreover,
     * predicates are only applied if the referenced field is among the selected fields.
     *
