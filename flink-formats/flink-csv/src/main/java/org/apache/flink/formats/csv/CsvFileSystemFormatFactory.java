...
public class CsvFileSystemFormatFactory implements FileSystemFormatFactory {
    ...
    private CsvSchema buildCsvSchema(RowType rowType, ReadableConfig options) {
        ...
        options.getOptional(LINE_DELIMITER).ifPresent(csvBuilder::setLineSeparator);
    ...
    public Optional<Encoder<RowData>> createEncoder(WriterContext context) {
        ...
        CsvRowDataSerializationSchema.Builder builder =
                new CsvRowDataSerializationSchema.Builder(context.getFormatRowType());

        options.getOptional(FIELD_DELIMITER)
                .map(s -> s.charAt(0))
                .ifPresent(builder::setFieldDelimiter);

        options.getOptional(LINE_DELIMITER).ifPresent(builder::setLineDelimiter);
        ...
        options.getOptional(ARRAY_ELEMENT_DELIMITER).ifPresent(builder::setArrayElementDelimiter);

        options.getOptional(ESCAPE_CHARACTER)
                .map(s -> s.charAt(0))
                .ifPresent(builder::setEscapeCharacter);

        options.getOptional(NULL_LITERAL).ifPresent(builder::setNullLiteral);
