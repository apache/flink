...
public final class CsvRowDataSerializationSchema implements SerializationSchema<RowData> {
	...
	private interface SerializationRuntimeConverter extends Serializable {
		JsonNode convert(CsvMapper csvMapper, ContainerNode<?> container, Object value);
	}

	private SerializationRuntimeConverter createRowConverter(RowType type, boolean isTopLevel) {
		...
		if (isTopLevel) {
			return (csvMapper, container, value) -> {
				...
				for (int i = 0; i < rowArity; i++) {
					...
					objectNode.set(
						fieldNames[i],
						fieldConverters[i].convert(csvMapper, container, field));
				}
				return objectNode;
			};
		} else {
			return (csvMapper, container, value) -> {
				...
				for (int i = 0; i < rowArity; i++) {
					Object field = RowData.get(row, i, fieldTypes[i]);
					arrayNode.add(fieldConverters[i].convert(csvMapper, arrayNode, field));
				}
				return arrayNode;
			};
		}
	...
	private SerializationRuntimeConverter createNullableConverter(LogicalType type) {
		final SerializationRuntimeConverter valueConverter = createConverter(type);
		return (csvMapper, container, value) -> {
			if (value == null) {
				return container.nullNode();
			}
			return valueConverter.convert(csvMapper, container, value);
		};
	}
	...
	private SerializationRuntimeConverter createConverter(LogicalType type) {
		switch (type.getTypeRoot()) {
			case NULL:
				return (csvMapper, container, value) -> container.nullNode();
			case BOOLEAN:
				return (csvMapper, container, value) -> container.booleanNode((Boolean) value);
			case TINYINT:
				return (csvMapper, container, value) -> container.numberNode((Byte) value);
			case SMALLINT:
				return (csvMapper, container, value) -> container.numberNode((Short) value);
			case INTEGER:
			case INTERVAL_YEAR_MONTH:
				return (csvMapper, container, value) -> container.numberNode((Integer) value);
			case BIGINT:
			case INTERVAL_DAY_TIME:
				return (csvMapper, container, value) -> container.numberNode((Long) value);
			case FLOAT:
				return (csvMapper, container, value) -> container.numberNode((Float) value);
			case DOUBLE:
				return (csvMapper, container, value) -> container.numberNode((Double) value);
			case CHAR:
			case VARCHAR:
				return (csvMapper, container, value) -> container.textNode(value.toString());
			case BINARY:
			case VARBINARY:
				return (csvMapper, container, value) -> container.binaryNode((byte[]) value);
			case DATE:
	...
	private SerializationRuntimeConverter createDecimalConverter() {
		return (csvMapper, container, value) -> {
			...
			return container.numberNode(bd);
		};
	}
	...
	private SerializationRuntimeConverter createArrayConverter(ArrayType type) {
		...
		final SerializationRuntimeConverter elementConverter = createNullableConverter(elementType);
		return (csvMapper, container, value) -> {
	...
	private static void validateArity(int expected, int actual) {
		if (expected != actual) {
			throw new RuntimeException("RowData length mismatch. " + expected +
