 ...
 * to satisfy various Flink types, this class sorts out instances of {@link TypeInformation}
 * and {@link LogicalType} that are not supported. It converts supported types to one of
 * CsvSchema's types.
 *
...
public final class CsvRowSchemaConverter {
	...
	private static final HashSet<LogicalTypeRoot> NUMBER_TYPE_ROOTS =
		new HashSet<>(Arrays.asList(LogicalTypeRoot.TINYINT, LogicalTypeRoot.SMALLINT,
			LogicalTypeRoot.INTEGER, LogicalTypeRoot.BIGINT, LogicalTypeRoot.DOUBLE,
			LogicalTypeRoot.FLOAT, LogicalTypeRoot.DECIMAL));
	...
	private static final HashSet<LogicalTypeRoot> STRING_TYPE_ROOTS =
		new HashSet<>(Arrays.asList(LogicalTypeRoot.CHAR, LogicalTypeRoot.VARCHAR,
			LogicalTypeRoot.BINARY, LogicalTypeRoot.VARBINARY, LogicalTypeRoot.DATE,
			LogicalTypeRoot.TIME_WITHOUT_TIME_ZONE, LogicalTypeRoot.TIMESTAMP_WITHOUT_TIME_ZONE,
			LogicalTypeRoot.TIMESTAMP_WITH_LOCAL_TIME_ZONE, LogicalTypeRoot.TIMESTAMP_WITH_TIME_ZONE));
	...
	private static void validateNestedField(String fieldName, LogicalType type) {
		if (!NUMBER_TYPE_ROOTS.contains(type.getTypeRoot()) &&
			...
			throw new IllegalArgumentException(
				"Only simple types are supported in the second level nesting of fields '" +
					fieldName + "' but was: " + type.asSummaryString());
		}
	}
}
