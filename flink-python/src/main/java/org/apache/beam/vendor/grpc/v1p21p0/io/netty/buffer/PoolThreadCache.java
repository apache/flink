...
/**
 * Acts a Thread cache for allocations. This implementation is moduled after <a
 * href="http://people.freebsd.org/~jasone/jemalloc/bsdcan2006/jemalloc.pdf">jemalloc</a> and the
 * descripted technics of <a
...
final class PoolThreadCache {
    ...
    /**
     * Try to allocate a tiny buffer out of the cache. Returns {@code true} if successful {@code
     * false} otherwise
     */
    boolean allocateTiny(
            PoolArena<?> area, PooledByteBuf<?> buf, int reqCapacity, int normCapacity) {
        return allocate(cacheForTiny(area, normCapacity), buf, reqCapacity);
    }

    /**
     * Try to allocate a small buffer out of the cache. Returns {@code true} if successful {@code
     * false} otherwise
     */
    boolean allocateSmall(
            PoolArena<?> area, PooledByteBuf<?> buf, int reqCapacity, int normCapacity) {
    ...
    /**
     * Add {@link PoolChunk} and {@code handle} to the cache if there is enough room. Returns {@code
     * true} if it fit into the cache {@code false} otherwise.
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    boolean add(
            PoolArena<?> area,
    ...
    void free(boolean finalizer) {
        // As free() may be called either by the finalizer or by FastThreadLocal.onRemoval(...) we
        // need to ensure
        // we only call this one time.
        if (freed.compareAndSet(false, true)) {
            int numFreed =
                    free(tinySubPageDirectCaches, finalizer)
    ...
    private abstract static class MemoryRegionCache<T> {
        ...
        /**
         * Clear out this cache and free up all previous cached {@link PoolChunk}s and {@code
         * handle}s.
         */
        ...
        private void freeEntry(Entry entry, boolean finalizer) {
            ...
            if (!finalizer) {
                // recycle now so PoolChunk can be GC'ed. This will only be done if this is not
                // freed because of
                // a finalizer.
                entry.recycle();
            }
