...
class SerializerSnapshotTest {
    ...
    private static RowDataSerializer rowSerializer(LogicalType... types) {
    ...
    private static ArrayDataSerializer arraySerializer(LogicalType elementType) {
        return new ArrayDataSerializer(elementType, fieldSerializerFor(elementType));
    }

    private static MapDataSerializer mapKeySerializer(LogicalType keyType) {
        LogicalType valueType = new IntType(false);
        return new MapDataSerializer(
                keyType, valueType, fieldSerializerFor(keyType), fieldSerializerFor(valueType));
    }

    private static MapDataSerializer mapValueSerializer(LogicalType valueType) {
        LogicalType keyType = new VarCharType(false, 5);
        return new MapDataSerializer(
                keyType, valueType, fieldSerializerFor(keyType), fieldSerializerFor(valueType));
    }
