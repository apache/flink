...
public class PythonTypeUtilsTest {
    ...
    public void testTypeInfoToProtoConverter() {
        ...
        for (Map.Entry<TypeInformation, FlinkFnApi.TypeInfo.TypeName> entry :
            ...
            assertEquals(
                    entry.getValue(),
                    PythonTypeUtils.TypeInfoToProtoConverter.getFieldType(entry.getKey())
                            .getTypeName());
        }

        TypeInformation primitiveIntegerArrayTypeInfo =
                PrimitiveArrayTypeInfo.INT_PRIMITIVE_ARRAY_TYPE_INFO;
        FlinkFnApi.TypeInfo.FieldType convertedFieldType =
                PythonTypeUtils.TypeInfoToProtoConverter.getFieldType(
        ...
        FlinkFnApi.TypeInfo.FieldType convertedBasicFieldType =
                PythonTypeUtils.TypeInfoToProtoConverter.getFieldType(basicIntegerArrayTypeInfo);
        assertEquals(
        ...
        assertEquals(
                convertedFieldType.getRowTypeInfo().getField(0).getType().getTypeName(),
                FlinkFnApi.TypeInfo.TypeName.INT);
        ...
        assertEquals(
                convertedFieldType.getTupleTypeInfo().getField(0).getType().getTypeName(),
                FlinkFnApi.TypeInfo.TypeName.INT);
    }
