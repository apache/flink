...
def _build_serialized_fn(udf_protos, output_indices=None):
    """
    ...
    if output_indices is not None:
        for index in output_indices:
            serialized_fn.output_indices.append(index)
...
class TestScalarFunctionCse(unittest.TestCase):
    ...
    def test_cse_sequential_execution(self):
        ...
        # only the projected results are emitted, selected by index
    ...
    def test_output_indices_reorder_results(self):
        """
        Flattening keeps post-order, so a nested sub-expression is evaluated before the call
        referencing it. When that sub-expression is itself projected, the projection order no
        longer matches the evaluation order and output_indices carries the permutation.

        Scenario: SELECT outer(inner(x)), inner(x)
        Evaluated: [inner(v[0]), outer(results[0])]  ->  output_indices = [1, 0]
        """
        inner_payload = _make_udf_payload(lambda x: x + 1)
        outer_payload = _make_udf_payload(lambda x: x * 10)

        inner_proto = _make_udf_proto(inner_payload, [_input_offset(0)])
        outer_proto = _make_udf_proto(outer_payload, [_input_ref_index(0)])

        op = ScalarFunctionOperation(
            _build_serialized_fn([inner_proto, outer_proto], output_indices=[1, 0]))

        self.assertEqual(op.func.__name__, '_sequential_execute')
        self.assertIn('return [results[1],results[0]]', op._generated_code)

        # inner(3) = 4, outer(4) = 40; projection order is [outer, inner]
        result = op.func([3])
        self.assertEqual(result[0], 40)
        self.assertEqual(result[1], 4)

    def test_output_indices_skip_intermediate_results(self):
        """
        A purely intermediate sub-expression is consumed by reference only and must not occupy an
        output column, otherwise it would shift the operator's output schema.

        Scenario: SELECT outer(inner(x))
        Evaluated: [inner(v[0]), outer(results[0])]  ->  output_indices = [1]
        """
        inner_payload = _make_udf_payload(lambda x: x + 1)
        outer_payload = _make_udf_payload(lambda x: x * 10)
        ...
        op = ScalarFunctionOperation(
            _build_serialized_fn([inner_proto, outer_proto], output_indices=[1]))

        self.assertEqual(op.func.__name__, '_sequential_execute')
        self.assertIn('return [results[1]]', op._generated_code)

        # only the outer result is emitted
        result = op.func([3])
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0], 40)

    def test_output_indices_without_result_ref(self):
        """
        output_indices must be honored even when no UDF consumes another result.

        This occurs when one evaluation supplies multiple output columns, for example when equal
        top-level calls from an older compiled plan are deduplicated during translation.
        """
        func_payload = _make_udf_payload(lambda x: x + 1)
        func_proto = _make_udf_proto(func_payload, [_input_offset(0)])

        op = ScalarFunctionOperation(
            _build_serialized_fn([func_proto], output_indices=[0, 0]))

        self.assertEqual(op.func.__name__, '_sequential_execute')
        self.assertIn('return [results[0],results[0]]', op._generated_code)
        self.assertEqual(op.func([3]), [4, 4])
