...
class FieldCoderImpl(ABC):
    ...
    def decode(self, encoded):
        return self.decode_from_stream(
            create_InputStream(encoded), len(encoded))


class IterableCoderImpl(BaseCoderImpl):
    """
    ...
    def encode_to_stream(self, value: List, out_stream, nested):
        for item in value:
...
class ValueCoderImpl(BaseCoderImpl):
    """
...
class ArrowCoderImpl(FieldCoderImpl):
    ...
    def decode_one_batch_from_stream(self, in_stream: create_InputStream, size: int) -> List:
...
class PickledBytesCoderImpl(FieldCoderImpl):
    ...
    def _decode_one_value_from_stream(self, in_stream: create_InputStream):
...
class TimeWindowCoderImpl(FieldCoderImpl):
    ...
    def encode_to_stream(self, value, out_stream):
        out_stream.write_bigendian_int64(value.start)
        out_stream.write_bigendian_int64(value.end)

    def decode_from_stream(self, in_stream, length=0):
        start = in_stream.read_bigendian_int64()
        end = in_stream.read_bigendian_int64()
...
class CountWindowCoderImpl(FieldCoderImpl):
    ...
    def encode_to_stream(self, value, out_stream):
        out_stream.write_bigendian_int64(value.id)

    def decode_from_stream(self, in_stream, length=0):
        return CountWindow(in_stream.read_bigendian_int64())
