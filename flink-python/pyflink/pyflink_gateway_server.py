...
def read_from_config(key, default_value, flink_conf_directory):
    ...
    if not os.path.isfile(flink_conf_file):
        # if flink-conf.yaml is not found, try to use flink-config.yaml instead
        flink_conf_file = os.path.join(flink_conf_directory, "flink-config.yaml")
        if not os.path.isfile(flink_conf_file):
            # if neither file exists, return default_value
            return default_value
        else:
            # if flink-config.yaml is found, use YAML parser to read values
