...
public class PythonStreamer extends Streamer {
	...
	private void startPython() throws IOException {
		...
		pb.command(pythonBinaryPath, "-O", "-B", planPath, "FLINK_KEY_EXECUTION", "" + server.getLocalPort());

		if (debug) {
			socket.setSoTimeout(0);
			LOG.info("Waiting for Python Process : " + function.getRuntimeContext().getTaskName()
					+ " Run python /tmp/flink" + FLINK_PYTHON_PLAN_NAME + " FLINK_KEY_EXECUTION " + server.getLocalPort());
		} else {
			process = pb.start();
