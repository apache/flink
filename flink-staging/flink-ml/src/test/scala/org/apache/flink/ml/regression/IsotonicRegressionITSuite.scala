...
class IsotonicRegressionITSuite extends FlatSpec with Matchers with FlinkTestBase {
	...
	it should "predict labels correctly with duplicate features" in {
		...
		val trainDS = env.fromCollection(
			Seq[(Double, Double, Double)]((2, 1, 1), (1, 1, 1), (4, 2, 1), (2, 2, 1), (6, 3, 1), (5, 3, 1)))
		val ir = new IsotonicRegression().setIsotonic(true)
		ir.fit(trainDS)
	...
	it should "predict labels correctly with duplicate features (antitonic)" in {
		...
		env.setParallelism(2)
		val trainDS = env.fromCollection(
			Seq[(Double, Double, Double)]((5, 1, 1), (6, 1, 1), (2, 2, 1), (4, 2, 1), (1, 3, 1), (2, 3, 1)))
	...
	private def runIsotonicRegression(labels: Seq[Double], weights: Seq[Double], isotonic: Boolean): IsotonicRegression = {
		val env = ExecutionEnvironment.getExecutionEnvironment
		env.setParallelism(2)
	...
	private def generateIsotonicInput(env: ExecutionEnvironment, labels: Seq[Double], weights: Seq[Double], reverse: Boolean): DataSet[
		(Double, Double, Double)] = {
		val tuples = labels.zipWithIndex.map { case (label, i) => (label, i.toDouble, weights(i)) }
		env.fromCollection(if (reverse) { tuples.reverse } else { tuples })
	}

	private def generateIsotonicInput(env: ExecutionEnvironment, labels: Seq[Double], reverse: Boolean = false): DataSet[
		(Double, Double, Double)] = {
