...
class KNNBenchmark_numPoints extends FlatSpec {
  ...
  val rSeq = Seq.fill(nFill)(DenseVector(r.nextFloat, r.nextFloat))
  /// GENERATE RANDOM SET OF POINTS IN [0,1]x[0,1]
  val trainingSet = env.fromCollection(rSeq)

  val rSeqTest = Seq.fill(nFill)(DenseVector(r.nextFloat, r.nextFloat))
  val testingSet = env.fromCollection(rSeqTest)

/////////////////////////////
    val rSeq2D = Seq.fill(nFill)(DenseVector(r.nextFloat, r.nextFloat))
    /// GENERATE RANDOM SET OF POINTS IN [0,1]x[0,1]
    val trainingSet2D = env.fromCollection(rSeq2D)

    val rSeqTest2D = Seq.fill(nFill)(DenseVector(r.nextFloat, r.nextFloat))
    val testingSet2D = env.fromCollection(rSeqTest2D)


  /////////////////////////////////////
  val rSeq3D = Seq.fill(2*nFill)(DenseVector(r.nextFloat, r.nextFloat))

  /// GENERATE RANDOM SET OF POINTS IN [0,1]x[0,1]
  val trainingSet3D = env.fromCollection(rSeq3D)

  val rSeqTest3D = Seq.fill(2*nFill)(DenseVector(r.nextFloat, r.nextFloat))
  val testingSet3D = env.fromCollection(rSeqTest3D)



  ///////////////////////////////
  val rSeq4D = Seq.fill(4*nFill)(DenseVector(r.nextFloat, r.nextFloat))

  /// GENERATE RANDOM SET OF POINTS IN [0,1]x[0,1]
  val trainingSet4D = env.fromCollection(rSeq4D)

  val rSeqTest4D = Seq.fill(4*nFill)(DenseVector(r.nextFloat, r.nextFloat))
  ...
  println("Elapsed time first nfill =       : " + (tf - t0)/1000000000 + "s")
  println("Elapsed time nfill =       : " + (tf2D - t02D)/1000000000 + "s")
  println("Elapsed time 2nfill =       : " + (tf3D - t03D)/1000000000 + "s")
  println("Elapsed time 4nfill =       : " + (tf4D - t04D)/1000000000 + "s")
