...
public class KeyedDataStream<T, KEY> extends DataStream<T> {
	 ...
	 */
	public KeyedWindowDataStream<T, KEY, TimeWindow> timeWindows(AbstractTime size) {
	 ...
	 * @param size The size of the window.
	 */
	public KeyedWindowDataStream<T, KEY, TimeWindow> timeWindows(AbstractTime size, AbstractTime slide) {
		AbstractTime actualSize = size.makeSpecificBasedOnTimeCharacteristic(environment.getStreamTimeCharacteristic());
