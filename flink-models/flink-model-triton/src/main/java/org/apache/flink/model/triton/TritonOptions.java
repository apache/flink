...
public class TritonOptions {
    ...
    public static final ConfigOption<Boolean> CONNECTION_REUSE_ENABLED =
            ConfigOptions.key("connection-reuse-enabled")
                    ...
                    .defaultValue(true)
                    .withDescription(
                            "Whether to enable HTTP connection reuse. When enabled, connections are kept alive "
                                    + "and reused for subsequent requests to the same endpoint, reducing connection "
    ...
    public static final ConfigOption<Duration> CONNECTION_TIMEOUT =
            ConfigOptions.key("connection-timeout")
                    ...
                    .withDescription(
                            "Timeout for establishing a new HTTP connection. This is separate from the "
                                    + "request timeout and only applies to the initial TCP connection establishment. "
    ...
    public static final ConfigOption<Integer> CONNECTION_POOL_MAX_TOTAL =
            ConfigOptions.key("connection-pool-max-total")
                    ...
                    .defaultValue(200)
                    .withDescription(
                            "Maximum number of HTTP connections allowed in the connection pool. "
                                    + "When the limit is reached, new requests will wait until a connection is released. "
                                    + "Defaults to 200. Only effective when connection-reuse-enabled is true.");
            ...
            ConfigOptions.key("connection-pool-max-idle")
                    ...
                    .withDescription(
                            "Maximum number of idle HTTP connections to keep in the pool. "
                                    + "Idle connections exceeding this limit will be closed. "
                                    + "Defaults to 50. Only effective when connection-reuse-enabled is true.");
    ...
    public static final ConfigOption<Duration> CONNECTION_POOL_KEEP_ALIVE =
            ConfigOptions.key("connection-pool-keep-alive")
                    ...
                    .withDescription(
                            "Duration to keep idle connections alive in the pool before closing them. "
                                    + "Longer durations reduce connection overhead but may hold resources unnecessarily. "
                                    + "Defaults to 5 minutes. Only effective when connection-reuse-enabled is true.");
    ...
    public static final ConfigOption<Boolean> CONNECTION_POOL_MONITORING_ENABLED =
            ConfigOptions.key("connection-pool-monitoring-enabled")
                    ...
                    .defaultValue(false)
                    .withDescription(
                            "Whether to enable connection pool metrics monitoring. When enabled, "
                                    + "metrics such as active connections, idle connections, and pending requests "
                                    + "will be exposed through Flink's metric system. Defaults to false. "
                                    + "Only effective when connection-reuse-enabled is true.");
}
