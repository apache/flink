...
public class QwenModelProviderFactory implements ModelProviderFactory {
    ...
    public ModelProvider createModelProvider(Context context) {
        ...
        helper.validate();
        String model = helper.getOptions().get(AbstractQwenModelFunction.MODEL);

        AsyncPredictFunction function;
        if (model.startsWith(QwenEmbeddingModelFunction.MODEL_PREFIX)) {
            function = new QwenEmbeddingModelFunction(context, helper.getOptions());
        } else if (model.startsWith(QwenChatModelFunction.MODEL_PREFIX)) {
    ...
    public Set<ConfigOption<?>> optionalOptions() {
        ...
        set.add(QwenEmbeddingModelFunction.DIMENSION);
        return set;
    }
